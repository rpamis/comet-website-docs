...
需要语义归纳且尚未经过复用的候选先进入 `trial`（试用）状态。用户明确提出的项目约定、已有项目指令、可直接核对的项目事实和经过成功复验的执行结果可以进入 `proven`。`trial` 成功帮助后续任务后也可以晋升为 `proven`。
...
Policy Compiler（策略编译器）根据内容是否可机器判定和使用方式，为 Project Policy 选择以下激活形式：
...
SQLite FTS5 提供 section 级全文索引和排序；有限 ripgrep 保留精确匹配、变化文件补充和故障回退能力。SQLite 索引损坏、锁冲突或 FTS 不可用时，系统可以重建读模型或使用有限源码搜索。
