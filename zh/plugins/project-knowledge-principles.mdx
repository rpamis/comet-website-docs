...
如何开启、配置与常用命令见[项目知识](/zh/plugins/project-rules)；本页说明项目知识如何从项目证据建立记录、核对来源并提供给任务。
...
| 组件                             | 职责                                               |
| -------------------------------- | -------------------------------------------------- |
| Corpus（项目语料）               | 定义可以拆分、索引和检索的 Markdown 来源集合       |
...
- **SQLite FTS 索引**可随时重建，用于提高候选召回效率。

Rule 由用户或团队维护，并由平台按作用域加载。Project Knowledge 可以在来源配置允许时引用 Rule 的依据，但不会写入、替换或接管 Rule。项目 Wiki 属于面向人的完整项目文档，也不在这条自动学习链中。

## Corpus：项目语料范围

Corpus（项目语料）定义 Local Provider 可以拆分、索引和检索的 Markdown 来源。默认来源包括：
...
默认语料范围只包含 Comet 可以明确识别的来源。目标平台支持的 Rule 文件或规则目录由平台管理；需要将其中的 Markdown 同时作为知识语料时，请通过 `include` 明确加入。

源码、manifest、配置和测试可用于建立项目模型，并保留为记录证据，但不会直接进入 Markdown 检索语料。Dashboard 的检索语料页展示实际参与检索的 Markdown；其他证据在对应知识记录中查看。
...
Project Policy Learner 在任务执行过程中处理沉淀下来的工程经验：
...
需要归纳总结的经验默认交给宿主 Agent 评审，使用方式见[项目知识](/zh/plugins/project-rules#从执行结果中沉淀项目策略)。新提炼的策略先进入 `trial`（试用）状态，成功帮助后续任务后再晋升为 `proven`；刷新来源不会代替实际应用验证。
...
系统按内容判断多条记录是否指向同一件事：内容更新会生成新版本并保留与旧版本的关系，减少近义重复。用户明确纠正的正文具有更高权威，自动提炼只能补充证据或形成待确认版本。

## Policy Compiler：策略激活

Policy Compiler（策略编译器）根据内容能否由程序自动判定，以及内容的使用方式，为 Project Policy 选择以下激活形式：
...
Local Provider 将这些通道组合为一条有限检索链：
...
SQLite FTS5 提供 section 级全文索引和排序；有限 ripgrep 在 Markdown 语料中保留精确匹配、变化文件补充和故障回退能力。SQLite 索引损坏、锁冲突或 FTS 不可用时，系统可以重建索引或使用有限 Markdown 搜索。

混合检索同时覆盖语义召回和精确匹配两条路径，减少大范围的盲目搜索，并提高完整修改范围的覆盖率。
...
- 将 repository 级来源和用户明确给出的记录排在自动推断之前；
- 提高具体范围和最近成功应用内容的排序；
- 降低被纠正或参与失败内容的排序。
...
Agent 通过 `expand` 获取完整正文、来源和 verification。单次字符预算只限制当前任务注入的内容，不限制 Provider 的记录总量。
...
- 查询只发送有限的 task、path、phase、operation 和 ID selector；
...
| 平台 Rule 文件或规则目录   | 由平台按作用域加载            | 依赖 Agent 解释与遵循                   | 团队指令和行为要求               |
| Hook / Guard               | 作为运行时事件或命令执行      | 可以观察、阻断或调整流程                | 权限、安全、工作流边界           |
| linter / test / build / CI | 提供诊断或验证结果            | 使用退出码和项目规则判断结果            | 可自动判断的工程约束             |

你可以继续通过目标平台支持的 Rule 载体编写项目约束，平台仍负责加载。Codex 的 `AGENTS.md`、Claude Code 的 `CLAUDE.md` 和 Cursor 的 `.cursor/rules` 都只是具体示例。Comet 复用这些 Rule 和项目现有检查；项目知识记录约束存在的依据、适用路径和验证命令，确定性判断仍由对应执行器完成。
