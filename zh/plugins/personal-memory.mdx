...
Dashboard 的**个人记忆**页面会显示记录类型、作用范围、来源摘要、状态、应用原因和最近结果。当前有效记录与历史记录分开显示。被遗忘或冲突的记录仍可用于追踪和回滚，但不会作为当前个人记忆提供给任务。你可以新增、纠正、遗忘、回滚或展开完整信息。
...
`remember` 用于明确的长期保存要求。`observe` 用于记录 Agent 发现的稳定协作方式，不应写入任务摘要、进度、命令输出或测试结果。`forget` 默认保留回滚能力；永久删除需要显式使用 `--permanent`，会清除对应正文、历史和关联状态，并保留阻止再次自动学习的遗忘标记。同类但文本不同的其他记忆会保留，仍存在的冲突不会自动解除。

**跨设备同步**：个人记忆默认保存在本机。换设备使用时，先用 `comet memory remote --set <Git 地址>` 绑定一个专用记忆仓库，之后在每台设备上运行 `comet memory sync` 拉取并推送最新记忆。

首次同步可以连接空仓库或已有记忆分支，并保留远端内容。同步失败会返回错误；连接失败与合并冲突会分别提示，出现冲突时先解决冲突再重试。
