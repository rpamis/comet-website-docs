...
项目知识不替代 Rule。你可以继续通过目标平台支持的 Rule 文件或规则目录编写团队指令和行为约束，例如 Codex 的 `AGENTS.md`、Claude Code 的 `CLAUDE.md`、Cursor 的 `.cursor/rules`，以及其他平台提供的 Rule 载体。平台仍按自己的作用域规则加载这些内容。
...
模块记录包含入口、依赖、调用方、注册点和对应测试，供 Agent 按任务展开查看。Local Provider 会补充缺失或过期的模型记录，并整理重复的自动生成历史。
...
Project Policy 只有在关联的验证命令当前存在且已成功执行后，才能进入 `enforced`（已验证约束）状态。在 Native 工作流中，这些验证命令会并入 Verify 阶段的必要检查一起执行（见[验证与修复](/zh/native/verification-and-repair)），通过后策略才升级。该状态表示项目已有工具能够检查这条策略，自然语言正文仍作为上下文提供给 Agent。
...
| Rule / Agent 指令 | 规定 Agent 在当前作用域内的行为      | 目标平台支持的 Rule 文件或规则目录      | 平台加载并提供给 Agent   |
| Hook / Guard      | 在动作发生时观察、阻断或调整流程     | Hook、权限规则、Comet Guard             | 平台或 Runtime           |
...
额外路径均相对于项目根目录解析。只有 Markdown 进入全文检索语料；源码、配置和测试可以作为模型记录的证据，但不会因此成为检索语料。默认语料范围不会自动包含目标平台的所有 Rule 文件或规则目录，需要时可通过 `include` 加入其中的 Markdown。
...
Comet 从这些事件中提取候选记录，并在写入前重新核对来源。需要归纳总结的经验可交给当前宿主 Agent 评审，无需另配模型或 API Key。Agent 通过以下命令查看待评审证据并提交评审结果：

```bash
comet knowledge review . --json
comet knowledge review . --id <评审标识> --file <评审结果.json>
```

首条命令会返回待评审证据和提交格式。新提炼的知识先进入 `trial`，成功用于后续任务后再晋升；来源刷新只核对有效性，不会将未经试用的经验直接提升为已确认知识。
...
刷新时，Comet 会重新核对来源内容，即使文件大小和修改时间没有变化，也能识别内容变化。Dashboard 的状态统计覆盖项目中的全部记录，当前列表的显示数量不代表记录总量。

项目知识为 Agent 提供有依据的任务线索。Agent 仍会读取当前代码、配置和测试，确认本次修改的实际情况。

## 任务召回与提供

Context Director（上下文调度器）会按当前 project、path、task、operation、phase、类型和状态过滤候选，再结合 Markdown 全文检索、有限精确搜索、项目关系、来源状态和应用结果进行排序。尚未指定文件路径时，Agent 也可以先发现相关知识，再展开适用范围、来源和验证方式；已明确的范围不匹配仍会被排除。
...
**排查召回**：按 Provider → Record → Query → Context 的顺序检查：来源是否已进入默认 Corpus 或 `knowledge.local.include`；Record 的 project、path、operation、phase 和 lifecycle 是否与任务匹配；来源版本是否仍然有效；Query 是否返回相关候选；Context Manifest 中的 `whyApplied` 和 application history 是否符合预期。
