...
Project Policy 只有在关联当前存在且成功执行的确定性验证入口后，才能进入 `enforced`（已验证约束）状态。该状态表示项目已有工具能够检查这条策略，自然语言正文仍作为上下文提供给 Agent。
...
> 项目知识提供理解，Rule 提供指令，Hook 和工程检查提供确定性执行。
...
额外路径相对项目根目录。默认语料范围不会自动包含目标平台的所有 Rule 文件或规则目录。需要将其中的 Markdown 同时作为知识来源时，请通过 `include` 明确加入。
...
Comet 从这些事件中提取候选记录，并在写入前重新核对来源。需要语义归纳且尚未经过复用的候选先进入 `trial`；具有确定性来源或成功复验结果的内容可以直接进入 `proven`。`trial` 成功帮助后续任务后也可以晋升为 `proven`。因此，项目知识随实际工作逐步积累，不依赖一次性生成完整项目说明。
