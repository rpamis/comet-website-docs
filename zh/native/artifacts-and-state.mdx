...
每个 active 或 archived Native change 都有独立的 `comet-state.yaml`。它记录 phase、revision、approval、`approved_contract_hash`、verification 与规格操作等机器状态。正常流程只有在 `approval: confirmed` 后进入 Build；`implicit` 只用于兼容旧 change，不能证明用户已经确认，离开 Build 前必须升级为明确确认。`approved_contract_hash` 把 approval 绑定到当时的 brief/spec contract；后续 contract 漂移也必须取得用户重新确认。不要手改这些字段，也不要把它与 Classic change 的 `.comet.yaml` 混为一谈。
...
Pattern 语义、三套可复制配置以及如何判断“提高预算还是排除文件”，见 [Native 配置](/zh/native/configuration#内容快照范围与预算) 和 [Native 常见问题](/zh/native/faq#内容快照与实现范围)。
