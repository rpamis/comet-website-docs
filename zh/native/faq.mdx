---
title: 'Native 常见问题'
description: '解答 Native 模式选择、需求澄清、状态恢复、内容快照、验证与冲突处理的常见问题。'
---
...
<AccordionGroup>
  <Accordion title="Native 和 Classic 应该怎么选">
    若你的主力模型是 Fable 5、GPT-5.6 这一档，默认 Native：强模型被方法论束缚是最大的浪费，Native 把方法选择权还给它，只锁住结果与证据。

    Classic 面向能力档更低、需要更明确过程约束的模型，通过 OpenSpec 与 Superpowers 提供 open、design、build、verify、archive 五阶段治理。两套工作流的选择由项目配置决定，不由 Comet 根据任务大小临时猜测。详见 [Native 工作流](/zh/concepts/native-workflow)。

  </Accordion>

  <Accordion title="Native 是否依赖 OpenSpec、Superpowers 或其他外部 Skill">
    不依赖。Native 的需求产物、状态机、Guard、恢复和验证协议都由 Comet 自己提供，零外部 Skill。日常仍统一调用
    `/comet`；它读取 `.comet/config.yaml` 的 `default_workflow` 后进入项目选定的永久入口。
</Accordion>

  <Accordion title="为什么 Native 只有四个阶段">
    四个阶段对应软件工程的四个不可合并的认知边界：Shape 把目标说清楚（契约），Build 把东西做出来（实现），Verify 证明做对了（证据），Archive 把成果固化进基线（事务）。

    阶段更少不代表可以跳过需求或验证。Runtime 仍会检查 brief、target spec、用户确认、implementation scope 和验证证据。
...
<AccordionGroup>
  <Accordion title="Sequential 和 Batch 澄清模式有什么区别">
    `sequential` 每轮只问一个最上游决定，适合前一个答案会改变后续问题的需求。`batch` 会一次提出当前前置条件都已确定的问题，适合各决定相对独立、想减少往返的需求。

    两种模式都要求在进入 Build 前确认完整共享理解。详细的模式机制和选择建议见 [澄清模式与共享理解](/zh/native/clarification-modes)。

  </Accordion>

  <Accordion title="为什么问题都回答完了还要确认共享理解">
    单个答案正确，不代表组合后的范围、非目标、默认行为和验收标准没有矛盾。最终确认把用户批准绑定到当时的
    brief 与 target spec。之后如果 contract 发生变化，旧确认会失效，Native 会重新请求确认。
</Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="哪些文件可以手工修改">
    brief、target spec 和 verification report 是用户可读产物，但应让 `/comet` 在对应阶段协调修改。`comet-state.yaml`、`run-state.json`、trajectory、锁、hash、snapshot evidence 和 `.comet/current-change.json` 都是 Runtime 管理的机器状态，不要手改。
  </Accordion>
</AccordionGroup>

## 内容快照与 implementation scope

<AccordionGroup>
  <Accordion title="baseline snapshot 和 current snapshot 分别是什么">
    创建 change 时，Native 对纳入范围内的真实文件内容做流式哈希，生成 `baseline-manifest.json`。它回答"开始实现时项目是什么状态"。
  ...
  <Accordion title="include 和 exclude 有什么作用">
    ...
    文件必须至少匹配一个 `include`，并且不能匹配任何 `exclude`，因此 `exclude` 最终优先。它们会写入 baseline 的可审计范围策略，不是随手减少扫描量的隐藏开关。
  ...
  <Accordion title="普通项目、Monorepo、大型仓库怎么配置 snapshot">
    普通项目优先用默认完整范围，最不容易遗漏共享配置、测试或锁文件；Monorepo 需同时纳入目标应用、共享 package、测试和根级构建文件；大型仓库可以保持 include 宽泛，只排除与交付无关的重型内容。

    三套现成 YAML recipe 和详细说明见 [Native 配置](/zh/native/configuration#内容快照范围与预算)。
...
<AccordionGroup>
  ...
  <Accordion title="Verify 中发现项目又变了会怎样">
    ...
    scope。只有 brief 或 target spec 也变化时才重新请求用户确认；仅实现变化不会制造额外确认。详见
    [验证证据与自主修复](/zh/native/verification-and-repair)。
  </Accordion>

  <Accordion title="多个 change 修改同一文件怎么办">
    每个 Native change 有独立 baseline、target spec 和状态。并发修改造成 canonical spec
...
继续阅读：[Native 配置](/zh/native/configuration)、[产物与状态](/zh/native/artifacts-and-state) 和 [验证证据与自主修复](/zh/native/verification-and-repair)。
