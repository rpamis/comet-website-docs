...
遇到中断或错误，先只读诊断，再按 Runtime 返回的具体动作恢复。Native 的恢复依据是项目配置、正式产物和 Runtime 状态——都是磁盘事实，不是聊天记录。硬规则：不要手工改 phase、hash、journal 或 selection 来绕过检查。
...
继续阅读：[连续推进与 Checkpoint](/zh/native/continuation-and-checkpoints) 和 [Native 安全与恢复](/zh/native/safety-and-recovery)。
