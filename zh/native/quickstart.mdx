...
`--workflow both` 会安装两套独立工作流，但 `/comet` 默认仍进入 Native。`--root docs` 只改变 Native 产物位置，使其位于 `docs/comet/`；不会改变 Classic 目录。
...
Native 会先调查仓库事实。只有仍会改变范围、输出、默认行为、兼容性、风险或不可逆结果的决定才交给用户，并且一次只问一个最上游问题，同时给出推荐、理由和选项影响。
...
下一步可阅读 [Native 工作流](/zh/concepts/native-workflow)、[Native 配置](/zh/native/configuration)、[产物与状态](/zh/native/artifacts-and-state) 和 [安全与恢复](/zh/native/safety-and-recovery)。所有确定性命令集中在 [Native CLI](/zh/cli/native)。
