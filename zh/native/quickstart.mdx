...
`snapshot` 定义新 change 的可审计项目范围和读取预算。普通项目先保留默认完整范围；单体仓库或大型仓库应在理解依赖边界后再配置纳入与排除范围。详见 [Native 配置](/zh/native/configuration#内容快照范围与预算)。
...
模型不会把“合理默认值”、相邻实现或行业惯例当作用户决定。澄清完成后，结论进入需求简述与完整目标规格，再继续 Build。
