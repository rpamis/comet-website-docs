...
确认拆分之前，不会创建 child、worktree 或独立会话。确认 Supervisor 拆分时，用户只选择一次推进方式：多会话协作（推荐）或单会话推进。这个选择会作为 `coordination_mode` 写入 `comet-state.yaml`；之后恢复流程，或因需求变化重新进入 Shape，都会沿用已有选择，不重复询问。确认之后，Runtime 只会推进当前依赖已经满足的 child。
...
如果宿主无法创建用户可见的独立任务，或恢复时原来的任务已经消失，`multi-session` 会自动改用 subagent，不再询问推进方式，也不会自动改为单会话。已经派发但丢失的任务会先通过 `supervisor-cancel` 使旧 `runId` 失效，再用 Runtime 返回的新任务包和新 `runId` 派发 subagent；旧执行的迟到结果会被拒绝。subagent 也不可用时，Comet 会报告真实阻塞。拆分范围、依赖和验收语义保持不变。
...
Agent Teams 不可用、不是交互式会话，或恢复时原来的团队已经消失时，Comet 会重新读取 Native Runtime。持久化为 `multi-session` 时会自动改用 subagent，不再要求用户重新选择；已经派发但丢失的 teammate 会按相同的 `supervisor-cancel` 和新 `runId` 协议重新派发。subagent 也不可用时才报告阻塞，不会静默改为单会话推进。
