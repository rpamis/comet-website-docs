...
- 无进展 override 只允许一次，并与当前签名绑定；仍有 override 时由 Agent 提出明确的新修复假设；
- 同一 contract 的 Verify fail 默认最多 5 次，可通过 `native.max_verify_failures` 配置正整数；override 或验证预算耗尽后才要求用户决定。

重复缺口恢复与总失败预算独立生效。只有用户确认后的 contract 变化会开始新的目标周期；恢复条件不按机械 phase 往返次数计算。
