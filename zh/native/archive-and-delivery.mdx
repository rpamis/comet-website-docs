...
进入 Archive-ready 后，Runtime 会先在 `continuation` 中给出一次完整的 `archive --dry-run`。隔离工作区尚未选择交付方式时，预检查会带上对应的 `--finish` 选项。

预检查会同时检查归档内容、当前工作区和生成文件，并在不能继续时列出阻塞原因、涉及路径和下一条确认命令。`--dry-run` 不会归档、提交、合并或推送；只有预检查确认可以继续时，才执行 Runtime 返回的唯一确认命令。
