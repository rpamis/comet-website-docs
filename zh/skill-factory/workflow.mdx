...
<p align="center">
  <img src="/assets/skill-factory-workflow-illustrations/01-comet-any-long-chain.png" alt="小鱼沿着 /comet-any 长路线经过恢复、方案确认、生成、评估、发布和预览分发" width="800" />
</p>

<p align="center">/comet-any 的主线很长，但用户只需要跟着恢复、确认、评估和发布门禁走</p>
...
从三种起点里选一个（见[Skill Factory 概览 · 三种起点](/zh/skill-factory/overview#三种起点)），它们对应三种 **Skill Maker 意图**（`skillMakerIntent`）：
...
不管选哪种起点，`/comet-any` 最终都会把目标编译成同一种 **Workflow Contract**（`workflow.kind` 为 `comet-five-phase-overlay` 或 `workflow-kernel`）。详见[Workflow Contract](/zh/skill-factory/overview#workflow-contract所有路径的共同模型)。
...
`/comet-any` 把目标编译成 **Workflow Contract**，按偏好提出组合方案，并标注每个节点的职责、实现 Skill、Required Skill Call 和 Output Schema。这一页就是"**Skill Maker 方案确认页**"。

方案确认页至少包含这些区块：

| 区块 | 内容 |
| --- | --- |
| 你在做什么 | Skill Maker 意图、Skill 名、目标 |
...
`plan.json` 的主输入是 `workflow`（Workflow Contract），不再用 `callChain` / `stageNames`：
...
```json
{
  ...
  "workflow": {
    ...
    "nodes": {
      ...
      "review": {
        "requiredSkillCalls": [
          { "skill": "whitebox-code-standard", "scope": "review" }
        ]
      }
    }
  },
...
| 字段 | 说明 |
| --- | --- |
| `skillMakerIntent` | `customize-comet` / `new-skill` / `upgrade-existing`，对应三种起点；不写会按 `workflow.kind` 和 `mode` 推断 |
...
<Warning>
  Factory plan 不允许未知字段；<code>workflow</code> 必须是合法的 Workflow Contract。读取时会立刻调用 <code>normalizeWorkflowDefinition</code> 校验，不通过会直接报错（见[Skill Factory 概览 · 校验失败码](/zh/skill-factory/overview#校验失败码validation-findings)）。
</Warning>
...
`/comet-any` 通过**创作车道**（authoring lanes）组装产物。生成物包含入口 `SKILL.md`、每个 Workflow Node 对应的内部 Skill，并写入：
