...
<p align="center">
  <img src="/assets/skill-factory-publishing-illustrations/01-evidence-to-distribute-preview.png" alt="小鱼把 eval evidence 经过 review、approval、publish 和 distribute preview 逐步盖章放行" width="800" />
</p>
...
| 阻塞码 | 含义 | 恢复建议 |
| --- | --- | --- |
| `[candidate]` | 还有未解决的缺失/歧义候选 | 用 `comet bundle factory-resolve` 解决候选 |
| `[preference]` | （strict）required Skill 缺失/歧义；偏好漂移 | 确认偏好或重新生成 |
| `[proposal]` | proposal 未确认 | `comet bundle factory-init ... --confirmed-proposal` |
...
<Note>
  每个 blocker 都带 <code>nextAction: { label, command }</code>，直接给出恢复用的 CLI 命令。看 <code>comet publish status</code> 的输出就能知道下一步该跑什么。
</Note>
