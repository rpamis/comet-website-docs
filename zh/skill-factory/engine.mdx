...
<p align="center">
  <img src="/assets/skill-factory-engine-illustrations/01-engine-pending-action-loop.png" alt="小鱼记录 Engine 提议的 pending action，外部执行后再通过 resume 提交 outcome" width="800" />
</p>
...
- **必需车道**是否齐全（`workflow-entry` / `skill-core` / `script-contract` / `reference` / `skill-review`，Engine 启用时还有 `eval`）。
...
| `comet/runtime/classic/checks.yaml` | runtime eval（`classic-completed`：completion 时 `status == completed`） |

它**不是**一个顶层、像用户 Skill 一样的 `comet-classic/` 目录——安装产物只暴露这三个 YAML，由 `assets/manifest.json` 的 `internalSkills` 注册，运行时由 Engine 加载。所有 `/comet*` 命令的用户面行为完全不变。
