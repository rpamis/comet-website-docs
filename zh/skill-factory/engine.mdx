...
description: "理解 Comet Skill 的三类来源、Skill 包结构、Engine 运行循环、pending action/resume 机制、不可变快照、guardrails、runtime evals，以及 /comet-any 生成物和 Engine 的关系。"
---

<Info>
  这是进阶内容，面向想深入理解 `/comet-any` 生成物和 `comet skill` 底层机制的用户。经典 Spec 模式的 `/comet` 用户不需要理解 Engine 就能使用五阶段工作流。
</Info>

Comet 的 Skill 不只是 Markdown 提示词。0.4.0-beta.1 起，Comet 内置了 **Skill Engine**——一个确定性运行时，负责执行、约束、持久化、恢复和评估 Comet Skill。它为多步骤 Skill 提供稳定内容 hash、不可变快照、pending action 恢复、guardrails 和 runtime eval。

## 三类 Skill

| 类型 | 来源 | 常见入口 | 说明 |
| --- | --- | --- | --- |
| 内置工作流 Skill | 随包分发在 `assets/skills/` | `/comet`、`/comet-open`、`/comet-any` | 驱动五阶段流程和 Skill Factory |
| 项目 Skill | 安装到 `.comet/skills/<name>` | `comet skill install` | 按名称覆盖内置 Skill，无效覆盖 fail closed |
...
<Warning>
  项目 Skill 按名称覆盖内置 Skill 时，如果项目 Skill 无效，会直接失败而不是回退到内置。这避免"你以为在用自定义版本，实际静默用了内置"的问题。
</Warning>

## Skill 包结构

一个 Engine-enabled 的 Skill 包不只是 `SKILL.md`，还包含一个 `comet/` 控制面：
...
│   ├── guardrails.yaml      # 运行时约束（可选）
│   ├── checks.yaml          # runtime evals（与 evals.yaml 二选一）
│   ├── evals.yaml           # runtime evals（与 checks.yaml 二选一）
...
| `comet/checks.yaml` 或 `comet/evals.yaml` | 运行期 | runtime evals（progress/step/completion），二选一 | `loadSkillPackage` → `SkillPackage.evals` |
| `comet/eval.yaml` | 创建期 | 创建期评估 manifest（benchmark、grader、review） | `comet eval` / Bundle Factory |

<Note>
  <strong>容易混淆的命名：</strong>复数 <code>evals.yaml</code>（或 <code>checks.yaml</code>）是运行期 eval，由 Engine 在运行时检查；单数 <code>eval.yaml</code> 是创建期评估 manifest，由 <code>comet eval</code> 在发布前验证。两者生命周期不同，不能混为一谈。<code>checks.yaml</code> 和 <code>evals.yaml</code> 不能同时存在。
</Note>
...
<Tip>
  如果你写的是多步骤、需要恢复的 Skill，用 deterministic。adaptive 是为 Agent 自主决策场景准备的，目前 loop-ready 但尚未通过 CLI 暴露。
</Tip>
...
flowchart TD
  ...
  G --> H["外部执行动作"]
  H --> I["comet skill resume<br/>提交 outcome"]
  I --> J{"outcome 成功?"}
...
`comet skill eval` 按需对持久化的 Run state 和 artifacts 重跑 evals，逐项输出 `PASS`/`FAIL`：
...
```bash
comet skill resume --run-id demo-run --upgrade my-skill --project .
```
...
<Warning>
  动态重新规划<strong>不能放宽 guardrails</strong>。允许列表和预算由 Skill 固定，运行时的 Agent 不能协商扩大。这是 Engine 安全性的核心——Skill 定义了边界，执行者只能在边界内行动。
</Warning>
...
runtime evals 是 Engine 在运行时检查 Run 是否满足条件，和创建期 eval（`comet eval`）不同。
...
```yaml
# comet-classic 的 runtime eval（evals.yaml）
runtime:
...
<Warning>
  <code>comet skill eval</code> 只检查某次 Engine Run 的完成度，不是通用 Skill 评估。评估一个 Skill 的产品能力能否发布，用 <a href="/zh/cli/eval">comet eval</a>。详见 <a href="/zh/eval/runtime">Runtime eval</a>。
</Warning>

## 创作车道（authoring lanes）

`/comet-any` 生成 Skill 包时，通过**创作车道**（authoring lanes）组装产物，而不是一次性渲染所有文件。每个车道负责一类产物，有自己的作者（deterministic-adapter 或 subagent）。

| 车道 | 产出 | 关键 artifact |
| --- | --- | --- |
| `skill-core` | 入口 Skill 和各阶段内部 Skill | `SKILL.md`、各内部阶段 `SKILL.md` |
| `script-contract` | 控制面脚本 | `scripts/workflow-state.mjs`、`workflow-guard.mjs`、`workflow-handoff.mjs` |
| `reference` | 真实来源和组合证据 | `reference/resolved-skills.json`、`workflow-protocol.json`、`composition-report.md`、`authoring-lanes.json` |
| `pause-points` | 决策点和恢复路径 | `reference/decision-points.md`、`reference/recovery.md` |
| `eval` | 评估清单（Engine 启用时） | `comet/eval.yaml` |
| `skill-review` | 作者自审（最后跑） | `reference/skill-review.md`、`authoring-lanes.json` |

每个车道的产出都会带 `protocolHash`，必须等于 `workflowProtocolHash(workflow)`，否则抛 `Factory authoring protocol hash drift`。

### 评审门禁

生成完成后，`reviewFactoryArtifactProposals` 是强制评审门禁。它会检查：

- **必需车道**是否齐全（`skill-core` / `script-contract` / `reference` / `pause-points` / `skill-review`，Engine 启用时还有 `eval`）。
- **必需 artifact** 是否存在（`SKILL.md`、`reference/workflow-protocol.json`、`decision-points.md`、`recovery.md`、三个控制面脚本等）。
- **必需 claim** 是否存在且引用的 artifact 存在（`workflow-entry`、`script:workflow-state` 等）。
- comet-overlay 模式下五阶段是否齐全（`missing-comet-phase`）。

如果评审不通过（`passed === false`），`generateFactorySkillPackage` 抛 `Generated Skill package failed authoring review`，**不写任何文件**。

<Tip>
  这套车道设计是为了让平台可以用原生 subagent 并行产出 artifact（每个 subagent 只读自己的 brief）。当平台不支持 subagent 时，相同的 brief 会内联运行作为兜底。普通用户不需要关心这些，理解"每个产物都有作者和评审"即可。
</Tip>

## Run state 存储

Run state 是 machine-owned 的，不要手工编辑。

### 两种存储位置

| runnerMode | 存储位置 | 特点 |
| --- | --- | --- |
| `change` | `<changeDir>/.comet/` | 绑定 OpenSpec change 目录，一个目录一个 Run。经典 `/comet` 工作流用这种。 |
...
经典 `/comet` 五阶段工作流在底层由 Engine 驱动，但用户不需要直接操作 Engine。

```mermaid
flowchart LR
  A["/comet"] --> B[".comet.yaml 用户可见状态"]
  B --> C["run_id 链接"]
...
<Note>
  <code>comet-classic</code> 本身是一个 deterministic Skill，有 30 个步骤覆盖 full/hotfix/tweak 三种 profile。它是内部 Skill——用户从不直接调用它，而是用 <code>/comet</code>、<code>/comet-open</code> 等，Engine 在底层运行。
</Note>
...
- [工作流概念](/zh/concepts/workflow) — `/comet` 如何在底层使用 Engine
- [comet skill](/zh/cli/skill) — 完整命令参考
- [Runtime eval](/zh/eval/runtime) — `comet skill eval` 和 `comet eval` 的区别
- [Skill Factory 概览](/zh/skill-factory/overview) — 用 `/comet-any` 创建 Engine-enabled Skill
