...
`/comet-any` 也会在偏好不存在时主动扫描平台 Skill inventory，按能力分组推荐默认偏好，并询问你是否保存为项目级偏好。`factory-guide` 返回的 `preference` 字段会标记偏好状态为 `missing` / `present` / `invalid`。
      ...
      "sources": [
        {
          ...
          "description": "...",
          "skillMd": "...",
          "references": [{ "path": "...", "contentHash": "..." }],
          "scripts": [{ "path": "...", "sideEffect": "none" }],
          "hash": "..."
        }
      ]
    }
  ...
  "sourceSummaries": [
    {
      ...
      "preferenceIndex": 1,
      "status": "available",
      "source": { "name": "...", "platform": "...", "root": "...", "hash": "..." },
      "summary": "（从真实 SKILL.md 正文提炼，≤600 字符）"
    }
  ],
  "stageNames": [{ "skill": "writing-plans", "name": "review-workflow-plan" }],
  "preference": { "mode": "advisory", "source": "...", "hash": "..." }
}
```

`sourceSummaries` 是从真实 `SKILL.md` 正文提炼出的内容摘要（去掉 frontmatter、代码块、标题，取前 3 段散文）。

<Tip>
  这个文件是<strong>"不是按名字猜的"</strong>的证据。准备发布或审查时优先看它——它证明组合基于本地真实内容。每个候选的 <code>status</code> 会是 <code>available</code> / <code>missing</code> / <code>ambiguous</code> 之一。
</Tip>

## preferenceHash 和偏好漂移

Comet 对偏好文件计算一个 SHA-256 hash（`preferenceHash`），存入 Factory metadata。`.comet/skill-preferences.yaml` 在 Factory 初始化后发生变化时（`preferenceHash` 变化）：
...
- [Skill Factory 概览](/zh/skill-factory/overview) — 三种起点和受保护边界
