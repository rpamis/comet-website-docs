...
description: "深入理解 /comet-any（Skill Maker）如何用 Workflow Contract 把任意 Skill 编排成可验证、可评审、可分发的 Comet-native Skill，三种起点、受保护边界、Workflow Node 和发布门禁。"
---
...
`/comet-any`（也叫 **Skill Maker**）是 Comet 的 Skill Factory。你描述想要的工作流，它读取真实本地 Skill、把目标编译成一份 **Workflow Contract**，再生成**可验证、可评审、可分发**的 Skill Bundle，并把后端复杂度（Bundle、Factory、composition、resolved-skills）隐藏在内部。
...
用户第一层只需要从三种起点里选一个。它们对应三种 **Skill Maker 意图**（`skillMakerIntent`），后端会把每条路径都编译到同一种 Workflow Contract：

| 起点（用户层） | Skill Maker 意图 | 含义 | 典型场景 |
| --- | --- | --- | --- |
| **基于 Comet 现有 Skill 的五阶段定制** | `customize-comet` | 在 `/comet` 受保护边界内做增量调整 | 给 `/comet` 加一个评审环节、要求用组件库 |
...
不管你选哪种起点，`/comet-any` 最终都会把目标编译成同一种 **Workflow Contract**。这是理解整个 Skill Factory 的核心概念：
...
| **Required Skill Call** | 要求节点内**必须调用**某个 Skill，但**不替换**节点的实现。例如要求 `execute` 必须调用 `elementui` |
...
| **workflow-protocol.json** | 生成包的**唯一运行事实源**，runtime / eval / review / readiness 都读它 |

<p align="center">
  <img src="/assets/skill-factory-overview-illustrations/01-workflow-contract.png" alt="小鱼把新建、升级和组合三种起点整理到同一份 Workflow Contract 模板" width="800" />
</p>
...
| `verify` | 运行验证、调和分支、决定是否完成 | control | `require`、`augment` |
...
## 方案示例：给 /comet 加组件库和白盒审查

下面这个 `plan.json` 用 Required Skill Call 给 `execute`/`subagent-execute` 加组件库、给 `review` 加白盒审查——**不替换任何节点实现**：
...
关键点：`execute`/`subagent-execute`/`review` 的实现 Skill 没变，只是在节点内部**强制调用**额外 Skill，并通过 Handoff / Output Schema 收回 evidence。
...
7. 先展示 **Skill Maker 方案确认页**，用户确认后才写 draft。
8. 生成 entry Skill、每个节点的内部 Skill、`reference/workflow-protocol.json` 和 scripts。
...
<bundle-draft>/
  ...
  skills/<entry-skill>/
    ...
    reference/
      workflow-protocol.json          # 唯一运行事实源（runtime/eval/review/readiness 共用）
      ...
      authoring-lanes.json            # 创作车道记录（含 protocolHash）
...
| `reference/workflow-protocol.json` | 创建期 + 运行期 | **唯一运行事实源**：节点、边、Output Schema、state、evals |
...
| `comet/checks.yaml` | 运行期 | runtime eval（progress/step/completion） |
| `comet/eval.yaml` | 创建期 | Eval manifest（`comet eval` 的输入） |

<Tip>
  <strong>workflow-protocol.json 是核心。</strong>runtime、eval、review、publish readiness 全部读这一份协议，保证"创建时校验通过"和"运行时能恢复"指向同一个事实。
</Tip>
