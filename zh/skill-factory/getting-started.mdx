...
`/comet-any`（也叫 **Skill Maker**）是 Comet 创建、优化和组合可复用 Skill 的唯一入口。本页是它的最短上手路径——只讲怎么用，不讲内部机制。想深入理解再看[进阶内容](#进阶内容)。
...
<Note>
  <code>/comet-any</code> 是 <strong>用户唯一需要记住的入口</strong>。底层的 <code>comet bundle</code>、<code>comet publish</code> 命令是它的确定性后端，普通用户不需要手动跑。
</Note>
...
你只需要调用 `/comet-any`，它会在内部帮你调度后端命令、维护状态、推进发布门禁。下面用一次真实创建走完整条链路。

<p align="center">
  <img src="/assets/skill-factory-getting-started-illustrations/01-skill-maker-mainline.png" alt="小鱼把 Skill 卡片从 /comet-any 方案确认带到 eval 和 publish" width="800" />
</p>

<p align="center">`/comet-any` 负责创建 Skill，`comet eval` 给证据，`comet publish` 负责发布和分发</p>
...
`/comet-any` 会让你从**三种起点**里选一个。这是 Skill Maker 的第一层选择，决定了整个生成方式：

| 起点 | Skill Maker 意图 | 含义 | 典型场景 |
| --- | --- | --- | --- |
...
不管选哪种起点，`/comet-any` 最终都会把目标编译成同一种 **Workflow Contract**（由 Workflow Node、Skill Binding、Required Skill Call、Output Schema、Guardrail、Handoff 组成）。详见[Skill Factory 概览](/zh/skill-factory/overview#workflow-contract所有路径的共同模型)。
...
## 第三步：确认组合方案（Skill Maker 确认页）

`/comet-any` 会读取项目里的真实 Skill，把目标编译成 **Workflow Contract**，生成一个**组合方案**，然后**暂停等你确认**。这一页叫"Skill Maker 方案确认页"，是整个流程的关键检查点。它会展示：

- 你在做什么类型的 Skill（Skill Maker 意图）、目标是什么
...
完整的发布状态链和门禁见[发布和分发 Skill](/zh/skill-factory/publishing)。
...
flowchart TD
    ...
    Resolve -->|否| Proposal["编译 Workflow Contract<br/>展示 Skill Maker 确认页"]
    AskUser --> Proposal
    Proposal --> Confirm{"用户确认?"}
    Confirm -->|修改| Proposal
    Confirm -->|取消| End1["不写任何状态"]
    Confirm -->|确认生成| Generate["生成 Skill 包<br/>+ workflow-protocol.json + 评估清单"]
    Generate --> Eval["comet eval collect → run --html"]
    Eval --> Ready["comet publish review → approve → run"]
...
完整字段含义见[Skill 偏好与真实来源](/zh/skill-factory/preferences)。

## 常见问题

<AccordionGroup>
  <Accordion title="我不懂 Bundle、Factory、composition 这些词">
    普通用户不需要懂。这些是 <code>/comet-any</code> 的内部后端概念。你只需要调用 <code>/comet-any</code>，它会在内部调用 CLI 维护状态。
  </Accordion>

  <Accordion title="/comet-any 和 /comet 有什么区别">
    <code>/comet</code> 是经典 Spec 模式入口，驱动 open/design/build/verify/archive 五阶段流程。<code>/comet-any</code> 是 Skill Factory，用来<strong>创建、优化、组合可复用的 Skill</strong>。两者是不同入口，但 <code>/comet-any</code> 的"基于 Comet 现有 Skill 的五阶段定制"可以以 <code>/comet</code> 为基础做增量调整（在受保护边界内增加 Required Skill Call、替换 producer 节点、关闭可选节点）。
  </Accordion>

  <Accordion title="做到一半中断了怎么办">
    直接对 Agent 说"继续上次的 Skill 创建"。<code>/comet-any</code> 会先尝试<strong>恢复现有创作状态</strong>，展示名称、状态、下一步和阻塞原因，让你选择继续哪一个。详见[创建 Skill 的完整流程 · 恢复中断流程](/zh/skill-factory/workflow#恢复中断流程)。
  </Accordion>
...
| 进阶主题 | 适合场景 |
| --- | --- |
| [Skill Factory 概览](/zh/skill-factory/overview) | 理解 Workflow Contract、三种起点、受保护边界和内置节点 |
| [创建 Skill 的完整流程](/zh/skill-factory/workflow) | 从恢复状态到生成、评估、评审、发布、分发的每一步详解 |
...
| [Skill 与 Engine](/zh/skill-factory/engine) | Skill 包结构、Engine 运行循环、runtime eval、不可变快照 |
