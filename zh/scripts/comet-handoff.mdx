---
title: 'comet handoff'
description: '生成阶段交接包和 handoff hash，把 OpenSpec 产物整理成 Superpowers 能读懂的上下文。'
---

`comet handoff` 负责把当前阶段的 OpenSpec 产物整理成交接包，并计算 handoff hash。它是 design 阶段衔接 OpenSpec（WHAT）和 Superpowers（HOW）的稳定公开命令，也服务于长任务、子代理执行和上下文恢复。
...
handoff hash 对以下 OpenSpec 产物计算内容 hash：
...
| `beta`（压缩） | `spec-context.json` + `spec-context.md`     | spec 文件保留完整原文，辅助文件只存 hash，节省约 25–30% token |
...
| verify               | 用 `--hash-only` 快速重算，一致说明 OpenSpec 产物未变           |
