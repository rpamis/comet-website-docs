...
`comet handoff ... --write` 在 `openspec/changes/<name>/.comet/handoff/` 下产出：
...
<Tip>
  普通用户不需要手工运行 handoff。`/comet-design` 会在 design 阶段自动生成，恢复时先用 `comet status` 和 `/comet-classic`。
</Tip>

<Note>
  安装包仍包含 <code>comet-handoff.mjs</code> 兼容 launcher，但普通使用和 Agent 指令应优先采用 <code>comet handoff</code>。
</Note>
