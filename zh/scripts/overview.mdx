...
当前支持的 Node.js 版本见[安装要求](/zh/guides/install-and-update)。

## 内部 launcher 的兼容背景

0.3.x 时代，Comet 的状态、守卫、交接和归档逻辑由 7 个 Bash 脚本承担。这套脚本在 macOS 和 Linux 上可以正常运行，但 Windows 用户必须额外安装 Git Bash 或 WSL 才能跑完整工作流，路径转义和 shell 差异也常常带来隐患。

0.4.0-beta.1 把这些脚本迁移到 Node `.mjs` 运行时；当前发布形态是薄 launcher 加共享的生成 runtime，带来了三点变化：

| 方面 | 0.3.x（Bash） | 0.4.0-beta.1（Node） |
| --- | --- | --- |
| 运行依赖 | Bash / Git Bash / WSL | 仅 Node.js 22+ |
...
| `comet-yaml-validate.mjs` | 校验 `.comet.yaml` 字段、枚举和路径，是阶段检查的预检查步骤 | — |
...
```mermaid
flowchart LR
    A["YAML 状态预检查"] --> B["comet guard<br/>检查阶段条件"]
    B -->|全部 PASS| C{"--apply?"}
...
- **`comet guard` 先做 YAML 状态预检查**；状态文件结构不合法时直接中止，不会继续检查阶段条件。
- **不带 `--apply`** 时，守卫只报告"是否就绪"，不改任何状态；带 `--apply` 时，校验全过后才会推进 `phase`。
- **`comet state transition` 和 `comet guard --apply` 共享同一份转移语义**，所以两条推进路径的行为一致。
- **`comet-hook-guard`** 作为 PreToolUse 钩子常驻，在 `open`/`design`/`archive` 阶段阻止源码写入，在"提前写实现"发生之前就拦住。
...
| 文件 | 归属 | 内容 |
| --- | --- | --- |
| `.comet.yaml` | 用户可读 | 工作流状态字段（`workflow`/`phase`/`build_mode` 等）+ `run_id` 链接 |
