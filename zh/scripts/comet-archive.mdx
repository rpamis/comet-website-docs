...
9. **校验主 spec 干净**：扫描 `openspec/specs/*/spec.md`，残留 delta-only 标题就 FATAL。
...
<Note>
  已归档的 Design Doc、Plan 和验证报告仍从项目根的 <code>docs/superpowers/</code> 解析，因此可以继续在 Dashboard 中查看。安装包保留 <code>comet-archive.mjs</code> 兼容 launcher，但普通使用应优先采用 <code>comet archive</code>。
</Note>
