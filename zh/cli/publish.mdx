...
`comet publish` 是普通用户面对 `/comet-any` 产物的发布入口。它负责 eval 证据就绪后的 review、approval、publish 和 distribute。创建、恢复和"下一步该做什么"由 [comet creator](/zh/cli/creator) 负责。

预设的工作方式是：

- `/comet-any` 创建、恢复和优化 Skill
- `comet eval` 验证生成物
- `comet creator status` / `comet creator next` 是普通用户查 readiness 和**下一步要做什么**的入口
- `comet publish` 处理评审、人工批准、发布和分发
...
<Note>
  <code>comet publish</code> 没有 reject 子命令。<code>approve</code> 是用户唯一批准入口，内部会以固定结论 approved 记录本次评审。需要拒绝时用高级的 <code>comet bundle review &lt;name&gt; --reject --reviewer &lt;name&gt;</code>。
</Note>
...
## 只想知道下一步：comet creator next

`comet creator status` 给你完整的 readiness 结论和阻塞项；如果你**只想知道现在该跑哪一条命令**，用 `comet creator next`——它打印**单个**推荐下一步，不暴露任何后端 Bundle 命令：
...
<Tip>
  <code>comet creator next</code> 是给"被打断的 <code>/comet-any</code> 流程"恢复用的：它把后端 Bundle 的 next action 翻译成一条用户命令，所以你不需要去读内部状态。<strong>偏好漂移</strong>（<code>preferenceDrift.changed</code>）为真时，<code>advisory</code> 模式会警告，<code>strict</code> 模式默认阻塞——你需要决定是继续旧方案还是重新生成。
</Tip>

## readiness 结论

`comet creator status` 会展示四种 readiness 结论之一：
...
readiness blockers 会阻止 publish。每个 blocker 有一个**阻塞码**前缀，`comet creator status` 会直接展示 `Readiness:`、`Blockers:`、`Warnings:` 和 `Evidence:`，并给出 `nextAction` 恢复命令：
...
| `[authoring]` | 生成物还有未完成的 authored 内容 |
| `[draft]` | 缺少 `currentHash` |
| `[eval]` | 缺少/失败/对应旧 hash 的 Eval 证据 |
| `[agent]` | Claude Code custom agent 没出现在平台预览里 |
| `[publish]` | status `ready` 但没有 ready metadata |

完整的阻塞码表和恢复建议见[发布和分发 Skill · 阻塞码](/zh/skill-creator/publishing#阻塞码-blocker-codes)。
...
- [comet creator](/zh/cli/creator) — 创建状态、恢复和唯一下一步
