...
日常创建使用 `/comet-any`；恢复和查看创建状态使用 [comet creator](/zh/cli/creator)；评审与分发使用 `comet publish`。只有审计底层状态或调试 compile、capability gap、eval evidence、executable disclosure 时，才直接运行 Bundle CLI。
