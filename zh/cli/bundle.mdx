...
普通用户通常**不需要**直接使用它。`/comet-any` 的创建、恢复和 authoring flow 由 [comet creator](/zh/cli/creator) 暴露；`comet publish` 暴露 review、approve、publish 和 distribute。直接使用 Bundle CLI 的场景是**审计后端状态、调试平台 compile、capability gap、eval evidence 或 executable disclosure**。
