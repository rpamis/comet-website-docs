---
title: "comet skill"
description: "管理本地 Comet Skill 包，并调试高级 deterministic Engine Run。"
---

`comet skill` 是低层 Skill 包和 Engine Run 工具。它发现显式 Skill 目录、`.comet/skills/` 下的项目覆盖和内置 Skill，并提供本地 Skill 包管理和高级 deterministic Engine Run 调试。

普通用户创建可复用 Skill 时优先用 [`/comet-any`](/zh/skill-creator/overview)。`comet skill run` / `comet skill continue` 更适合调试高级 Engine Run。
...
| `comet skill run <skill>` | 启动高级 deterministic Engine Run |
...
<Warning>
  <code>comet skill check</code> 只检查某次 Engine Run 的完成度，不是通用 Skill 评估。评估一个 Skill 产品能力，请用 <a href="/zh/cli/eval">comet eval</a>。详见 <a href="/zh/eval/runtime">Runtime check</a>。
</Warning>
...
- [Skill 与 Engine（进阶）](/zh/skill-creator/engine) — 理解 Skill 包和 Engine Run 的运行语义、pending action、不可变快照
