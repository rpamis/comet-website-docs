...
<Note>
  如果你只想把本地 Skill 放进项目，先用 <code>comet skill add</code> 和 <code>comet skill show</code>。运行、恢复和 runtime check 是高级调试能力，主要面向 Engine-enabled Skill。
</Note>
...
两者解决的问题不同。日常使用 `/comet` 时大多不需要管理项目 Skill。只有安装自定义 Engine Skill、覆盖内置 Skill 或调试 Engine Run 时，才需要本页命令。
...
| `--confirm <ref>` | 确认受 guardrail 保护的动作，可多次传入；`run` 输出 `User confirmation required for: <ref>` 时带上对应 ref 重新运行 | `run`、`continue` |
...
6. 引擎决策：解析 entry step、构造第一个动作、过 guardrails、写入 pending action、`status = waiting`。
...
Run state 由 Engine 自动维护（machine-owned），**不要手工编辑**。只有 `run_id` 会镜像到 `.comet.yaml`。
...
| `.comet/artifacts.json` | 产出的产物映射 |

所有文件 IO 都限制在 change/run 目录内，拒绝绝对路径、`~`、盘符和 `..` 路径穿越。写入是原子的（写 tmp 再 rename）。

## 文本模式的恢复提示

`comet skill` 在文本模式下会打印 `Pending action` 和 `Next:`，说明 Run 暂停或 eval 失败后的下一步。
