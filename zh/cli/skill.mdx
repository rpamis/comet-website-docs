...
解析 Skill 并返回名称、版本、来源、根目录和内容 hash。
...
额外返回 goal、orchestration、steps、skills、agents、tools、guardrails 和 evals。
...
`--scope` 可选 `progress`、`step` 或 `completion`（默认 `progress`）。读 `comet/checks.yaml` 的 runtime eval。

<Warning>
  <code>comet skill eval</code> 只检查某次 Engine Run 的完成度，不是通用 Skill 评估。评估一个 Skill 产品能力，请用 <a href="/zh/cli/eval">comet eval</a>。详见 <a href="/zh/eval/runtime">Runtime eval</a>。
</Warning>
...
- [Skill 与 Engine（进阶）](/zh/skill-factory/engine) — 理解 Skill 包和 Engine Run 的运行语义、pending action、不可变快照
