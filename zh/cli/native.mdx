...
`comet native` 是 Native 工作流的确定性 Runtime 入口。它的状态与工作流产物只归属共享项目配置 `.comet/config.yaml` 和配置指定的 `<artifact-root>/comet/`，不会读取或转换 Classic/OpenSpec change。为生成 baseline 与 implementation scope，Runtime 会有界读取项目所有范围内的文件内容；项目存在 Git metadata 时，仅在构建 Git-aware snapshot 时调用 Git。除显式的 `receipt automated` 外，Runtime 不调用 shell、项目命令或外部 Skill；内置 `comet native check` 仍不调用 Git 或任何外部进程。
...
不带 change 的 `status` 返回 active change 的只读分页投影：
...
Manual receipt 记录可复核的操作步骤与观察结果，身份固定为 Runtime 来源，不接受 `--responsible` 或 `--confirmed`。Automated receipt 从项目根目录运行明确给出的命令，继承当前项目环境，支持 Windows 的 `.cmd`、`.bat` 和 `.ps1` shim，并在保存有界输出摘要前清理疑似凭据。命令的退出状态、超时、工作区前后身份和输出 hash 都会写入 receipt。

把每条 Acceptance ID 对应的 receipt 引用写入 `verification.md`，再用 `evidence format` 生成 Runtime 可解析的规范区块。失败验收可以引用非通过的 automated 或 manual receipt；Runtime 会据此推导失败检查与修复缺口，无需调用方另行分类。
...
检查发现问题或 scope 已 stale 时返回退出码 `1`，但 receipt 仍会保存。提交 `next --result pass --report ...` 时，Runtime 会自动运行并绑定这项必需检查；无需调用方传入 receipt。模型仍应按任务风险自行运行真实项目测试；内置 check 不是通用测试执行器。
...
Shape 只有在用户刚确认当前共享理解后才传 `--confirmed`；Sequential 与 Batch 的正常 Skill 流程都会先保存这次最终确认，任何确认都不能绕过其他 `[blocking]` 问题。Sequential 缺少显式确认时 Runtime 会拒绝进入 Build；Batch 的既有 Runtime 兼容路径保持不变。进入 Build 时，Runtime 把 approval 绑定到当时的 `approved_contract_hash`。之后若需求变化引入新的用户决定，仍保持在 Build：先记录 `[blocking]`、暂停实现并重新澄清；用户确认后更新 Decisions、brief 和完整 target spec，再移除阻塞项，并在 Runtime 返回的 transition 中传入 `--confirmed`。不能复用旧确认或手改 hash。
...
- 相同语义缺口第二次出现时告警；第三次出现且没有进展、但仍有 override 时，continuation 保持 Agent-owned，并要求一个具体的新修复假设；
- 使用 `status` 返回的精确签名和非空假设摘要最多 override 一次；不能把这个 Agent-owned 步骤转交给用户；
- 同一 contract 的 Verify fail 默认上限为 5，可通过 `native.max_verify_failures` 配置正整数；达到上限后停止。

只有用户确认后的 contract 变化会开始新的目标周期。Override 已使用或验证预算耗尽时，Runtime 返回一次明确决策，由用户选择扩大预算、修改已确认 contract 或停止；不应反复调用 `next`、弱化检查或提前归档。
