...
Shape 只有在用户刚确认当前共享理解后才传 `--confirmed`；Sequential 与 Batch 的正常 Skill 流程都会先保存这次最终确认，任何确认都不能绕过其他 `[blocking]` 问题。Sequential 缺少显式确认时 Runtime 会拒绝进入 Build；Batch 的既有 Runtime 兼容路径保持不变。进入 Build 时，Runtime 把 approval 绑定到当时的 `approved_contract_hash`。之后若 brief 或完整目标规格变化，Build 会保持阻塞并返回 `re-confirm-contract`；只有用户重新确认当前 contract 后才能再次传 `--confirmed`，不能复用旧确认或手改 hash。
...
- 相同语义缺口第二次出现时告警，第三次出现且没有进展时 continuation 变为 manual stop；
