...
提交 Verify `pass` 时，Runtime 会自动运行这项内置 required check 并把 receipt 绑定到当前验证。`comet native check` 仍可用于提前查看同一检查结果。
...
- 相同语义缺口第二次出现时告警，第三次出现且没有进展时进入 `repair-stagnation-stop` 恢复分支；
- 仍有 override 时，Agent 先提出明确的新修复假设，再使用 `status` 返回的签名和非空摘要继续一次；
- 同一 contract 的 Verify fail 默认上限为 5，可通过 `native.max_verify_failures` 配置正整数；override 或验证预算耗尽后，Runtime 才要求用户决定。

只有用户确认后的 contract 变化会开始新的目标周期。进入恢复分支时先按 finding 形成新假设，不应反复调用 `next`、弱化检查或提前归档；只有 continuation 明确要求用户决定时才等待用户。
