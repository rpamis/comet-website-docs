...
交互式项目初始化会说明并提供 Native、Classic 或同时启用两者。非交互初始化按以下规则选择默认工作流：
...
Classic 或 global 初始化在选择平台后会询问**安装模式**（0.4.0-beta.1 起新增符号链接模式）。项目级 Native 固定使用 Copy，避免创建 `.comet/skills/`；Native change 状态只存在于 `<artifact-root>/comet/`，共享项目配置与 selection 位于 `.comet/`。
...
- **刷新**注释，对齐当前版本
- **保留**你额外添加的自定义字段
- Native 的 `default_workflow`、`workflows`、`ambient_resume` 和 `native.*` 与 Classic 设置保持独立语义
