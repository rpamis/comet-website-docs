...
`comet init` 是 Comet 的安装与接入入口。
它会识别当前项目环境，安装所需 Skill 与运行资产，并写入 `/comet` 的默认工作流配置。
...
交互式初始化在 project 和 global 范围都支持选择 Native、Classic 或同时启用两者。项目级非交互初始化会按下面顺序决定默认工作流：

1. 先读取合法的 `.comet/config.yaml`，这是状态唯一可信来源。
2. 如果传入 `--workflow native|classic|both`，按显式参数执行；`both` 下 `/comet` 默认进入 Native。
3. 没有有效 workflow 配置，但检测到 Classic `.comet.yaml` 或旧版 Ambient Resume 证据时，维持 Classic。
4. 其他情况默认 Native；已有代码、普通 `openspec/` 或 `docs/superpowers/` 不会改变默认值。
