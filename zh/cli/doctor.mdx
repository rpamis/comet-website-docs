...
CodeGraph 现在分成四个维度报告：CLI 是否安装、当前项目索引状态、检测到的 Agent 是否注册 MCP，以及每个检测到的 Agent 是否具备有效能力。项目索引最新本身不代表 Agent 已经注册有效的 CodeGraph MCP；需要详细状态时可以查看 JSON 输出中的 `codegraph` 对象。
...
`--repair` 会执行写操作。Classic 根目录迁移中断时，根据现场选择 `continue` 或 `rollback`；涉及 CodeGraph 等需要授权的项目修复时，再加 `--yes`。如果只想重新检查，请省略 `--repair`。
