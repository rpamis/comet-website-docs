...
`comet doctor` 用来定位“为什么 Comet 现在跑不动”。

它会检查安装环境、CLI 与依赖、项目目录、Skill/Rule/Hook、Runtime、CodeGraph，以及当前 change 状态。可以把它理解成环境与现场体检：`comet status` 负责告诉你流程位置，`comet doctor` 负责告诉你故障位置。

默认检查是只读的，不会因为诊断而修复或改写项目。只有显式使用 `--repair` 时，才会执行支持的修复动作。

CodeGraph 部分会分成四项独立结果：CLI 安装状态、项目索引状态、MCP 注册状态、以及各 Agent 的 effective 能力。索引最新只说明数据已经同步，不等于 Agent 一定能用 CodeGraph；需要精确定位时请查看 JSON 里的 `codegraph` 对象。
...
- `✓`：该项检查通过，不需要处理。
- `⚠`：发现警告或可选组件缺失。多数情况下可以继续，但建议按输出提示尽快补齐。
