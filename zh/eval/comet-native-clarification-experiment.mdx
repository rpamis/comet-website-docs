...
这次实验比较的是 Comet Native 的两种澄清模式：`batch` 一次提出所有独立问题，再单独确认双方理解一致；`sequential` 每次只解决一个决策。
...
Sequential 的失败情况不同：一次多问了一个决策轮；另两次虽然完成了预期的 3 个决策点，但业务行为或归档产物不完整。因此少轮次不能单独作为优势，必须与业务结果、澄清协议和终态证据一起判断。
