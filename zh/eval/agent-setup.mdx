...
description: '在运行 comet eval 前，分别准备 Claude Code、Codex 或 Qoder 的 CLI、认证信息和模型配置。'
---

`comet eval` 会在隔离的 Docker 容器中启动你选择的 Agent。不同 Agent 的登录方式和模型协议并不通用：Claude Code 可以使用 Anthropic 兼容接口，Codex 需要 OpenAI Responses API，Qoder 需要 Qoder 自己的登录态或 Personal Access Token。
...
<Info>
  Beta.18 暂不把 CodeBuddy 的启动方式写成保证可用的教程。它的 CLI 配置和当前 Eval
  适配器仍在单独验证，避免让用户把未经确认的 Claude Code 配置误用于 CodeBuddy。
</Info>
