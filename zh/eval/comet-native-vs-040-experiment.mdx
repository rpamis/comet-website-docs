...
0.4.0 的 48 个样本都因日志中出现 outer runner、API 或环境失败字样而被保守标记为 `flagged`，但仍全部进入 analysis set。`flagged` 只是可观测性标记，并不代表样本被排除或判定失败。本文保留其 strict pass 结果，并将这一可观测性差异纳入比较限制。
