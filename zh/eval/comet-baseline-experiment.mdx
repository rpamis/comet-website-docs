...
本文整理一次真实的 Comet workflow 基线评估实验，用超130亿+的Credits调用量(等价15亿+Token)评估了Comet不同版本基线的性能，主Agent模型为`mimo-2.5 pro`，裁判模型为`glm-5.2`，产出Pass@5总计实验次数`240`次。目标不是介绍功能，而是展示一份对比报告应如何阅读：先确认数据范围，再看任务结果、可靠性指标、rubric 维度、成本和失败归因。
