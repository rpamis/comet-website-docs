...
本文整理一次真实的 Comet workflow 基线评估实验。目标不是介绍功能，而是展示一份对比报告应如何阅读：先确认数据范围，再看任务结果、可靠性指标、rubric 维度、成本和失败归因。
...
报告给出的 verdict 是：`COMET_FULL_040_BETA` 的综合 workflow 分数为 `0.89`，高于 `COMET_FULL_039` 的 `0.82`，且没有维度退步超过 `0.05` 的容忍线。
