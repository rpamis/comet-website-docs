...
`comet eval` 是 Comet 的通用 Skill 评估入口。它先回答一个问题：**你手里的 Skill 能不能作为可靠的产品能力，通过真实任务评估？**
...
日常使用只需关注评估任务和结果。`comet eval` 已封装 pytest、任务发现、profile、实验条件（treatment，指本次注入哪些 Skill 做对照）、Docker 和本地评测框架的启动细节，你可以直接在项目根目录运行评估。
...
`collect` 是成本最低的排错入口。它只做发现和预检查，不执行模型或 Docker 任务，适合快速发现路径、manifest、任务缓存和配置问题。
...
- **rubric 多维评分**：把 Skill 质量拆成多个维度（如五阶段的 main_flow/gate_guard、通用 Skill 的 safety_boundary），每维度 0.0–1.0，加权汇总成 `weighted_score`。仅用于诊断，不作为门禁。
- **pass@k / pass^k**：区分**能力上限**（k 次里至少成功一次）和**可靠性下限**（k 次全部成功）。基于多次重复运行计算（重复运行目前由 eval harness 内部的 pytest 选项驱动），同样仅用于诊断。
- **任务校验器通过/失败**：这次实现到底对不对（`target_artifacts` + `test_scripts`）。**这才是真正决定 pass/fail 的硬性结果**。
