---
title: "评估系统概览（进阶）"
description: "深入了解 comet eval 在 Skill 创建、验证和发布 readiness 中的位置，profile/task 体系，eval.yaml 格式，以及它和 runtime check 的边界。"
---
...
| **运行期检查** | `comet skill check` | 某次 deterministic Engine Run | 否 |

`comet eval` 回答"这个 Skill 作为产品能力能不能通过评估"，通过共享 eval harness 执行真实模型任务。`comet skill check` 回答"这次 Run 是否完整"，只检查 runtime checks，不跑模型。详见 [Runtime check](/zh/eval/runtime)。

<p align="center">
  <img src="/assets/eval-overview-illustrations/01-two-eval-systems-new.png" alt="小鱼把 comet eval 的发布证据桌面和 comet skill check 的 Run 完成度桌面分开，并举牌提醒不要混用" width="800" />
</p>

<p align="center">`comet eval` 产出发布前证据，`comet skill check` 只检查某次 Run 是否完整，两者不要混用</p>
...
/comet-any 生成 Skill
  ...
  -> /comet-any 或 comet creator 读取评估结果并进入 readiness / review / publish / distribute
```

`comet eval` 不负责发布。发布仍然由 `/comet-any` 背后的 Bundle 后端处理：创建和恢复状态对普通用户暴露为 [comet creator](/zh/cli/creator)，发布和分发暴露为 [comet publish](/zh/cli/publish)。eval 的职责是提供发布前证据。

## 两种入口

| 场景 | 命令 | 适合阶段 |
| --- | --- | --- |
| `/comet-any` 生成物 | `comet eval ./skill/comet/eval.yaml --html` | 发布前完整评估 |
...
| 是 `/comet-any` 生成物 | 直接传 `comet/eval.yaml` |
...
4. `comet eval`（创建期）和 `comet skill check`（运行期）是两套系统，不要混用。
