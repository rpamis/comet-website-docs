...
<Info>
  自定义 Agent 适配器属于源码/维护者进阶能力。普通用户不需要注册适配器；直接使用上表中的
  内置 Agent 即可。只有需要扩展 Eval harness 时，才需要参考[进阶配置](/zh/eval/configuration)。
</Info>

如果你正在维护源码，也可以注册自定义 Agent 适配器。仅仅把可执行文件放到 PATH 上不会自动启用它，适配器需要放在：
