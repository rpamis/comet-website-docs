...
<Note>
  `/comet-any` 生成的 <code>comet/eval.yaml</code> 可以保留 <code>&lt;current-bundle-hash&gt;</code>。<code>comet eval</code> 会在运行前计算当前 Bundle draft hash，创建临时 manifest，再执行 collect/run；源 Bundle 不会被修改。
</Note>
...
如果占位 hash 无法解析，错误会说明 manifest 是否已离开原 Bundle、是否缺少 `bundle.yaml`，或 draft 是否无法加载。先修复 Bundle 结构，不要手工把占位值替换成猜测的 hash。
