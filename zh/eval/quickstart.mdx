...
- 已经通过 [`/comet-any`](/zh/skill-factory/getting-started) 生成了一个 Skill（产物里有 `comet/eval.yaml`），或
- 有一个本地 Skill 目录（用于早期冒烟）。

如果你还没有 Skill，先看[组合任意 Skill 快速上手](/zh/skill-factory/getting-started)。

## 运行前你需要准备什么

`comet eval` 的核心 Comet 运行时**不需要**这些，但 eval 因为要跑真实模型任务，依赖以下环境。如果没准备好，eval 会**静默跳过**而不是报错，所以务必确认。
...
<Info>
  <code>run</code> 会真正调用模型，会消耗 token 和时间。一次评估的耗时取决于任务数和 <code>maxTurns</code>（默认 <code>authoring-skill</code> profile 是 8 轮）。先用 <code>collect</code> 排错能避免浪费。
</Info>

<p align="center">
  <img src="/assets/eval-quickstart-illustrations/01-collect-run.png" alt="小鱼先用 collect 做低成本预检查，再用 run 执行真实评估并生成报告" width="800" />
</p>

<p align="center">先 collect 排路径和任务发现问题，再 run 执行真实评估并生成可浏览报告</p>
...
理解这一段能帮你判断评估结果是否可信。`comet eval run` 内部做了这些事：

```mermaid
flowchart TD
    A["comet eval run<br/>--manifest ... --html"] --> B["在 eval/ 下用 uv run pytest 启动 harness"]
...
<AccordionGroup>
  ...
  <Accordion title="我应该用 comet eval 还是 comet skill eval">
    评估一个 Skill 的产品能力能不能通过，用 `comet eval`。检查某次 Engine Run 是否缺 artifact 或状态，才用 `comet skill eval`。发布前证据只需要 `comet eval`。详见[Runtime eval](/zh/eval/runtime)。
  </Accordion>
