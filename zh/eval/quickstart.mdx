...
只要你有一个包含 `SKILL.md` 的本地目录，就可以开始评估。
即使还没写评估用例，**Eval 也会基于 Skill 内容自动生成 2–4 个用例并执行**。
本文按实际操作顺序展开：先跑通一次，再选择 Agent 和后端，最后看任务配置、报告解读与失败定位。
...
`--collect` 只做静态发现和配置检查，不会启动 Agent、Docker、插件，也不会消耗凭据或网络调用。
所以你可以先用它验证 Skill 入口与任务发现，再准备完整模型环境。

如果要选择 Claude Code 以外的 Agent，先阅读 [Eval Agent 启动配置](/zh/eval/agent-setup)，确认对应的 CLI、认证方式和模型协议。

### 用户级 `.env` 配置

普通用户不需要 clone Comet 源码，也不需要进入源码目录下的 `eval/`。
第一次运行任意 `comet eval` 命令时，CLI 会自动创建完整的用户级配置文件：

- Windows：`%USERPROFILE%\.comet\eval\\.env`
- macOS/Linux：`~/.comet/eval/.env`

CLI 只会创建缺失文件，不会覆盖已有文件。
首次运行会打印实际路径。
你按需填写参数后再次运行即可。
如果当前 shell 已设置同名变量，则优先使用 shell 值。
...
如果评估几乎瞬间结束且没有真实执行，多数是 Docker、Agent CLI 或模型凭证未就绪。
harness 会把这类样本标为跳过，避免产生“假成功”。
...
如果没有 `comet/eval.yaml`，或 manifest 中缺少 `evaluation.tasks` 与 `recommendedTasks`，普通运行会根据 Skill 内容**自动生成 2–4 个评估用例**，冻结并缓存后执行。
你不需要先手写任务就能开始。
`--quick` 不会使用自动生成用例，而是固定跑 `generic-skill-smoke`，重点验证 Skill 能被注入、调用并产出 `result.md`。
...
主 Agent 与 LLM-as-Judge 可以使用不同的 Agent、模型、API 地址和凭据。
Judge 不会自动继承主 Agent 的模型或凭据。
启用 Judge 时，必须单独提供 Judge 配置。
...
失败归因可这样理解：`harness` 多数表示环境或依赖问题，`workflow` 表示 Skill 流程未达预期，`task` 表示任务定义或校验条件有问题，`model` 表示模型行为或调用不稳定。
