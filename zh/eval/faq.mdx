...
<AccordionGroup>
  ...
  <Accordion title="comet eval 和 comet skill check 有什么区别">
    `comet eval` 评估 Skill 的产品能力，产出发布前证据（读 `comet/eval.yaml`）。`comet skill check` 检查某次 Skill 运行是否缺 artifact 或状态，不执行模型任务，不产出发布证据（读 `comet/checks.yaml`）。发布 readiness 只需要 `comet eval`。详见[Runtime check](/zh/eval/runtime)。
  </Accordion>
