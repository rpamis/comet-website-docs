...
<AccordionGroup>
  ...
  <Accordion title="comet eval 和 comet skill eval 有什么区别">
    `comet eval` 评估 Skill 的产品能力，产出发布前证据（读 `comet/eval.yaml`）。`comet skill eval` 检查某次 Engine Run 是否缺 artifact 或状态，不执行模型任务，不产出发布证据（读 `comet/checks.yaml`）。发布 readiness 只需要 `comet eval`。详见[Runtime eval](/zh/eval/runtime)。
  </Accordion>
  ...
  <Accordion title="两套评估系统是什么意思">
    创建期评估（`comet eval`，读 `comet/eval.yaml`）和运行期评估（`comet skill eval`，读 `comet/checks.yaml`）。前者是发布证据，后者是 Run 完成度检查。详见[评估系统概览 · 两套评估系统](/zh/eval/overview#两套评估系统不要混淆)。
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="authoring-skill 和 generic profile 有什么区别">
    `authoring-skill`（8 维 rubric，maxTurns=8）用于 `/comet-any` 生成的 Skill，检查生成包完整性、resolved-skills 证据、Engine 契约等。`generic`（7 维 rubric，maxTurns=12）用于通用 Skill 冒烟。详见[评估系统概览 · Profile 体系](/zh/eval/overview#profile-体系)。
  </Accordion>
</AccordionGroup>
