...
<Accordion title="我需要懂 pytest 或 Docker 吗">
  ...
  <Accordion title="两套评估系统是什么意思">
    创建期评估（`comet eval`，读本地 Skill 目录或 `comet/eval.yaml`）和运行期检查（`comet skill check`，读 `comet/checks.yaml`）。前者是发布证据，后者是 Run 完成度检查。详见[评估系统概览 · comet eval 与 runtime check](/zh/eval/overview#comet-eval-与-runtime-check)。
  </Accordion>
</AccordionGroup>
