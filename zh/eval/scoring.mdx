...
<Warning>
  本页是源码/维护者进阶内容。普通用户不需要 clone Comet 来运行评估。只有要复核评分器、
  任务证据或自动交互实现时，才需要拉取源码。日常运行请看[快速上手](/zh/eval/quickstart)。
</Warning>

Comet eval 先用任务检查结果判断一次运行是否通过。
rubric、pass@k、pass^k 和加权分数是诊断层信息，用来定位问题和比较版本。
对于多阶段工作流，任务交互可由“被测 Agent + 用户模拟 Agent”自动完成。
...
<Note>
  发布判断不能只看 <code>weighted_score</code>。
  一次运行是否通过由 <code>checks_failed</code> 决定。
  rubric、pass@k 和 pass^k 用于诊断与对比。
</Note>
...
| 指标 | 回答的问题 | 类型 | 是否门禁 |
| --- | --- | --- | --- |
| **rubric 各维度分**（0.0–1.0） | 这个维度做得怎么样 | 诊断参考 | 否（不作为门禁） |
| **weighted_score**（加权总分） | 综合质量如何 | 诊断参考 | 否 |
| **RubricAvg**（报告列） | 该次运行的维度均分 | 诊断参考 | 否 |
| **pass@k** | k 次里至少成功一次的概率（能力上限） | 跨多次运行统计 | 否 |
| **pass^k** | k 次全部成功的概率（可靠性下限） | 跨多次运行统计 | 否 |
| **工作流契约检查** | 被测 Skill 是否真的按契约触发 | 硬性判定 | 是（缺必需 Skill 会失败） |
| **任务校验器通过/失败** | 这次实现到底对不对 | 硬性判定 | 是（决定 pass/fail） |

<Note>
  rubric 和 pass 指标用于诊断和比较。通过条件是 <code>checks_failed == []</code>。大多数失败来自任务校验器，但 profile 也会把必需 Skill 未触发等工作流契约问题写入 <code>checks_failed</code>。
</Note>

## 证据从哪里来

Comet eval 从 Claude CLI 的 stream-json、任务产物、校验器结果和 Runtime 状态中收集运行证据。
...
eval harness 会先保存本次运行的原始 `stdout` / `stderr`，再逐行解析 `stdout` 的 JSONL 事件。
解析器会提取：
...
因此，报告里的 `Skills invoked`、`commands_run`、`tool_calls`、`files_created`、token/cost 等，都是从 Claude CLI 结构化事件流整理出的可观测行为日志。

<Warning>
  stream-json 仍然只是<strong>可观测行为日志</strong>。
  它适合统计工具调用、命令、Skill 调用、文件变化和成本。
  但它不包含模型内部推理，也不能直接证明内部决策的因果链。
  schema 变化、损坏的 JSON 行、CLI 版本变化都可能影响解析。
  因此 raw stdout/stderr 必须保留用于审计。
</Warning>
...
| `comet-workflow` | 10 | 12.0 | `auto_user`（多轮） | 经典 `/comet` 五阶段 |
| `authoring-skill` | 11 | 12.8 | `auto_user`（多轮） | `/comet-any` 生成物 |

### comet-workflow rubric（10 维）

这一套用于评估经典五阶段工作流。
十个 `[RUBRIC]` 维度分只作诊断参考。
但 profile 仍会做工作流契约检查。
如果没有真实触发 `comet`、嵌套 Comet 阶段 Skill、OpenSpec 依赖 Skill 或 Superpowers 依赖 Skill，会触发硬失败。

| 维度 | 权重 | 检查什么 |
| --- | --- | --- |
| `business_completion` | **2.0**（最高） | 基线校验器通过比例（业务任务到底做对没） |
| `workflow_completion` | 1.0 | 工作流自身步骤的完成度 |
...
评估通用 Skill。和 comet-workflow 不同，**它对"必需 Skill 未被调用"会产生硬失败**（当 `require_skill_invocation: true` 时），其余维度仅提供诊断信息。
...
设置 `BENCH_LLM_JUDGE=1` 可启用 LLM-as-judge。
默认情况下，Comet 的最终 `weighted_score` 来自规则分，而不是 LLM judge。
规则型 rubric 能抓结构信号（文件存在、命令执行），但难以判断**产物是否有实质内容**。
LLM judge 会让裁判模型读取 workspace artifacts 后重新打分，并输出 `[RUBRIC-JUDGE]`（与规则分 `[RUBRIC]` 区分）。
该机制是 best-effort：若 judge 失败，会回退到规则分，不影响整次运行。

judge 在宿主机（不在 Docker 内）通过复用 `claude` CLI 运行，不引入新依赖。但裁判配置和被测 Agent 配置是**有意隔离**的：启用 `BENCH_LLM_JUDGE=1` 时，必须显式设置 `BENCH_JUDGE_MODEL`，不能回退复用主模型的 `ANTHROPIC_MODEL`。
...
| `output_quality` | 结构良好、完整、非平凡、有思考痕迹 | 存在但内容单薄、勉强达标 | 缺失、空、明显 stub |
...
这两个指标衡量**能力 vs 可靠性**，是评估 Skill 能否反复稳定运行的关键。它们基于**多次重复运行**产生的通过/失败序列计算。
...
- **pass@k 高、pass^k 低**：能力够，但**不稳定**。它代表“能做，但不能保证每次都做对”，对高频使用的 Skill 是危险信号。
- **两者都高**：既能做，又能**持续做对**，才算可信赖。

报告会显示 `pass@k − pass^k` 的差值，供比较重复运行结果。

### 怎么得到多次运行

重复运行由 eval harness 内部的 pytest 选项 `--count N` 驱动：它把每个 `(task, treatment)` 组合重复 N 次，产生 N 个独立的通过/失败结果（`comet eval` 单次运行只产出 pass@1/pass^1）。对比报告先过滤出 analysis set：明确的环境或运行器本身的问题会被排除，flagged run 仍进入主统计但会被标出。analysis set 里的这 N 个布尔值就是 pass@k/pass^k 的输入。

```bash
# 重复 5 次，让 pass@k/pass^k 有意义
# （重复运行在 eval harness 内部由 pytest --count 5 驱动）
```

<Warning>
  报告在运行数不足时会警告重复次数不够（pass@k/pass^k 的 k&gt;1 至少需要 2 次运行才有意义）。单次运行只能算 pass@1/pass^1，k&gt;1 的指标需要多次重复。
</Warning>
...
pass@k/pass^k 不在 `summary.md`，而在**对比报告**（`comparison_report.md`）的 `## pass@k / pass^k — capability vs reliability` 章节。它们是**信息性**的，不是门禁。
...
循环最多 `max_turns` 次外层往返（comet-workflow 常见为 12 次，authoring-skill 常见为 8 次），命中"完成"（`archive complete` / `workflow complete` / `all 5 phases` 等）则提前结束。

每一轮被测 Agent 都用 `--output-format stream-json --verbose` 运行。loop 驱动只把每轮被测 Agent 的 stream-json stdout 拼接起来给 harness 解析。用户模拟 Agent 的一次性回复不会进入主事件流。因此，事件统计只反映 subject Agent 的可观测行为。

<Note>
  <code>max_turns</code> 不是被测 Agent 内部真实工作轮数，也不是工具调用次数。一次外层往返指：被测 Agent 跑到决策点，用户模拟 Agent 回复，然后被测 Agent 用 <code>--resume</code> 继续。同一次往返内部仍可能包含多条 assistant 消息、工具调用和文件操作。
</Note>
...
模拟 Agent 收到的是 simulator prompt + 被测 Agent 的最后一条消息，指令要求它：

- 收到确认请求时，**批准**提出的方案/名字/计划
- 需要做选择时，**选最合理的默认**
...
auto_user 评测的模拟器指令可被覆盖。默认从 `eval/simulator-instruction.md` 读取。这个文件预置了上面几条原则。两种覆盖方式（优先级从高到低）：

| 方式 | 怎么设 | 作用范围 |
| --- | --- | --- |
| `--simulator-prompt "..."` | pytest 命令行参数 | 单次运行 |
| `BENCH_SIMULATOR_PROMPT_FILE=<path>` | 环境变量（可写进用户级 `.env`） | 所有 auto_user 运行 |

`BENCH_SIMULATOR_PROMPT_FILE` 的相对路径从 `eval/` 解析，默认值为 `simulator-instruction.md`，文件存在时才会读取。通过替换该文件，可以测试不同用户行为配置，而不需要修改 harness 代码。

### 设计目的

工作流类 Skill 会在决策点暂停等用户确认。如果评测只跑单轮，Skill 会卡在第一个决策点。双 Agent 循环让评测**自动跑完整条工作流**（open→...→archive），并由用户模拟 Agent 生成合理、能推动流程的决策点输入。这样测出来的 rubric 分和 pass/fail 才反映真实使用场景。
...
| **reps** | 重复次数（harness 内部 `--count N`） | `--count 5` 每个 (task,treatment) 跑 5 次 |
