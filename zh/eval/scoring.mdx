...
| `authoring-skill` | 8 | 9.7 | `auto_user`（多轮） | `/comet-any` 生成物 |
...
### authoring-skill rubric（8 维）

评估 `/comet-any` 生成的 Skill 包。它先继承 generic 的三个共享维度，再加四个包专属检查。**缺 SKILL.md、缺 resolved-skills.json、缺 workflow-protocol.json、缺 Engine 文件都会产生硬失败**。

| 维度 | 权重 | 检查什么 |
| --- | --- | --- |
| `completion` | 1.5 | 继承自 generic |
| `generated_package` | 1.5 | 包存在 + SKILL.md 含"调用链/call chain"和"停止点/stop" |
| `resolved_skill_evidence` | 1.5 | `reference/resolved-skills.json` 存在且记录了 workflow 节点绑定 |
| `safety_boundary` | 1.2 | 继承自 generic |
| `skill_invocation` | 1.0 | 继承自 generic |
| `artifact_presence` | 1.0 | 继承自 generic |
| `engine_contract` | 1.0 | `comet/` 下有 `skill.yaml`/`guardrails.yaml`/`checks.yaml`（轻量包无 Engine 则中性 1.0） |
| `review_readiness` | 1.0 | generic 层无硬失败则为 1.0 |
...
设 `BENCH_LLM_JUDGE=1` 启用。它用 LLM 重新打分**三个规则较弱的维度**（`artifact_quality`、`spec_drift`、`main_flow`），输出 `[RUBRIC-JUDGE]` 行（和规则分的 `[RUBRIC]` 区分）。best-effort：失败则回退到规则分，不影响运行。
