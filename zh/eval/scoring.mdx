...
评估 `/comet-any` 生成的 Skill 包。它先继承 generic 的三个共享维度，再加四个包专属检查。**缺 SKILL.md、缺 resolved-skills.json、sourceSummaries 为空、缺 Engine 文件都会产生硬失败**。

| 维度 | 权重 | 检查什么 |
| --- | --- | --- |
| `completion` | 1.5 | 继承自 generic |
| `generated_package` | 1.5 | 包存在 + SKILL.md 含"调用链/call chain"和"停止点/stop" |
| `resolved_skill_evidence` | 1.5 | `reference/resolved-skills.json` 存在且 `sourceSummaries` 非空 |
| `safety_boundary` | 1.2 | 继承自 generic |
| `skill_invocation` | 1.0 | 继承自 generic |
| `artifact_presence` | 1.0 | 继承自 generic |
| `engine_contract` | 1.0 | `comet/` 下有 `skill.yaml`/`guardrails.yaml`/`evals.yaml`（轻量包无 Engine 则中性 1.0） |
