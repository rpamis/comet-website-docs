...
裁判子进程会先清除继承来的主 `ANTHROPIC_*` provider 设置，再把独立裁判配置映射给自己的 Claude CLI 调用：

| 环境变量 | 用途 |
| --- | --- |
| `BENCH_JUDGE_MODEL` | 必填。裁判模型，同时传给 `claude --model` 和子进程 `ANTHROPIC_MODEL` |
...
这样设计是为了避免"同一个模型既当选手又当裁判"：主 Agent 可以继续使用自己的 `ANTHROPIC_MODEL`、`ANTHROPIC_BASE_URL` 和 token，judge 必须单独声明模型和 provider。若开启了 `BENCH_LLM_JUDGE=1` 但缺少 `BENCH_JUDGE_MODEL`，报告会写入：
