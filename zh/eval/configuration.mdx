...
<Note>
  如果同时设置 <code>BENCH_JUDGE_AUTH_TOKEN</code> 和 <code>BENCH_JUDGE_API_KEY</code>，auth token 优先。judge 子进程会清除继承来的主 <code>ANTHROPIC_*</code> provider 变量，再映射 <code>BENCH_JUDGE_*</code>，所以不要指望它自动复用被测 Agent 的 provider 配置。
</Note>
