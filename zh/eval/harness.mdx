...
<Tip>
  这是进阶内容。日常评估只需要[快速上手](/zh/eval/quickstart)里的一条命令。这里讲的是 harness 内部机制，适合排查问题或理解 profile/task 选择。
</Tip>
...
CLI 会先确认 eval 根目录包含 `pyproject.toml` 和任务入口，再检查 `uv`。因此错误顺序可以直接帮助定位问题：
...
两者共用同一套参数构建逻辑（`buildEvalArgs`），唯一区别是 collect 加 `--collect-only`，普通执行加 `-v`。`--suite local|langsmith` 选择入口与报告目录；`--report-config`、`--html`、`--quick` 用于真实评估路径。
...
这是评估任意本地 Skill 的默认入口。传目录时默认跑 `generic-skill-smoke`（验证"Skill 能被注入、被调用、产出文件"），所以 `--quick` 这个 flag 可省；`--skill-name` 会从目录名自动推断。你也可以用 `--task` 显式指定任务，或用 `--profile` 覆盖 profile。

| 模式 | 默认 profile | 默认 task | 适合场景 |
| --- | --- | --- | --- |
| `--skill-path`（默认 quick） | `generic` | `generic-skill-smoke` | 评估自己的 Skill，跑通 Docker 评估 |
...
<p align="center">
  <img src="/assets/eval-harness-illustrations/01-docker-agent-rubric-scene.png" alt="小鱼在 Docker 隔离盒外观察被测 Agent、用户模拟 Agent 和 rubric 评估现场" width="800" />
</p>

<p align="center">一次真实评估会在隔离环境里运行 Agent 互动，再用校验器和 rubric 记录证据</p>
...
这些变量在 `eval/.env.example` 里都有占位项。把它们写进 `eval/.env`，harness 启动 Docker 时会注入。
