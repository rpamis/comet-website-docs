---
title: "Runtime check（进阶）"
description: "区分 comet eval 和 comet skill check，理解 Engine Run 的本地完成度检查、checks.yaml 格式和 pending action 恢复。"
---
...
| `comet skill check` | 某次 deterministic Engine Run | 这个 Run 是否缺 artifact 或状态 | 否 | `comet/checks.yaml` |

<p align="center">
  <img src="/assets/eval-runtime-illustrations/01-comet-eval-skill-eval-boundary.png" alt="小鱼把 comet eval 的发布证据和 comet skill check 的 Run 完成度检查分到两张工作台" width="800" />
</p>

<p align="center">comet eval 产出发布证据；comet skill check 只检查某次 Run 是否完整</p>

## 为什么有两类

`comet eval` 面向"这个 Skill 作为产品能力能不能通过评估"，它通过共享 eval harness 执行真实模型任务，产出发布前证据。

`comet skill check` 面向"这个正在运行的 deterministic Skill Run 是否完整"，它只检查某次 Run 是否满足 `comet/checks.yaml` 里的 runtime checks，**不执行模型任务**，也**不产出发布证据**。
...
<Warning>
  准备发布 Skill 时，<strong>不要</strong>只跑 <code>comet skill check</code>。发布 readiness 需要通用 <code>comet eval</code> 证据。<code>comet skill check</code> 只检查某次 Run 的完成度，不是通用 Skill 评估。
</Warning>
...
- [Skill 与 Engine（进阶）](/zh/skill-creator/engine) — 理解 Engine Run 的运行语义、pending action、不可变快照
