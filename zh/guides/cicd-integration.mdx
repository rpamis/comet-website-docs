...
Comet CLI 为自动化场景提供了结构化 JSON 输出和非交互模式。你可以在 CI/CD 流水线里用它们做安装完整性门禁、变更阶段报告和发布前检查，不需要 Agent 平台参与。

## 适合自动化的场景

| 场景           | 用什么                    | 说明                                    |
| -------------- | ------------------------- | --------------------------------------- |
| CI 环境初始化  | `comet init --yes --json` | 非交互安装，跳过交互式 CodeGraph 安装 |
...
<p align="center">
  <img
    ...
    alt="小鱼把 status 和 doctor 的 JSON 票据放入 CI 闸机，用 pass 和 fail 判断是否继续"
    width="800"
  />
</p>
...
<Accordion title="CI 里 status 输出空数组">
  ...
  需要在 Agent 平台调用 `/comet`，进入 Classic
  后由 Comet 接管。详见[存量项目接入](/zh/guides/existing-project)。
</Accordion>
