...
<p align="center">
  <img src="/assets/cicd-integration-illustrations/01-ci-gate-json-checks.png" alt="小鱼把 status 和 doctor 的 JSON 票据放入 CI 闸机，用 pass 和 fail 判断是否继续" width="800" />
</p>
...
说明没有活跃 change，或 change 缺少 `.comet.yaml`（被静默跳过）。缺少 `.comet.yaml` 的 change 需要在 Agent 平台用 `/comet-classic` 接管。详见[存量项目接入](/zh/guides/existing-project)。
</Accordion>
