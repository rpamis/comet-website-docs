...
| `.comet/skills/` | 项目 Skill 池（按名覆盖内置） | `comet skill install` |
...
<Note>
  <code>.comet.yaml</code> 和 <code>run-state.json</code> 的边界：<code>.comet.yaml</code> 保存<strong>用户可理解的工作流投影</strong>（phase、build_mode、verify_result 等），只通过 <code>run_id</code> 链接到 Engine；完整的 Run 细节在 <code>run-state.json</code>。所以你看 <code>.comet.yaml</code> 就能理解当前进度，不必读 machine-owned 的 Run 文件。
</Note>
