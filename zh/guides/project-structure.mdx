...
```text
.comet/
├── config.yaml               # 项目级 Comet 配置（auto_transition/review_mode/context_compression 等）
...
| 路径 | 职责 | 谁读写 |
| --- | --- | --- |
| `openspec/changes/<name>/.comet.yaml` | Comet 工作流状态（phase、workflow、build 决策、verify_result、run_id 链接） | Agent 读写，用户可读 |
...
| `.comet/config.yaml` | 项目级 Comet 配置 | 用户编辑 |
