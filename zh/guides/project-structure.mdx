...
Comet 把**需求事实、设计计划、运行状态和用户自定义 Skill** 分开保存。本页说明这些文件的位置和职责，帮助你恢复中断的工作、排查状态问题和做跨设备/CI 集成。
...
<Note>
  ...
  <a href="/zh/native/artifacts-and-state">产物与状态</a>。
</Note>
...
`classic.artifact_layout` 可配置为 `docs` 或 `legacy`：`docs` 对应 `docs/openspec/`，`legacy` 对应仓库根目录的 `openspec/`。只能在这两种布局中选择，不支持自定义目录路径。
...
│   │   │   ├── .comet.yaml               # Comet 工作流状态（用户可读 + run_id 链接）
...
<Tip>
  ...
  <strong>恢复依据</strong>：上下文压缩后 Agent
...
- **应该提交**：Classic 产物根（新项目为 `docs/openspec/`，保留旧布局的项目为 `openspec/`）、`docs/superpowers/`、`.comet/config.yaml`、`.comet/skill-preferences.yaml`。这些文件是跨设备恢复的依据，提交后才能在新设备上恢复。Native 产物的提交建议见[产物与状态](/zh/native/artifacts-and-state)。
...
<Note>
  ...
  <strong>用户可理解的工作流状态</strong>（phase、build_mode、verify_result 等），只通过{' '}
