...
该事件会将 `workflow` 和 `classic_profile` 设为 `full`，将 `phase` 返回 `design`，并清空 `design_doc`。随后加载 `comet-design` 创建 Design Doc。
...
恢复 `/comet` 时，如果工作区存在未提交改动，按 dirty-worktree 协议处理：
