...
Classic 初始化运行的 `openspec init` 是幂等的——OpenSpec 会处理当前已配置的根目录，不会破坏已有 change。若要迁移到新布局，先运行 `comet classic root move docs --dry-run` 查看计划，再按计划应用；不要只改 `classic.artifact_layout`。详见[项目文件结构](/zh/guides/project-structure)。
