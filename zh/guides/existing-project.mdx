...
Classic 初始化运行的 `openspec init` 是幂等的——OpenSpec 会处理当前已配置的根目录，不会破坏已有 change。若要迁移到新布局，先运行 `comet classic root move docs --dry-run` 查看当前状态、冲突和阻塞项，确认后直接运行 `comet classic root move docs --apply`；不要只改 `classic.artifact_layout`。迁移会包含完整 `openspec/` 目录，包括活跃和未完整归档的 change。详见[项目文件结构](/zh/guides/project-structure)。
