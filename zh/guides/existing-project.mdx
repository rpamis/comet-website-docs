...
<Tip>
  如果你之前用的是旧版 Superpowers（低于 6.0.0），建议升级到 6.0.0+：官方数据约为 2 倍速度、
  token 消耗约减半。升级时可选择覆盖 Superpowers 组件。
</Tip>
...
<Warning>
  这个接管路径只在 change 还处于 open 阶段（或刚创建还没推进）时能干净完成。如果一个 change 已经做到
  build 或 verify 但没有 `.comet.yaml`，就没有可靠的方式补挂 Comet 状态——需要重新开 change。
</Warning>
