...
- `/opsx:new` 只创建 OpenSpec artifacts，change 会落在状态机之外，无法用 `/comet-classic` 恢复和守卫。
...
flowchart LR
  ...
  B --> C["选择一个 change"]
  C --> D["/comet-classic 进入该 change 的 design"]
  D --> E["完整流转该 change 到 archive"]
  E --> F["/comet-classic 选择下一个 change"]
  F --> G["继续，直到全部归档"]
```

- 每个 change 独立走 design → build → verify → archive。
- 一个 change 归档后，用 `/comet-classic` 选择下一个 active change 继续。
...
```
你：/comet-classic 帮我实现完整的用户系统，包含注册登录、个人资料、权限管理、操作审计
```
