---
title: "大型 PRD 拆分"
description: "Comet 如何在 open 阶段把大型需求、路线图或完整产品方案拆成多个可独立设计、交付、验证和归档的 change：拆分信号、流程、恢复方式和真实场景。"
---

现实需求开发经常从一份大型 PRD、产品路线图或完整方案开始。直接塞进一个 change 会导致任务膨胀、delta spec 混乱、一个部分失败阻塞整体。Comet 的 PRD 拆分（open 阶段）在创建任何产物文件前，就帮你把大需求拆成多个可独立流转的 change。
...
PRD 拆分预检查在 `/comet-open` 完成需求澄清之后、创建文档之前触发，条件是输入属于以下任一情况：
...
| 信号 | 含义 |
| --- | --- |
| 多个可独立设计/构建/验证/归档的 capability | 各部分可以独立推进 |
...
- 已确认拆分项默认跳过 PRD 拆分预检查（除非该拆分项本身仍明显包含多个独立 capability）。
...
这里的依赖是强串行的，候选拆分清单标注了推荐执行顺序。你按 1→2→3→4 依次推进。
