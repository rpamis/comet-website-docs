...
0.4.0-beta.4 起，`comet init` 和 `comet update` 会把一段 Comet 恢复说明合并进项目的 `AGENTS.md` 和 `CLAUDE.md`。Agent 收到“继续刚才的登录重构”这类自然语言请求时，可以先做一次只读恢复探测（CLI 命令为 `comet resume-probe`），再决定是否进入现有 workflow。

| 场景 | 探测结果 |
| --- | --- |
| 只有一个相关 active change，状态完整且不在决策点 | 返回 `auto_resume`，按建议阶段继续 |
| 有多个 active change、未提交改动或需要用户选择 | 返回 `ask_user`，先让你确认 |
| 只是问问题、明确不走 Comet，或当前已在 Comet 流程内 | 返回 `out_of_scope`，不重复进入 workflow |
| 没有 active Comet change | 返回 `none`，不执行恢复 |

这段说明只约束 Comet 的恢复探测，并会保留文件中已有的用户规则。它不会把所有后续请求强行路由到 Comet，也不会绕过已有决策点。你仍然可以随时显式输入 `/comet`；这是最直接、最确定的恢复方式。
