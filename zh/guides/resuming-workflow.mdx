...
就这样。Comet 不依赖对话历史：Native 重读 `comet.config.yaml`、`<artifact-root>/comet/`、实现和验证证据；Classic 重读 OpenSpec、`.comet.yaml` 与运行证据。
...
这段说明只约束 Comet 的恢复探测，并会保留文件中已有的用户规则。探针先解析配置，再只检查一套工作流；配置损坏时不会回退。它不会把所有后续请求强行路由到 Comet，也不会绕过已有决策点。你仍然可以随时显式输入 `/comet`；这是最直接、最确定的恢复方式。
...
状态完全保存在仓库文件里：Native 使用 `comet.config.yaml` 与 `<artifact-root>/comet/`，Classic 使用 `.comet.yaml`、OpenSpec 和 `docs/superpowers/`。所以在另一台设备或另一个 Agent 平台打开同一个仓库，`/comet` 能从对应文件恢复。
...
如果 Classic change 所在的 Agent 平台自动压缩了上下文（之前的对话被摘要了），输入 `/comet-classic`。Agent 内部会按上下文压缩恢复协议重载状态——必要时它会读 `brainstorm-summary.md`、handoff 交接包、`.comet/subagent-progress.md` 等检查点文件继续。你不需要手动操作这些文件。

## 从任意阶段入口恢复

Classic 正常恢复可用 `/comet-classic`；也可以直接从任意阶段或预设入口恢复——`/comet-open`、`/comet-design`、`/comet-build`、`/comet-verify`、`/comet-archive`、`/comet-hotfix`、`/comet-tweak`。Native 始终使用 `/comet-native`。
...
<Tip>
  这条规则让你在 <strong>已知自己在哪个阶段</strong> 时跳过 <code>/comet-classic</code>{' '}
