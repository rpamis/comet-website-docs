---
title: "状态损坏与恢复"
description: "当 /comet-classic 路由不对、.comet.yaml 缺失或格式异常、证据缺失时，如何用 comet status / comet doctor 诊断和恢复。"
---

Classic 正常恢复只需要 `/comet-classic`（见[恢复中断的工作](/zh/guides/resuming-workflow)）。**本页是当 `/comet-classic` 路由不对、或状态看起来坏了时**的诊断和恢复指南。
...
| 情况 | 这算什么 | 怎么办 |
| --- | --- | --- |
| 会话断了、上下文压缩了、换了设备 | 正常中断 | 直接 `/comet-classic`（见[恢复中断的工作](/zh/guides/resuming-workflow)） |
...
| build 无法进入 verify | 缺 `build_mode`/`isolation`/`tdd_mode` | 回 build 阶段补选（`/comet-classic` 会路由过去让你选） |
...
flowchart TD
  ...
  I --> J["重新 /comet-classic"]
```

## 下一步

- [恢复探测命令](/zh/cli/resume-probe) — 区分可自动恢复、需要确认和不应进入 workflow 的请求
- [恢复中断的工作](/zh/guides/resuming-workflow) — Classic 正常中断只需 `/comet-classic`
