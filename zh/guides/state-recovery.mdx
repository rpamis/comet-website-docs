...
Classic 的正常恢复只需要统一入口 `/comet`（见[恢复中断的工作](/zh/guides/resuming-workflow)）。
配置进入 Classic 后，内部 `/comet-classic` 会读取文件状态并选择阶段。本页只处理内部路由异常或状态损坏。

状态损坏时，先**恢复仓库的真实状态**，再让流程基于正确状态继续。请勿为了解锁流程而强行修改字段。
...
| `/comet-classic` 路由到的阶段不对  | 可能元数据和文件冲突       | `/comet-classic` 会以文件为准自动修正。如果仍不对，运行诊断       |
...
加 `--json` 适合给 Agent 或自动化工具读取。普通用户先看文本输出即可。

恢复探测只负责 workflow 入口前的判断。
它返回 `ask_user` 时，多数情况代表多个 change、未提交改动或用户决策点，不等于状态已经损坏。详见[恢复探测命令](/zh/cli/resume-probe)。
...
| archive 后仍显示 active                    | 手工移动目录或状态没同步               | 用 `openspec status` 核对，再用归档脚本重跑（可恢复）                     |
| `/comet-classic` 报 `.comet.yaml` 格式异常 | 文件被手工改坏                         | 先运行 `comet doctor`。严重时从 git 恢复 `.comet.yaml`                    |

## 恢复原则

- **以文件为证据**：OpenSpec 产物、Design Doc、Plan、测试结果、实际工作树是状态的唯一可信来源。
- **不要手工写 machine-owned Run 字段**（`.comet/run-state.json` 等，由 Comet 自动维护）。
