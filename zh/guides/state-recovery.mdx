---
title: "状态损坏与恢复"
description: "当 /comet 路由不对、.comet.yaml 缺失或格式异常、证据缺失时，如何用 comet status / comet doctor 诊断和恢复。"
---

正常恢复只需要 `/comet`（见[恢复中断的工作](/zh/guides/resuming-workflow)）。**本页是当 `/comet` 路由不对、或状态看起来坏了时**的诊断和恢复指南。

状态损坏时，目标是**恢复真实事实**，而不是强行改字段让流程继续。

<p align="center">
  <img src="/assets/state-recovery-illustrations/01-state-recovery-first-aid.png" alt="小鱼在状态急救台上用 status 和 doctor 诊断坏状态，并按文件事实恢复证据" width="800" />
</p>
...
| `/comet` 路由到的阶段不对 | 可能元数据和文件冲突 | `/comet` 会以文件为准自愈；若仍不对，跑诊断 |
...
| build 无法进入 verify | 缺 `build_mode`/`isolation`/`tdd_mode` | 回 build 阶段补选（`/comet` 会路由过去让你选） |
| verify 通过但不能 archive | 缺验证报告或分支状态未处理 | 补验证报告、设 `branch_status: handled` |
| archive 后仍显示 active | 手工移动目录或状态没同步 | 用 `openspec status` 核对，用归档脚本重跑（可恢复） |
| `/comet` 报 `.comet.yaml` 格式异常 | 文件被手工改坏 | `comet doctor`；严重时从 git 恢复 `.comet.yaml` |
...
- **不要手工伪造 archive 状态**——归档由 `/comet-archive` 脚本完成。
- **不要手工编辑 `.comet.yaml` 推进 phase**——用 guard `--apply` 或 `comet-state transition`。
...
如果 `/comet` 自己修不了（比如文件被严重改坏），从 git 恢复 `.comet.yaml`，再跑 `comet doctor` 确认。

## 推荐处理流程

```mermaid
flowchart TD
  A["/comet 路由不对或状态异常"] --> B["comet status 看 runtime_eval"]
...
- [恢复中断的工作](/zh/guides/resuming-workflow) — 正常中断只需 `/comet`
- [项目文件结构](/zh/guides/project-structure) — 状态都存在哪些文件里
- [状态与配置](/zh/concepts/state-management) — `.comet.yaml` 字段和状态守卫
