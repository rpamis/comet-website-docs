...
| 组合Skill 产物形态 | 只是一个 `SKILL.md` 单文件，没法组合、没法带依赖、没法分发 | `/comet-any` 生成稳定**组合 Skill Bundle**——可拼装 rule、hook、Engine 元数据；**可评审**，**可分发**（像 `comet init` 一样通过 `comet publish distribute` 落地到 34 个 AI 编码平台） |
