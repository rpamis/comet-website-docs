---
title: "工作流概念"
description: "理解 /comet 为什么把 OpenSpec 的 WHAT 和 Superpowers 的 HOW 串成五阶段，产物如何交接，重叠 spec 如何处理，以及如何归档。"
---
...
| archive | OpenSpec | 合并 delta spec 并归档 | `openspec/changes/archive/YYYY-MM-DD-name/` |
...
这是两套工具物理对接的阶段。Comet 用 `comet-handoff.mjs` 生成一个交接包，把 open 阶段的 OpenSpec artifacts 整理成 Superpowers 能读懂的上下文，再交给 Superpowers `brainstorming` 做深度设计。

```mermaid
flowchart LR
  A["OpenSpec artifacts"] --> B["comet-handoff.mjs --write"]
...
关键约束：**Agent 不能写第二份需求 spec**。如果 delta spec 缺少验收场景，只能在 `specs/*/spec.md` 上写 Spec Patch（回写），不能扩大范围。如果写了 Spec Patch，必须重新生成 handoff 更新 hash。

### build — Superpowers 执行，feed by WHAT
...
计划输入是 Design Doc（Superpowers）+ `tasks.md`（OpenSpec 任务边界）。执行方式由用户选择：`subagent-driven-development` 或 `executing-plans`，配合 `isolation`（branch/worktree）、`tdd_mode`、`review_mode`。
...
如果 verify 发现 delta spec 有 Design Doc 没反映的内容，让你三选一：

| 选项 | 含义 |
| --- | --- |
| A | 在 design doc 追加"Implementation Divergence"说明 |
| B | 退回 build + brainstorming 重新对齐 |
...
9. **标注 frontmatter**：design doc 加 `archived-with` 和 `status: final`，plan 加 `archived-with`。幂等设计，已有标注会先移除再重写。
