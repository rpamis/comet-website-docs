...
| archive | OpenSpec    | 合并 delta spec 并归档，给 Superpowers 文档打标 | `<classic-root>/changes/archive/YYYY-MM-DD-name/`、design doc/plan 的 `archived-with` 标注 |
...
计划输入是 Design Doc（Superpowers）+ `tasks.md`（OpenSpec 任务边界）。Design Doc 已经在 design 阶段消化了 proposal、原始 design 和 delta spec，所以 build 阶段不必回读这些原始产物；`tasks.md` 则作为活文档持续核对任务边界。执行方式由用户选择：`subagent-driven-development` 或 `executing-plans`，配合 `isolation`（branch/worktree）、`tdd_mode`、`review_mode`。
...
| 模式           | 产出文件                                    | 内容                                                                    |
| -------------- | ------------------------------------------- | ----------------------------------------------------------------------- |
| `off`（默认）  | `design-context.json` + `design-context.md` | 完整产物内容，每文件带 SHA256                                    |
...
Comet 会自动推进无歧义阶段，并把产品或风险决策交给你。常见阻塞点包括：
