---
title: '工作流概念'
description: '理解 /comet-classic 为什么把 OpenSpec 的 Spec 管理能力和 Superpowers 的执行能力串成五阶段。'
---

`/comet-classic` 是 Comet 的经典 Spec 模式永久入口。它把一次变更拆成五个阶段：open、design、build、verify、archive，把 OpenSpec 和 Superpowers 连成一条可恢复的工作流。项目配置为 Classic 时，统一别名 `/comet` 会确定性转发到这里；它不会与 Native change 混用。组合 OpenSpec + Superpowers 不是 Comet 的最终目的，我们希望持续追踪这类长程任务 Skill，找到能让长链路 Skill 稳定执行的 Harness 能力；如果你也感兴趣，欢迎参与项目贡献，或通过源码进行学习。

## 哪些值得注意

| 什么是需要关心的 | 结论                                                                  |
| ---------------- | --------------------------------------------------------------------- |
| 入口             | 直接使用 `/comet-classic`；项目默认配置为 Classic 时也可输入 `/comet` |
...
## `/comet-classic` 如何判断入口

每次调用 `/comet-classic`，Comet 都会重新读取活跃 change 和 `.comet.yaml`，而不是依赖聊天历史。入口判断不再只靠提示词里的经验规则；Comet 会把用户原话、active change 列表和风险信号整理成结构化路由上下文（实现类型名为 `CometIntentFrame`），再交给运行时评分得到最终 route。
...
```mermaid
flowchart TD
  A["调用 /comet-classic"] --> B["openspec list --json"]
  B --> C["整理路由上下文"]
  C --> D["运行时评分 route"]
  D --> E{"route?"}
  E -->|full| F["/comet-open"]
