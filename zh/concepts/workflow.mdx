...
`/comet` 是 Comet 的经典 Spec 模式入口。它把一次变更拆成五个阶段：open、design、build、verify、archive。这五个阶段把 OpenSpec 和 Superpowers 两套工具连成一条可恢复的工作流。组合 OpenSpec + Superpowers 不是 Comet 的最终目的，我们希望持续追踪这类长程任务 Skill，找到能让长链路 Skill 稳定执行的 Harness 能力；如果你也感兴趣，欢迎参与项目贡献，或通过源码进行学习。

## 哪些值得注意

| 什么是需要关心的 | 结论 |
| --- | --- |
| 入口 | 大多数时候只输入 `/comet`，让 Comet 判断当前阶段 |
