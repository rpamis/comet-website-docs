...
**写入放行**：默认情况下，项目内（产物区之外）的写入受 Classic 阶段守卫约束。如果实现代码目录需要在任意阶段都可写，可在 `.comet/config.yaml` 配置共享的 `hook.allow_paths`，详见 [Native 配置 — Hook 写入放行](/zh/native/configuration#hook-写入放行)。
