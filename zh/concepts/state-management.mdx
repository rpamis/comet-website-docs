...
全局 `comet init` 和 `comet update` 会把选择的产物语言保存到 `~/.comet/config.yaml`。新 workflow 优先使用项目 `.comet/config.yaml`；项目没有配置语言时才回退到全局默认。因此个人可以设置跨项目默认值，团队项目仍能用仓库内配置覆盖。
...
| `isolation` | `current` \| `branch` \| `worktree` | 工作区隔离方式。full workflow 离开 build 前需 `branch` 或 `worktree`；hotfix/tweak 默认 `current`（如实记录当前工作区），创建分支/工作区后才改为 `branch`/`worktree` |
...
| 约束 | 说明 |
| --- | --- |
| full workflow `build → verify` 前 | `isolation` 必须是 `branch` 或 `worktree`；hotfix/tweak 可用 `current` |
