...
| 控制是否自动探测当前用户问题需要进入现有comet流程     | `.comet/config.yaml` 的 `ambient_resume`              | 默认 `true`；不影响显式 `/comet`              |
| 放行实现目录在任意阶段可写     | `.comet/config.yaml` 的 `hook.allow_paths`            | 默认空；`.comet`、产物根等受保护区域无法放行  |
...
| `hook.allow_paths` | 项目相对目录列表（默认空） | 受保护阶段允许 Agent 写入的项目相对目录；前缀匹配，受保护区域无法放行。详见 [Native 配置](/zh/native/configuration#hook-写入放行) |

`hook.allow_paths` 是 Native 与 Classic 共享的写入放行策略：默认为空时，项目内（产物区之外）的写入按各工作流阶段规则处理；配置后匹配目录及其子目录的写入会直接放行。`.comet`、产物根、openspec/superpowers 根永远受保护，无法通过它放行。完整说明见 [Native 配置 — Hook 写入放行](/zh/native/configuration#hook-写入放行)。
...
`native.artifact_root`、`native.language` 和 `native.clarification_mode` 的含义及完整示例见 [Native 配置](/zh/native/configuration)。Native 不读取 Classic 的 `context_compression`、`review_mode` 或 `auto_transition`。
