...
| 字段 | 允许值 | 含义 |
|-------|---------------|---------|
| `workflow` | `full`, `hotfix`, `tweak` | 此 change 使用的 workflow 预设。它在 init 时设置，并驱动整个 pipeline 中的 routing decisions。 |
...
- **`full` workflow 中的 `build_mode: direct` 需要 `direct_override: true`**。没有此 flag 时，guard 会拒绝 transition。`hotfix` 和 `tweak` 预设可以自由使用 `direct`。
