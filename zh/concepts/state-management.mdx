...
<Warning>
  <code>build_command</code> 和 <code>verify_command</code> 已移除，不再是合法字段。它们出现在 change 的 <code>.comet.yaml</code> 中时会被当作未知字段拒绝，而不是作为兼容配置读取。build guard 会自动探测项目构建命令（例如 <code>npm run build</code>、Maven 或 Cargo），没有探测到时会要求你补齐可验证的构建路径。
</Warning>
