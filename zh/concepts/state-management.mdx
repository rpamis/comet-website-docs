...
- 选择会记录 branch；切换 branch/worktree、目标归档或选择文件损坏后，守卫会失败闭合。
...
| `branch_status` | `pending` \| `handled` | 分支处理状态。verify 和 archive 期间保持 `pending`；归档提交和分支处理完成后才写 `handled` |
...
| archive 完成 | 归档提交 + 分支处理后写 `branch_status: handled`，`comet guard archive` 通过 |
