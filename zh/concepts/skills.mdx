...
Comet 的核心能力通过 Skill 组织。经典 Spec 模式的 `/comet-classic` 用户不需要深入理解 Skill 的底层机制——你只需要知道有哪几类 Skill、它们各自的作用、以及它们安装在哪里。`/comet` 只是按项目配置选择 Native 或 Classic 的薄入口。
...
这些 Skill 随 Comet 包分发，Classic 初始化会安装到你的平台目录。你只需要用 `/comet-classic` 调用；项目默认配置为 Classic 时也可以使用 `/comet`。
...
<Note>
  ...
  这些内部概念——它们只存在于实现和审计证据里。用户视角只有三个起点：<strong>定制 /comet-classic</strong>、
  <strong>创建新 Skill</strong>、<strong>升级已有 Skill</strong>。
</Note>
