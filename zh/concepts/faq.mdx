...
description: '解答 /comet-classic 五阶段工作流、状态恢复、阶段守卫和轻量预设的常见疑问。'
---

关于经典 Spec 模式的 `/comet-classic` 五阶段工作流、状态、守卫和预设的常见问题。
...
<Accordion title="/comet 和 /comet-any 有什么区别">
  `/comet-classic` 是经典 Spec 模式永久入口，驱动 open/design/build/verify/archive
  五阶段流程。`/comet` 是项目配置驱动的统一别名，可能进入 Native 或 Classic。`/comet-any` 是 Skill
  Creator，用来创建、优化、组合可复用的 Skill；三者职责不同。
</Accordion>
...
<AccordionGroup>
  <Accordion title="中断后再回来，Agent 怎么知道做到哪了">
    Comet 不依赖聊天历史。每次调用 `/comet-classic` 都会重新读取活跃 change 的 `.comet.yaml` 和 OpenSpec artifacts，判断当前阶段和证据是否一致，然后路由到正确的阶段 Skill。详见[工作流概念](/zh/concepts/workflow)的"自动检测怎么工作"。
  </Accordion>

<Accordion title="comet status 看不到我的 change">
  这个 change 可能是用原始 `/opsx:new` 创建的，缺少 `.comet.yaml`，会被 `comet status` 静默跳过。在
  Agent 平台调用 `/comet-classic`
  ...
  <Accordion title="上下文压缩后丢了上下文怎么办">
    design 阶段写了 `brainstorm-summary.md` 作为恢复检查点，build 阶段的子代理有持久化 checkpoint。恢复时调用 `/comet-classic`，它会从文件状态恢复而不是靠记忆。详见[恢复中断的工作](/zh/guides/resuming-workflow)。
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="代码写到一半，我不满意方案怎么办">
    ...
    - **你改了 spec、design 或 tasks**：直接再次调用 `/comet-classic`。Comet 会重新读取 `.comet.yaml` 和 OpenSpec artifacts，按当前 phase 恢复到正确阶段。build 阶段发现 spec 变化时，会按增量更新规则处理：小改直接更新 delta spec 和 tasks；中等设计变化会重新 brainstorming；变成新需求时会让你拆成新 change。
    - **你自己改了代码**：也直接再次调用 `/comet-classic`，并告诉 Agent："我改过代码，请按当前工作区恢复。" 只要存在未提交改动，Comet 会走 dirty-worktree 协议，先看 `git status` 和 diff，把改动归因到当前 change、其他 change 或来源不确定，再决定继续、并入、拆分或请你确认。

    如果 spec 和代码都改了，也让 `/comet-classic` 先恢复。你需要做的是说明这些改动是否代表新方案；不要手工改 `.comet.yaml` 的 `phase` 或 `.comet/run-state.json`。Comet 的原则是以当前文件状态和持久化 artifacts 为准，而不是靠聊天记忆继续。详见[中途修改 Spec 或回退工作流](/zh/guides/mid-workflow-changes)和[状态与配置](/zh/concepts/state-management)。
