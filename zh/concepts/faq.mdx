...
<Accordion title="comet status 看不到我的 change">
  ...
  <Accordion title=".comet.yaml 能手工编辑吗">
    用户可见字段（workflow、phase、build_mode 等）原则上通过 `/comet-classic` 和阶段守卫流转，不要手工改 `phase`。machine-owned Run 字段（在 `.comet/run-state.json` 或 `.comet/runs/<run-id>`）绝对不要手工改。排障时可以用 `comet-state` 命令，详见[状态管理](/zh/concepts/state-management)。
  </Accordion>
