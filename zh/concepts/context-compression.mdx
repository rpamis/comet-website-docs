...
它和传统的摘要式压缩不同。摘要式压缩把重点内容总结后传递，在 Spec 任务上容易漏掉任务和细节，导致 Spec 漂移，进而影响下游 Agent 的实现。Comet 的做法是结构化投影加 hash 引用：delta spec 原样保留，其余文件用 hash 关联源文件，需要时回读原文，在保证 Spec 完整的同时减少 token 消耗。
