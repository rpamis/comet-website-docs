...
上下文压缩（context compression）控制 design→build 阶段交接时，有多少 OpenSpec 上下文传给 Superpowers。它是一个 beta 功能，目的是在不损失实现正确性的前提下减少 token 消耗。在Spec Coding任务上与传统的上下文摘要式压缩不同，传统上下文摘要式压缩将重点内容进行了总结，在Spec任务上往往会导致漏掉任务，漏掉细节的问题。这会导致Spec产生漂移，从而完全影响下游Agent的任务实现。Comet的上下文压缩机制通过结构化投影和hash引用的方式，保证了Spec任务的完整性，同时减少了token消耗。
...
用 `--full` 标志可以输出未截断的 verbatim 完整内容。
...
- proposal/design/tasks 的完整性由 sha256 验证；需要时按 hash 回查原文件，Agent 摘要只用于快速阅读。
