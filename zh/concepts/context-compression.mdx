...
- `spec-context.md` — 原样投影 delta spec，包括验收场景；proposal、design 和 tasks 只记录 hash 引用
...
- delta spec 的正文不经过摘要。
- proposal、design 和 tasks 通过 sha256 关联源文件，需要时可以回读原文。
...
在该基准中，beta 模式的 token 使用量降低约 25–30%，spec 覆盖率从 100% 变为 95%。原因是 supporting 文件只保留 hash 引用。
