---
title: "意图识别与路由"
description: "了解 /comet 如何用结构化路由上下文把你的自然语言请求变成可解释的 full / hotfix / tweak / resume 路由，而不是靠经验规则猜测。"
---

每次你输入 `/comet` 加一句话，Comet 都要做一个关键判断：这次到底该走哪条工作流？是完整的五阶段 `full`，还是快速修 bug 的 `hotfix`，还是 OpenSpec 链路的 `tweak`，又或者你是想恢复之前没做完的变更？

0.4.0 起，这个判断不再依赖提示词里一长串"经验规则"。Comet 会把你的原话、仓库状态和风险信号整理成一个结构化的 **路由上下文**（实现类型名为 `CometIntentFrame`），再交给运行时评分，得到一个可解释、可复核的路由结果。这篇文章讲清楚这套机制怎么工作，以及它为什么这样设计。
...
<Note>
  一句话概括：路由上下文让 <code>/comet</code> 的路由<strong>可解释、可复核、可恢复</strong>，而不是一个黑盒猜测。
</Note>

在路由上下文之前，`/comet` 入口靠的是 Skill 正文里的一段自然语言规则——"如果是 bug 就走 hotfix，如果是文案改动就走 tweak……"。这种做法有三个问题：
...
这份路由上下文由六个顶层部分组成。你可以把它想成一份"路由申请单"：
...
| 部分 | 作用 |
| --- | --- |
| `utterance` | 触发 `/comet` 的用户原话，便于事后追溯 |
...
槽位里最影响路由的是几个**风险信号**布尔值。它们决定了"这事是不是该老老实实走完整流程"：
...
只要命中**任何一个**风险信号（`new_capability`/`public_api_change`/`schema_change`/`cross_module_change`），运行时就倾向于走 `full`，哪怕你嘴上说的是"小改一下"。

<Warning>
  这是 Comet 的安全设计：<strong>风险信号优先于你口头说的流程</strong>。如果你明确要求走 <code>hotfix</code> 但同时命中了风险信号，运行时会停下来问你（<code>ask_user</code>），而不是默默放行一个高风险的轻量流程。
</Warning>

## 运行时怎么评分

Agent 整理好路由上下文后，运行时（`comet-intent.mjs`）会按一套**固定优先级**重新评分，给出最终 `route`。这套顺序是可解释路由的核心：

```mermaid
flowchart TD
    Start(["运行时评分开始"]) --> C1{"意图置信度<br/>≥ 0.7？"}
    C1 -->|否| AU1["ask_user<br/>置信度不足"]
    C1 -->|是| C2{"恢复/继续 +<br/>有多个 active change<br/>且未指定 change_id？"}
    C2 -->|是| AU2["ask_user<br/>多个变更需指定"]
    C2 -->|否| C3{"恢复 + 指定了<br/>有效的 change_id？"}
    C3 -->|是| RES["resume"]
    C3 -->|否| C4{"只是提问，<br/>没要求执行工作流？"}
    C4 -->|是| OOS["out_of_scope"]
    C4 -->|否| C5{"显式流程<br/>与风险信号冲突？"}
    C5 -->|是| AU3["ask_user<br/>流程与风险冲突"]
    C5 -->|否| C6{"用户明确<br/>指定了流程？"}
    C6 -->|是| WF["走显式流程<br/>full/hotfix/tweak"]
    C6 -->|否| C7{"命中风险信号？"}
    C7 -->|是| FULL1["full"]
    C7 -->|否| C8{"修 bug +<br/>有 workflow 证据？"}
    C8 -->|是| HF["hotfix"]
    C8 -->|否| C9{"小改 + 有<br/>tweak 证据？"}
    C9 -->|是| TW["tweak"]
    C9 -->|否| C10{"有 workflow_candidate<br/>且有证据？"}
    C10 -->|是| WF2["走候选流程"]
    C10 -->|否| AU4["ask_user<br/>证据不足或歧义"]
```
...
对普通用户，你**不需要手写路由上下文**——上下文整理和评分都是 `/comet` 内部自动完成的。你只需要知道：
...
路由上下文让 `/comet` 能自动路由到 `tweak`，那 `/comet-tweak` 这个独立命令还有用吗？

有用，但定位不同：

- **`/comet`**：通用入口，由路由上下文决定走 `full`/`hotfix`/`tweak`/`resume`。适合不确定走哪条路时。
- **`/comet-tweak`**：**tweak 专用**的 OpenSpec action 路径。你已经明确知道要做一次 OpenSpec 链路的中等改动，直接走它更快。

换句话说，`/comet` 是"帮我判断该走哪条"，`/comet-tweak` 是"我已经知道要走 tweak"。完整的五阶段 `full` 仍然走 Superpowers 的 design/plan/build 路径。
