---
title: "意图识别与路由"
description: "了解 /comet 如何用 CometIntentFrame 把你的自然语言请求变成可解释的 full / hotfix / tweak / resume 路由，而不是靠经验规则猜测。"
---

每次你输入 `/comet` 加一句话，Comet 都要做一个关键判断：这次到底该走哪条工作流？是完整的五阶段 `full`，还是快速修 bug 的 `hotfix`，还是 OpenSpec 链路的 `tweak`，又或者你是想恢复之前没做完的变更？

0.4.0 起，这个判断不再依赖提示词里一长串"经验规则"。Comet 会把你的原话、仓库状态和风险信号整理成一个结构化的 **`CometIntentFrame`**（意图帧），再交给运行时评分，得到一个可解释、可复核的路由结果。这篇文章讲清楚这套机制怎么工作，以及它为什么这样设计。

## 为什么需要意图帧

<Note>
  一句话概括：意图帧让 <code>/comet</code> 的路由<strong>可解释、可复核、可恢复</strong>，而不是一个黑盒猜测。
</Note>

在意图帧之前，`/comet` 入口靠的是 Skill 正文里的一段自然语言规则——"如果是 bug 就走 hotfix，如果是文案改动就走 tweak……"。这种做法有三个问题：
...
意图帧把"用户想干什么"变成一份**有字段、有证据、有置信度**的结构化数据。运行时不再信任 Agent 自己提的路由建议，而是**重新评分**后给出最终路由，并把分歧写进诊断日志。
...
`CometIntentFrame` 由六个顶层部分组成。你可以把它想成一份"路由申请单"：

```mermaid
flowchart LR
    U["你的原话<br/>utterance"] --> F["CometIntentFrame"]
...
Agent 填好意图帧后，运行时（`comet-intent.mjs`）会按一套**固定优先级**重新评分，给出最终 `route`。这套顺序是可解释路由的核心：
...
意图帧让 `/comet` 能自动路由到 `tweak`，那 `/comet-tweak` 这个独立命令还有用吗？

有用，但定位不同：

- **`/comet`**：通用入口，由意图帧决定走 `full`/`hotfix`/`tweak`/`resume`。适合不确定走哪条路时。
