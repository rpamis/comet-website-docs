...
description: '输入一句话后，Comet 会判断这次该走哪条流程，把调用路由到 full、hotfix、tweak、resume、ask_user、out_of_scope 六种结果之一；证据不足或有冲突时，它会停下来问你。'
---

用户只需要输入 `/comet` 加一句话。项目配置选择 Classic 后，内部 `/comet-classic` 要判断这次该走哪条 Classic profile；这个判断由**意图识别**、**槽位提取**和实际文件状态共同驱动。
...
你只需要输入一句话，Comet 会把这次调用路由到下面六种结果之一：
...
对普通用户，你**不需要手写路由上下文**。调用 `/comet` 后，配置选择 Classic，再由 `/comet-classic` 在内部自动完成上下文整理和评分。你只需要知道：
...
两者的差别在于：`/comet-classic` 是“在 Classic 内帮我判断该走哪条”，`/comet-tweak` 是“我已经知道要走 tweak”。完整的五阶段 `full` 仍然走 Superpowers 的 design/plan/build 路径。
...
路由上下文把"用户想干什么"变成一份**有字段、有证据、有置信度**的结构化数据。运行时不再直接采纳 Agent 提的路由建议，而是**重新评分**后给出最终路由，并把分歧写进诊断日志。

0.4.0 起，这套机制（实现为 `CometIntentFrame`）取代了 Skill 正文里的自然语言经验规则，让路由结果变得**可解释、可复核、可恢复**；在我们的基准测试中，Comet 的可恢复能力因此大幅提升。
