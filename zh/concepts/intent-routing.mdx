...
<Note>
  一句话概括：路由上下文让 <code>/comet-classic</code> 的路由<strong>可解释、可复核、可恢复</strong>
  ，而不是一个黑盒猜测。
</Note>
...
<Info>
  运行时评分和 Agent 的 <code>proposed_route</code> 可能不一致。这种时候运行时输出最终{' '}
  <code>route</code>，并把分歧写进 <code>diagnostics</code>（例如"agent proposed_route 'hotfix'
  normalized to 'full'"）。这让路由<strong>可复核</strong>：出问题时你能看到是哪一步改写了建议。
</Info>
