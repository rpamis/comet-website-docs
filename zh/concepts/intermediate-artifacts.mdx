...
<Note>
  Comet 的经典工作流<strong>刻意避免</strong>创建一堆内部 JSON 状态文件。跨会话恢复靠的是 <code>.comet.yaml</code> + handoff 文件 + 检查点 markdown，而不是一批 machine-owned JSON。<code>.comet/run-state.json</code>、<code>trajectory.jsonl</code> 这些是 Skill Engine（<code>comet skill run</code>）的内部文件，经典 <code>/comet</code> 工作流不用。
</Note>
