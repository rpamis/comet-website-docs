---
title: "工作流中间产物"
description: "理解 Comet 在五阶段工作流中创建的中间产物——handoff 交接包、brainstorm-summary、subagent-progress、验证报告和归档标注——各自的作用、位置和生命周期。"
---

Comet 在 `/comet-classic` 五阶段流程中会创建一系列中间产物。它们不是最终交付物（proposal、design、tasks、代码），而是 Comet 用来做**上下文压缩、状态恢复、证据记录和阶段交接**的工作文件。
...
flowchart TD
  ...
  subgraph build["build 阶段"]
    ...
    B2["subagent-progress.md<br/>SubAgent调度检查点"]
    B3["tasks.md 勾选状态"]
  end
...
<p align="center">
  <img src="/assets/intermediate-artifacts-illustrations/01-artifact-purposes.png" alt="小鱼把 Comet 中间产物按上下文、恢复、交接和验证证据归类" width="800" />
</p>

<p align="center">中间产物不是散落文件，而是分别服务上下文、恢复、交接和验证证据</p>
...
这是 Comet 唯一的跨阶段状态文件。字段随阶段推进逐步填写。详见[状态与配置](/zh/concepts/state-management)。

<Note>
  Comet 的经典工作流仍以 <code>.comet.yaml</code> 作为当前状态来源。<code>.comet/state-events.jsonl</code> 是追加式审计日志，用来解释状态转换历史；<code>.comet/run-state.json</code> 和 <code>trajectory.jsonl</code> 属于 machine-owned 运行细节。跨会话恢复主要靠 <code>.comet.yaml</code>、handoff 文件和检查点 markdown，排查状态为何变化时再看事件日志。
</Note>
...
| 属性 | 说明 |
| --- | --- |
| 位置 | `openspec/changes/<name>/.comet/state-events.jsonl` |
...
<Tip>
  <code>brainstorm-summary.md</code> 是 design 阶段最重要的恢复锚点。上下文压缩后，Agent 从这里恢复已确认决策，而不是重新 brainstorming。它在 brainstorming 过程中<strong>增量更新</strong>，在用户确认方案后定稿。
</Tip>

## build 阶段产物

build 阶段的中间产物主要服务于SubAgent调度恢复和任务跟踪。
...
| 创建 | build 阶段（由SubAgent加载 `writing-plans` 生成） |
...
| 作用 | **SubAgent调度检查点**——`brainstorm-summary.md` 在 build 阶段的对应物 |
...
<Warning>
  <code>subagent-progress.md</code> 每个任务替换一次。SubAgent（implementer/reviewer）<strong>不能</strong>编辑它——只有协调会话能写。实现者只能写代码，审查者只能写反馈，协调会话负责推进检查点。这避免SubAgent擅自标记任务完成。
</Warning>
...
SubAgent模式下，实现者**不能**勾选任务——只有协调会话在双审查通过后才勾选。
...
| SubAgent做到哪个任务的哪一步 | `.comet/subagent-progress.md` |
