...
| 作用 | **SubAgent 调度检查点**——`brainstorm-summary.md` 在 build 阶段的对应物 |
...
<Warning>
  ...
  <strong>不能</strong>
  编辑它——只有主会话能写。实现者只能写代码，审查者只能写反馈，主会话负责推进检查点。这避免 SubAgent 擅自标记任务完成。
</Warning>
...
SubAgent 模式下，实现者**不能**勾选任务——只有协调会话在双审查通过后才勾选。
