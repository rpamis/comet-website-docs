...
`review_mode` 控制 Classic build 阶段何时派发自动代码审查，以及允许多少轮修复。`off` 不派审查者；`standard` 审查风险任务并执行一次最终轻量审查；`thorough` 审查每个任务，并执行一次最终完整审查。
...
<Info>
  在 build 阶段选择 `off`、`standard` 或 `thorough`。Superpowers SDD 可以独立升级实现，但需要继续满足公开的 task reviewer 合同。
</Info>
## build 阶段审查预算表
