...
- **`review_mode: thorough`**：除最终一次审查外，每 3 个任务做一次**分段代码审查**（按该段 diff 范围）。总任务 ≤ 3 时跳过中间分段，只做最终审查。每段审查用 `requesting-code-review` 针对该段 commit 区间。这是 `executing-plans` 下最接近每任务审查的做法：主会话没有隔离的实现者子代理，无法逐任务派发审查者。
...
<Note>
  ...
  （风险触发的每任务审查）处理，而不再是无审查。这一默认值统一作用于初始化模板、运行时兜底和旧版本的兼容路径。
</Note>
