...
**0.4.0 的决策**：把所有脚本重写为 Node `.mjs` 启动器，背后由一份共享的 TypeScript 运行时（`domains/comet-classic/`）支撑。每个 `.mjs` 都是 esbuild 生成的单文件包，`yaml` 等依赖一并内联。
...
最体现这个设计的是**风险信号优先于口头流程**：你说"走 hotfix"，但如果命中了 `public_api_change`，运行时会暂停并请你确认是否继续这条会影响外部契约的轻量流程。这是 Comet 的安全底线：高风险近路始终需要你明确确认。
...
**0.4.0 的决策**：将 `/comet-any` 定位为一个完整的 **Skill 创作平台**。它通过组合任意 Skill，帮助用户创建具备稳定运行、分发和评审能力的 Skill。

**为什么要做成平台**：创建一个可复用、可分发、可评审的 Skill 包，需要比生成一段提示词更完整的工具链。一个能和 `/comet` 平起平坐的 Skill，需要：
...
所以 `/comet-any` 产出一个包含多类产物的**稳定组合 Skill Bundle**：
...
为什么两个都要？因为对工作流类 Skill——用户会反复跑的东西——**可靠性比能力更重要**。`pass@k` 高、`pass^k` 低意味着 Skill 偶尔能做对，但单次结果仍然难以预期。这个间隙（`pass@k − pass^k`）就是最关键的质量信号：高能力 + 低可靠性的 Skill，下一步应优先投入稳定性。
