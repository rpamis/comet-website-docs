...
`/comet` 会读取 `.comet/config.yaml`，并将用户请求转发到 `/comet-native` 或 `/comet-classic` Skill。默认 Native 会创建 change目录用于定义一个需求或功能，用 Shape → Build → Verify → Archive 管理需求、完整的实现目标、验收证据和归档，同时把实现方法交给强模型自主决定。
...
如果会话中断，重新打开项目后不需要再描述一遍任务背景，只需要输入：

```text
/comet 继续
```

Comet 会先解析项目默认工作流，再从对应 change 和运行证据继续你上次的剩下的工作。
