...
Classic 初始化才会安装或配置 OpenSpec、Superpowers、CodeGraph、阶段 rules 和 hooks。
...
`/comet` 不会猜测应该使用哪套工作流。它读取 `comet.config.yaml`，确定性转发到 `/comet-native` 或 `/comet-classic`，并完整保留你的原始请求。默认 Native 会创建自有 change，用 Shape → Build → Verify → Archive 管理需求、完整目标规格、证据和归档，同时把实现方法交给强模型自主决定。
