...
运行成功标识：运行 `comet --version`，能输出版本号，说明安装成功。
...
运行成功标识：项目根目录出现 `.comet/config.yaml`。也可以运行 `comet doctor`，输出里没有 `✗` 失败项就说明安装完整。
...
运行成功标识：Comet 回复已进入 Native 的 Shape 阶段并创建 change。也可以在终端运行 `comet status`，看到 `Native Changes` 下列出这个 change（查看方式见下方「查看状态」）。
