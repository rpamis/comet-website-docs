...
description: "使用 /comet-any 创建、评估、发布和分发一个可复用 Skill。"
---

<Tip>
  在 Agent 中调用 <code>/comet-any</code> 开始创建。流程会提示需要确认的方案、评估、发布和分发步骤。
</Tip>

`/comet-any`（Skill Creator）用于创建、优化和组合可复用 Skill。本页按使用顺序说明从目标描述到分发的操作。
...
- 将团队重复执行的流程整理为可复用 Skill。
- 在 Classic 五阶段基础上增加、替换或关闭环节。
- 为已有 Skill 补充评估、发布和分发产物。

## 你需要先准备什么

1. **已安装 Comet**。运行 `comet doctor` 确认 CLI 正常。如果还没装，见 [安装与更新](/zh/guides/install-and-update)。
...
调用 `/comet-any` 开始。流程中断后，运行 `comet creator next <name>` 查看建议动作。运行 `comet creator status <name>` 查看 readiness、blocker 和证据。
...
| **创建全新 workflow Skill** | `new-skill` | 从目标描述创建全新工作流 | 保存团队专属流程     |
| **整理已有 Skill** | `upgrade-existing` | 把现有 Skill 整理成可发布的标准包 | 把散乱的 Skill 规范化 |

不管选哪种起点，`/comet-any` 最终都会把目标编译成同一种 **Workflow Contract**（由 Workflow Node、Skill Binding、Required Skill Call、Output Schema、Guardrail、Handoff 组成）。详见[Skill Creator 概览](/zh/skill-creator/overview#workflow-contract-所有路径的共同模型)。
...
<Tip>
  候选缺失或存在多个来源时，<code>/comet-any</code> 会暂停并请求选择，不会自动忽略或决定来源。
</Tip>

## 第四步：评估

确认后，`/comet-any` 生成 Skill 包。发布前必须生成与当前草稿 hash 匹配的评估证据：
...
<Info>
  <strong>前提</strong>：运行 <code>comet eval</code> 需要本机有 <code>uv</code>、Python 3.11+、Docker，并设置了 <code>ANTHROPIC_API_KEY</code>（或 <code>ANTHROPIC_AUTH_TOKEN</code>）。完整的准备工作见<a href="/zh/eval/quickstart#你要准备什么">评估快速上手 · 你要准备什么</a>。
</Info>
...
<AccordionGroup>
  <Accordion title="/comet-any 和 /comet 有什么区别">
    ...
    做功能、修 bug 或修改项目代码时使用 <code>/comet</code>。将流程整理成可复用 Skill，或在 <code>/comet</code> 五阶段基础上增加团队规则时，使用 <code>/comet-any</code>。
  </Accordion>

  <Accordion title="我需要理解 Workflow Node、Output Schema 这些词吗">
    普通使用不要求理解这些术语。方案确认页会列出每一步的行为、调用的 Skill，以及新增或关闭的内容。
  ...
  <Accordion title="我不想自己跑 eval 和 publish 命令">
    可以全程使用 <code>/comet-any</code>，并在流程要求时确认。页面中的命令用于排障和自动化集成。
  </Accordion>
</AccordionGroup>
