...
- 想在经典 `/comet-classic` 五阶段基础上**增加、替换或关闭**某些环节。
...
| --- | --- | --- | --- |
| **基于 Comet 现有 Skill 的五阶段定制** | `customize-comet` | 在经典五阶段上做增量调整 | 想给 `/comet-classic` 加一个评审环节、要求用组件库 |
...
<Warning>
  选择<strong>基于 Comet 现有 Skill 的五阶段定制</strong>时，<code>/comet-classic</code> 被视为<strong>受保护边界</strong>：control 节点（<code>open</code>/<code>execute</code>/<code>verify</code>/<code>archive</code>）不能被替换实现，你只能对节点<strong>增加 Required Skill Call、增强（augment）、关闭（仅可选节点）</strong>。
</Warning>
...
<AccordionGroup>
  <Accordion title="/comet-any 和 /comet 有什么区别">
    ...
    想做功能、修 bug 或改项目代码，用 <code>/comet</code>。想把一套流程沉淀成可复用 Skill，或在 <code>/comet-classic</code> 五阶段基础上增加团队规则，用 <code>/comet-any</code>。
  </Accordion>
