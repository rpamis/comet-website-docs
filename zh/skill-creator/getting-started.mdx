...
<Note>
  <code>/comet-any</code> 是 <strong>用户唯一需要记住的入口</strong>。需要看恢复状态时用 <code>comet creator</code>；进入发布后再用 <code>comet publish</code>。普通用户不需要直接跑 <code>comet bundle</code>。
</Note>
...
<p align="center">`/comet-any` 负责创建 Skill，`comet eval` 给证据，`comet creator next` 把中断流程翻译成下一条用户命令</p>
