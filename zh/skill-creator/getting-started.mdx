...
```text
/comet-any 创建  →  comet eval 验证  →  comet publish status/next 看下一步  →  review/approve/run  →  distribute
```

你只需要调用 `/comet-any`。如果流程中断，优先运行 `comet publish next <name>` 看唯一推荐下一步；如果要看完整 readiness、blocker 和证据，再运行 `comet publish status <name>`。下面用一次真实创建走完整条链路。

<p align="center">
  <img src="/assets/skill-creator-getting-started-illustrations/01-skill-creator-mainline.png" alt="小鱼把 Skill 卡片从 /comet-any 方案确认带到 eval 和 publish" width="800" />
</p>

<p align="center">`/comet-any` 负责创建 Skill，`comet eval` 给证据，`comet publish next` 把中断流程翻译成下一条用户命令</p>
...
# 只想知道下一步该跑什么
comet publish next my-skill --project . --json
...
flowchart TD
    ...
    Eval --> Next["comet publish status / next"]
...
<AccordionGroup>
  ...
  <Accordion title="/comet-any 和 /comet 有什么区别">
    <code>/comet</code> 是经典 Spec 模式入口，驱动 open/design/build/verify/archive 五阶段流程。<code>/comet-any</code> 是 Skill Creator，用来<strong>创建、优化、组合可复用的 Skill</strong>。两者是不同入口，但 <code>/comet-any</code> 的"基于 Comet 现有 Skill 的五阶段定制"可以以 <code>/comet</code> 为基础做增量调整（在受保护边界内增加 Required Skill Call、替换 producer 节点、关闭可选节点）。
  </Accordion>
