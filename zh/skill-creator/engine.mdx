...
description: "理解 Comet Skill 的三类来源、Skill 包结构、Engine 运行循环、pending action/resume 机制、不可变快照、guardrails、runtime checks，以及 /comet-any 生成物和 Engine 的关系。"
---
...
| 类型 | 来源 | 常见入口 | 说明 |
| --- | --- | --- | --- |
| 内置工作流 Skill | 随包分发在 `assets/skills/` | `/comet`、`/comet-open`、`/comet-any` | 驱动五阶段流程和 Skill Creator |
...
│   ├── checks.yaml          # runtime checks（与 evals.yaml 二选一）
│   ├── evals.yaml           # runtime checks（与 checks.yaml 二选一）
...
| `comet/eval.yaml` | 创建期 | 创建期评估 manifest（benchmark、grader、review） | `comet eval` / Bundle 后端 |
...
<p align="center">
  <img src="/assets/skill-creator-engine-illustrations/01-engine-pending-action-loop.png" alt="小鱼记录 Engine 提议的 pending action，外部执行后再通过 resume 提交 outcome" width="800" />
</p>
...
<Warning>
  <code>comet skill check</code> 只检查某次 Engine Run 的完成度，不是通用 Skill 评估。评估一个 Skill 的产品能力能否发布，用 <a href="/zh/cli/eval">comet eval</a>。详见 <a href="/zh/eval/runtime">Runtime check</a>。
</Warning>
