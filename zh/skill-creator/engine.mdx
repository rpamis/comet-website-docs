...
<Tip>
  创建、优化或组合 Skill 时，先在 Agent 里调用 <code>/comet-any</code>{' '}
...
Comet 的 Skill 不只是 Markdown 提示词。
从 0.4.0-beta.1 开始，Comet 内置了 **Skill Engine**。
它是一个确定性运行时，负责执行约束、状态持久化、恢复和评估。
对于多步骤 Skill，Engine 提供稳定内容 hash、不可变快照、pending action 恢复、guardrails 和 runtime check。
...
- `SKILL.md` 是给人和 Agent 的入口说明。
- `comet/` 目录是运行时控制面，定义如何执行、恢复、检查和评估。
- `comet eval` 看发布前证据，`comet skill check` 看单次 Run 的运行期检查。两者解决的问题不同。
...
<Warning>
  项目 Skill 按名称覆盖内置 Skill。
  如果项目 Skill 无效，会直接失败，并禁止回退到内置版本。
  这样可以避免“自定义失效但系统静默降级”的问题。
</Warning>
...
- Engine 解析当前 step，构造动作，通过 guardrails，然后暂停等待结果。
- 成功后推进到 `next`。失败后增加重试计数，并停留在当前 step。
...
- 不能定义 `entry` 或 `steps`。
- Engine **不自己提议动作**。
- 动作由外部 Agent 提议，再通过 `acceptAdaptiveAction` 注入，同样要经过 guardrails。
...
Engine 的核心是 ReAct 风格循环，但它本身**不执行副作用**。
Engine 只负责决定、约束、记录和评估。
真正执行动作的是外部执行者（Agent、平台或人）。
...
为什么要暂停：Engine 是确定性状态机，它不直接调模型，也不直接写代码。
它会把“下一步该做什么”写入 pending action 并持久化到磁盘，等待外部执行后通过 `resume` 回传结果。
这也让 Run 具备**跨进程恢复**能力。
因为 pending action 在磁盘上，换进程后读取状态即可继续。
...
3. **创建不可变快照**。把整个 Skill 包冻结到 `.comet/skill-snapshots/<hash>/`，并将 hash 锁定到 Run 的 `skillHash`。
...
- `.comet.yaml` 保存用户可理解的工作流状态（phase、build_mode、verify_result 等），只通过 `run_id` 链接到 Engine Run。
...
<Note>
  Engine 内部的 <code>comet-classic</code> 定义是一个 deterministic Skill，有 28 个步骤覆盖
...
| 文件                                    | 内容                                                                      |
| --------------------------------------- | ------------------------------------------------------------------------- |
| `comet/runtime/classic/skill.yaml`      | deterministic 编排图（28 步，full/hotfix/tweak 三种 profile）             |
