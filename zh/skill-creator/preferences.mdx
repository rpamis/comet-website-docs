...
<Tip>
  创建、优化或组合 Skill 时，先在 Agent 里调用 <code>/comet-any</code> 并按提示确认。首次使用时它会自动推荐默认偏好。除非你要手写偏好或排查组合问题，不需要先关心本页的配置细节。
</Tip>
...
<Warning>
  顶层只允许这五个 key：<code>version</code>、<code>mode</code>、<code>prefer</code>、<code>require</code>、<code>policies</code>。任何未知字段会触发 <code>unknown-field</code> 警告。<code>version</code> 必须是 <code>1</code>。
</Warning>
