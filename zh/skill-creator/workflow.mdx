...
<Tip>
  ...
  评估、发布、分发和恢复会由流程自动引导。
  除非你在排障或做自动化集成，否则不需要先理解底层命令、内部状态和生成物结构。
  本页是进阶参考，覆盖完整流程。
  如果你只想快速跑通，先看[快速上手：组合任意 Skill](/zh/skill-creator/getting-started)。
</Tip>

本页列出 `/comet-any` 的阶段、状态和对应 CLI。
创建入口仍是 `/comet-any`。
其余命令主要用于查看状态、恢复流程或接入自动化。
...
/comet-any 创建
  ...
  → comet creator status / next（看 readiness 和唯一的下一步）
  → comet publish review / approve / run（发布）
  → comet publish distribute --preview / distribute（分发）
```

如果你只想完成一次 Skill 创建，优先按这条主线执行。
下方的 17 个阶段是完整参考，用于解释流程为什么会暂停、卡住时该看什么，以及下一步命令是什么。
...
<Note>
  本页会出现 draft hash、eval evidence、Bundle draft 等内部术语。它们用于解释恢复和发布门禁。日常使用时优先看 <code>comet creator next</code> 给出的唯一的下一步。
</Note>
...
`/comet-any` 默认会**先尝试恢复**已有流程。
只有你明确表示“重新开始”或“放弃旧状态”，它才会新建流程。

它会把恢复信息整理成**恢复摘要**，直接展示 `resumeSummary`、`Current step`、`Suggested user command` 和阻塞原因。
你不需要再手动翻 `.comet/bundle-authoring/`。
...
如果候选是 `missing` 或 `ambiguous`，`/comet-any` **必须暂停并询问你**。
它不能静默忽略缺失项，也不能在多个来源里替你做决定。

你选定来源后，`/comet-any` 会更新 Skill Creator metadata。
如果你明确同意忽略缺失偏好，系统会记录理由并写入评审摘要。
只要还有 unresolved Skill candidates，流程就不会继续生成。
...
`/comet-any` 会把目标编译成 **Workflow Contract**，再按偏好给出组合方案。
方案中会标注每个节点的职责、实现 Skill、Required Skill Call 和 Output Schema。
这一页就是“**Skill Creator 方案确认页**”。
...
- `reference/workflow-protocol.json` 。 **运行状态唯一可信来源**（节点、边、Output Schema、state、evals），runtime / eval / review / readiness 全读它
