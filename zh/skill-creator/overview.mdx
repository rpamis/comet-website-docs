...
| --- | --- | --- | --- |
| **基于 Comet 现有 Skill 的五阶段定制** | `customize-comet` | 在 `/comet-classic` 受保护边界内做增量调整 | 给 `/comet-classic` 增加 `grill-me` 需求澄清 Skill |
...
<Columns columns={2}>
  <Column>
    ...
    以 Comet 经典五阶段为骨架做增量调整。保留 `/comet-classic` 主流程和 `.comet.yaml` 状态语义，内置 8 个节点。
  </Column>
  <Column>
    #### `workflow-kernel`
...
当你选择"基于 Comet 现有 Skill 的五阶段定制"时，`/comet-classic` 被视为**受保护边界**。`comet-five-phase-overlay` 内置这 8 个节点：
...
## 方案示例：给 /comet-classic 增加 grill-me 需求澄清

下面这个 `plan.json` 用 Required Skill Call 在 `open` 节点强制调用 `grill-me`，让 `/comet-classic` 在进入设计前先把需求问清楚——**不替换任何节点实现**：
...
- 想在 `/comet-classic` 五阶段基础上**增加、替换或关闭**某些环节。
