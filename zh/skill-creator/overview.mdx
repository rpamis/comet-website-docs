...
description: "深入理解 /comet-any（Skill Creator）如何用 Workflow Contract 把任意 Skill 编排成可验证、可评审、可分发的 Comet-native Skill，三种起点、受保护边界、Workflow Node 和发布门禁。"
---

<Info>
  这是进阶内容。如果你只想快速创建一个 Skill，先看[快速上手：组合任意 Skill](/zh/skill-creator/getting-started)。
</Info>

`/comet-any`（也叫 **Skill Creator**）是 Comet 的 Skill Creator。你描述想要的工作流，它读取真实本地 Skill、把目标编译成一份 **Workflow Contract**，再生成**可验证、可评审、可分发**的 Skill Bundle，并把后端复杂度（Bundle、Factory、composition、resolved-skills）隐藏在内部。
...
**用户唯一需要记住的入口是 `/comet-any`**。需要恢复创建状态时用 `comet creator`；进入发布后用 `comet publish`；`comet bundle` 只作为高级后端保留给审计和排障。
...
/comet-any 创建
  ...
  → comet creator status / next（readiness 和唯一下一步）
  → comet publish review / approve / run（发布）
  → comet publish distribute --preview（强制预览）
  → comet publish distribute（真实分发）
```

`comet creator` 是 `/comet-any` 的恢复和创建状态表面；`comet bundle` 是高级 Bundle 后端；`comet skill` 是底层 Skill 工具，适合本地调试和 Engine Run。它们都**不是**普通用户创建 Skill 的主入口。
