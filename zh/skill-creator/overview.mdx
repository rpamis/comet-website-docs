...
<Tip>
  ...
  本页是进阶内容。若你只想快速创建一个 Skill，先看[快速上手：组合任意 Skill](/zh/skill-creator/getting-started)。
</Tip>

`/comet-any` 是 Comet 的 Skill 创建入口，也叫 **Skill Creator**。
你给出目标工作流后，它会读取本地真实 Skill，把需求编译成 **Workflow Contract**，再产出**可验证、可评审、可分发**的 Skill Bundle。
Bundle、Factory、composition、resolved-skills 这些后端复杂度会被收敛在内部。
...
`comet creator` 用于查看和恢复 `/comet-any` 创建流程。
`comet bundle` 是高级后端工具。
`comet skill` 偏底层，常用于本地调试和 Engine Run。
它们都**不是**日常创建 Skill 的主入口。

## 三种起点

在用户层，你只需要先选三种起点之一。
它们对应三种 **Skill Creator 意图**（`skillCreatorIntent`）。
后端会把三条路径统一编译为同一种 Workflow Contract：
...
无论选择哪种起点，`/comet-any` 最终都会把目标编译成同一种 **Workflow Contract**。
这是理解 Skill Creator 的核心概念：
...
`/comet-any` 内置 8 个 Output Schema。
runtime、eval、readiness 都按 Schema 判断节点是否达标。
Skill 名字只用于识别，不作为达标依据。
...
关键点是：`open` 的实现 Skill 没变。
变化只发生在节点内部，系统会**强制调用**额外 Skill，并通过 Output Schema 回收澄清 evidence。
