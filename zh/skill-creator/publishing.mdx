...
本页是 Skill 发布状态机、readiness 与阻塞码的权威说明。命令级参考见 [comet publish](/zh/cli/publish)。
...
还有一个异常状态 `drift-conflict`：处于 `ready` 状态的 Bundle，如果 draft hash **和** ready hash 都发生变化，会进入冲突状态，并带上 `conflict: {draftHash, readyHash}`。只有其中一个变化则直接回退到 `draft`。
