...
| `eval-passed` | 有当前 hash 的 eval 证据 | `comet eval run --html` 通过且 hash 匹配 |
...
<p align="center">
  <img src="/assets/skill-creator-publishing-illustrations/01-evidence-to-distribute-preview.png" alt="小鱼把 eval evidence 经过 review、approval、publish 和 distribute preview 逐步盖章放行" width="800" />
</p>

<p align="center">发布前先绑定 eval evidence，再评审、批准、发布，并强制做分发预览</p>
...
| 你想做什么 | 用哪个命令 | 输出重点 |
| --- | --- | --- |
| 看完整 readiness、blocker、warning 和 evidence | `comet publish status <name>` | 完整状态和恢复上下文 |
| 从中断流程恢复，只想知道现在跑什么 | `comet publish next <name>` | 单个推荐动作、命令、原因和是否需要用户确认 |

`comet publish next` 不暴露后端 Bundle 命令。它会把后端 next action 翻译成普通用户命令，例如 `comet publish review ...`、`comet publish approve ...`，或提示你回到 `/comet-any` 解决候选、确认方案、重新生成。
...
| 阻塞码 | 含义 | 恢复建议 |
| --- | --- | --- |
| `[candidate]` | 还有未解决的缺失/歧义候选 | 回到 `/comet-any` 选择来源，或让 `comet publish next` 给出恢复命令 |
...
| `[eval]` | 缺少/失败/对应旧 hash 的 Eval 证据 | 重新跑 `comet eval run --html` |
| `[publish]` | status `ready` 但没有 ready metadata | 重新走 review 流程 |

<Note>
  每个 blocker 都带 <code>nextAction: { label, command }</code>，直接给出恢复建议。想看完整上下文用 <code>comet publish status</code>；只想复制下一条用户命令时，用 <code>comet publish next</code>。
</Note>
...
```json
{
  ...
  "level": "quick",
  "bundleHash": "<64 位 hex SHA-256>",
  "entries": [
    { "id": "<entry skill id>", "passed": true, "passRate": 1.0, "evidence": ["..."] }
  ],
...
<Warning>
  <code>bundleHash</code> 必须是 64 位 hex SHA-256 且等于当前 <code>currentHash</code>。<code>full</code> 级别必须提供 <code>variance</code>。每个 entry Skill 必须恰好出现一次。普通用户不需要手写这个文件——<code>/comet-any</code> 会通过 <code>comet eval</code> 自动产出。
</Warning>
