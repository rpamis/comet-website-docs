...
1. **先复现、后修复**：改代码前必须先复现 bug 并记录失败证据；可以自动化时先添加一个能失败的回归测试。没有复现证据不得修改代码；无法自动化复现时，把手动复现步骤和结果记入 proposal 或验证报告。
2. 读 tasks.md 未完成任务
...
它**一次性**设 `workflow: full`、`classic_profile: full`、`phase` 回退到 `design`、清空 `design_doc`，然后加载 `/comet-design` 补 Design Doc。已完成的代码、tasks 和 OpenSpec 产物都保留——你只补一个 Design Doc，不是从头来。`preset-escalate` 只能从 `phase: build` 的 hotfix/tweak 触发，其他情况会报错。
