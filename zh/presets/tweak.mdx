---
title: 'tweak 预设'
description: 'tweak 是串联 OpenSpec 的轻量预设：open→build→verify→archive，跳过 brainstorming 和完整 plan，通过 OpenSpec apply 构建，delta spec 是正式产物。适合配置调整、文档/prompt 优化、spec 驱动的中等变更。'
---

tweak 是 Comet 串联 OpenSpec 的轻量预设。它走 `open → build → verify → archive` 四步，跳过 brainstorming 和完整 plan，但把 **delta spec 当作正式产物**——并通过 OpenSpec 原生的 apply action 构建。
