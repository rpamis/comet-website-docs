...
<Tip>
  <strong>正常情况下你只需要 <code>/comet-classic</code>。</strong>当你描述的是<strong>可收敛为单一 OpenSpec change 的轻量/中等变更</strong>（需通过 OpenSpec apply 执行、不需要完整深度设计/plan）时，<code>/comet-classic</code> 的<strong>意图识别会优先命中 tweak 预设</strong>，自动调用 <code>/comet-tweak</code>。只有想手动控制时才直接输入 <code>/comet-tweak</code>。详见<a href="#怎么触发">怎么触发</a>。
</Tip>

## 怎么触发

tweak 通常**不是你手敲的命令**。`/comet-classic` 的 Step 0 会**优先做预设检测**（优先级高于列活跃 change）：
...
你也可以直接输入 `/comet-tweak`。中断后用 `/comet-classic` 恢复时，`/comet-classic` 会读 `.comet.yaml` 的 `workflow` 字段，在 `phase: build` 时自动路由回 `/comet-tweak`。
...
open guard 对 tweak **不要求** `design.md`（只对 full 要求），所以 open 完成后直接跳到 build，**跳过 design 阶段**。`init` 写入的默认值和 hotfix 完全一样（`build_mode: direct`、`tdd_mode: direct`、`review_mode: off`、`isolation: branch`、`verify_mode: light`），详见 [hotfix 预设 · 初始化的默认值](/zh/presets/hotfix#hotfix-初始化的默认值)。
...
<Warning>
  这条 apply 路径<strong>只属于 tweak</strong>。完整 <code>/comet-classic</code> 或 <code>workflow: full</code> <strong>不得套用</strong> tweak 的 <code>openspec-apply-change</code> 构建路径——full 仍必须先通过 <code>/comet-design</code> 生成 Design Doc，再由 <code>/comet-build</code> 走 Superpowers 计划和执行。
</Warning>
...
tweak 幂等。中断后用 `/comet-classic` 恢复——它读 `.comet.yaml` 的 `workflow` 字段，在 `phase: build` 时自动路由回 `/comet-tweak`。`comet-state check <name> build --recover` 给出恢复上下文，从 tasks.md 第一个未勾选任务继续。
