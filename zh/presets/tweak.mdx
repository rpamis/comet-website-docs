...
| `openspec-apply-change`  | OpenSpec apply 构建，逐任务执行与勾选 | Step 2，tweak 专属执行方式                           |
| `systematic-debugging`   | 异常调试                              | 执行中出现崩溃、测试失败或构建失败时强制加载         |
| `openspec-verify-change` | delta spec 与实现一致性验证           | Step 3，有 delta spec 时强制 `verify_mode: full`     |

- `brainstorming` 和 `writing-plans` 同样跳过：tweak 不做技术设计和实施计划，直接按 delta spec 应用变更。
