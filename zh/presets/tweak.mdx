...
和 hotfix 一样的三层分工，但**文件数阈值更高**（超过 6 个文件，因为 tweak 本身可能触及更多文件）：
...
<Note>
  注意 tweak 的第二个信号是“需要拆分为多个 OpenSpec change”，而 hotfix 是“需要新增
...
它**一次性**设 `workflow: full`、`classic_profile: full`、`phase` 回退到 `design`、清空 `design_doc`，然后加载 `/comet-design` 补 Design Doc。已完成的代码、tasks 和 OpenSpec artifacts 都保留——你只补一个 Design Doc。`preset-escalate` 只能从 `phase: build` 的 hotfix/tweak 触发。
...
（tweak 始终用 OpenSpec apply 执行，任务数量不改变执行方式，也不会因此停下来确认。）
