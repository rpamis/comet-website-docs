...
<Tip>
  <strong>正常情况下你只需要 <code>/comet</code>。</strong>当你描述的是文案/配置/文档/prompt 小调整时，<code>/comet</code> 的<strong>意图识别会优先命中 tweak 预设</strong>，自动调用 <code>/comet-tweak</code>。只有想手动控制时才直接输入 <code>/comet-tweak</code>。详见<a href="#怎么触发">怎么触发</a>。
</Tip>
...
| 你的描述 | 路由 |
| --- | --- |
| 明确描述为文案/配置/文档/prompt 小调整 + 满足 tweak 条件 | 自动 `/comet-tweak` |
| 明确描述为 bug fix / 热修复 | 自动 `/comet-hotfix`（见 [hotfix 预设](/zh/presets/hotfix)） |
