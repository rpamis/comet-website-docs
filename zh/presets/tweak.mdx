...
<p align="center">
  <img src="/assets/tweak-preset-illustrations/01-tweak-tuning-board.png" alt="小鱼拿着 delta spec 在 tweak 调音台上做单点调整，连接 apply、full verify，并在范围变大时看到升级信号" width="800" />
</p>
...
<Tip>
  <strong>正常情况下你只需要 <code>/comet</code>。</strong>当你描述的是<strong>可收敛为单一 OpenSpec change 的轻量/中等变更</strong>（需通过 OpenSpec apply 执行、不需要完整深度设计/plan）时，<code>/comet</code> 的<strong>意图识别会优先命中 tweak 预设</strong>，自动调用 <code>/comet-tweak</code>。只有想手动控制时才直接输入 <code>/comet-tweak</code>。详见<a href="#怎么触发">怎么触发</a>。
</Tip>

## 怎么触发

tweak 通常**不是你手敲的命令**。`/comet` 的 Step 0 会**优先做预设检测**（优先级高于列活跃 change）：
...
你也可以直接输入 `/comet-tweak`。中断后用 `/comet` 恢复时，`/comet` 会读 `.comet.yaml` 的 `workflow` 字段，在 `phase: build` 时自动路由回 `/comet-tweak`。
...
<Note>
  <strong>关键区别是 delta spec 的地位</strong>：hotfix 把 delta spec 当例外，tweak 把它当正常产物——<strong>需要 delta spec 本身不构成升级理由</strong>。如果你的变更天然需要修改 spec，tweak 是正确选择。另一个关键区别是 build 方式：tweak 用 OpenSpec 原生 apply 路径（只属于 tweak，full 不得套用）。
</Note>
...
flowchart TD
    ...
    CreateDelta --> S2
    S2 --> Esc{"命中升级信号?"}
    Esc -->|是| EscStop["停顿点 10：升级判定<br/>继续 tweak / 升级 full"]
    ...
    Auto --> S2
    VfStop -->|修复/继续| S2
    Vf -->|是| S4["Step 4 归档<br/>comet-archive（归档确认 + 分支处理）"]
    S4 --> Done["完成"]
```
...
open guard 对 tweak **不要求** `design.md`（只对 full 要求），所以 open 完成后直接跳到 build，**跳过 design 阶段**。`init` 写入的默认值和 hotfix 完全一样（`build_mode: direct`、`tdd_mode: direct`、`review_mode: off`、`isolation: current`、`verify_mode: light`），详见 [hotfix 预设 · 初始化的默认值](/zh/presets/hotfix#hotfix-初始化的默认值)。
...
<Warning>
  这条 apply 路径<strong>只属于 tweak</strong>。完整 <code>/comet</code> 或 <code>workflow: full</code> <strong>不得套用</strong> tweak 的 <code>openspec-apply-change</code> 构建路径——full 仍必须先通过 <code>/comet-design</code> 生成 Design Doc，再由 <code>/comet-build</code> 走 Superpowers 计划和执行。
</Warning>
...
它**原子地**设 `workflow: full`、`classic_profile: full`、`phase` 回退到 `design`、清空 `design_doc`，并清除预设专属的轻量执行设置，然后加载 `/comet-design` 补 Design Doc。已完成的代码、tasks 和 OpenSpec artifacts 都保留——你只补一个 Design Doc。升级到 full 后会在 Design Doc 之后的联合决策里重新选择完整工作流配置。`preset-escalate` 只能从 `phase: build` 的 hotfix/tweak 触发。

## 连续执行和必停点

tweak 默认**一次性连续执行**——调用后自动推进，不主动停顿。但无论 `auto_transition` 取何值，以下真正的用户决策**必须暂停**等你确认：
...
`auto_transition: false` 时，每个阶段边界结束当前调用并交还控制权（打印 HINT），由你稍后运行下一阶段——这是手动交接，不是新的确认点。
...
tweak 幂等。中断后用 `/comet` 恢复——它读 `.comet.yaml` 的 `workflow` 字段，在 `phase: build` 时自动路由回 `/comet-tweak`。`comet state check <name> build --recover` 给出恢复上下文，从 tasks.md 第一个未勾选任务继续。
