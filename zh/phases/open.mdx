...
open 阶段把你脑子里的想法变成一个可执行的 change：探索需求、澄清范围、创建 proposal/design/tasks，并初始化 Comet 状态。从你的角度看，这是一段**引导式的对话**——Comet 会反复问你问题，确认理解对了，才动手写文档。
...
`/comet-open` 通常**不是你手敲的命令**，而是 `/comet` 自动路由到的结果。
...
所以你只需要输入 `/comet` 并描述想做什么，它会自己判断该不该开新 change。open 阶段完成后，`/comet` 还会自动衔接下一个阶段（design）。详见[自动推进机制](/zh/concepts/auto-transition)。
...
flowchart TD
    ...
    S1a -->|是| Stop1a["停顿点 1a：PRD 拆分预检<br/>三选一"]
    S1a -->|否| S1sum["生成澄清摘要<br/>5 个必备部分"]
    Stop1a --> S1sum
    S1sum --> Stop1b["停顿点 1b：确认需求澄清完成"]
    Stop1b --> Stop1c["停顿点 1c：确认 change 名称"]
    Stop1c --> S2["加载 openspec-new-change<br/>逐个生成 proposal → design → tasks"]
    S2 --> S3["入口状态验证 + 内容完整性检查"]
    S3 --> Stop5["停顿点 5：审视三个文档并确认"]
...
<Tip>
  拆分后 Comet <strong>不会</strong>自动推进任何单个 change 到 design。它会暂停问你想先做哪一个，只推进你选的那个，其余保持活跃等以后 <code>/comet</code> 恢复。
</Tip>
...
<Warning>
  如果 <code>openspec instructions</code> 失败、返回无效 JSON、或没提供可用的 <code>resolvedOutputPath</code>，Comet 会<strong>立即停止</strong>并报告 OpenSpec 错误，<strong>不会</strong>回退成硬编码文档结构——那会绕过项目规则。change 名必须是你确认过的 kebab-case 名，Comet 不会自作主张扩大或缩小范围。
</Warning>
...
| 停顿点 | 何时出现 | 你要做什么 |
| --- | --- | --- |
| 1a PRD 拆分 | 输入是大 PRD 或多个独立能力 | 拆分 / 不拆 / 调整方案 |
| 1b 需求澄清完成 | 澄清摘要生成后 | 确认理解对了 |
| 1c change 名称 | 创建 change 前 | 选名 / 自定义名 |
| 5 文档审视 | 三文档生成后 | 确认 / 要调整 |
