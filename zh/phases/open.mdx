...
<Tip>
  <strong>正常情况下你只需要 <code>/comet-classic</code>。</strong><code>/comet-classic</code> 自带意图识别，会读取文件状态自动判断当前在 open 阶段并调用 <code>/comet-open</code>。本文描述的 <code>/comet-open</code> 是它内部调用的阶段 Skill——只有当你想<strong>手动控制</strong>（例如关闭了 <code>auto_transition</code>、或想只跑这一个阶段）时，才需要直接输入 <code>/comet-open</code>。详见<a href="#本阶段如何触发">本阶段如何触发</a>。
</Tip>

## 本阶段如何触发

`/comet-open` 通常**不是你手敲的命令**，而是 `/comet-classic` 自动路由到的结果。`/comet-classic` 会先读取 active change 列表，整理路由上下文，再由运行时评分决定 route。只有 route 是 `full`，或着你明确选择完整流程时，才进入 `/comet-open`。

### 默认：用 /comet-classic 自动识别
...
你只需要输入 `/comet-classic` 并描述想做什么，它会自己判断该不该开新 change。open 阶段完成后，`/comet-classic` 还会自动衔接下一个阶段（design）。详见[自动推进机制](/zh/concepts/auto-transition)。
...
<Tip>
  拆分后 Comet <strong>不会</strong>自动推进任何单个 change 到 design。它会暂停问你想先做哪一个，只推进你选的那个，其余保持活跃等以后 <code>/comet-classic</code> 恢复。
</Tip>
...
open 阶段所有操作都可以安全重复执行。如果 `.comet.yaml` 已经在 `phase: open` 且三个产物都已存在，Comet 会**跳过已完成步骤**，从第一个缺失的步骤继续。这让你在中断后重新 `/comet-classic` 也不会搞乱状态。
...
<AccordionGroup>
  <Accordion title=".comet.yaml 缺失怎么办">
    不要用 `/opsx:new` 绕过——它只创建 OpenSpec artifacts，不会创建 `.comet.yaml`，change 会落在 Comet 状态机之外。重新运行 `/comet-classic`，让它通过 `/comet-open` 补齐状态。
  </Accordion>
