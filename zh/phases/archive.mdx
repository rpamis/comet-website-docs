...
archive 阶段不加载任何 Skill。状态检查、归档和远端交付全部由 `comet state`、`comet guard`、`comet archive` 命令和 git 操作完成；`comet archive` 内部调用 OpenSpec archive 完成 delta 到主 spec 的合并。
...
| 选项                            | 结果                                                                                              |
| ------------------------------- | ------------------------------------------------------------------------------------------------- |
| **仅本地归档（不推送）**        | 归档、创建唯一归档提交并保留在当前绑定分支，不推送、不创建 PR                                     |
...
`archive_confirmation` 由 Comet 自动维护（machine-owned），不能手工编辑。
...
- [状态管理](/zh/concepts/state-management)
