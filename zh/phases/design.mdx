...
design 阶段做**深度技术设计**。它先用脚本把 open 阶段的 OpenSpec 产物打包成交接包，再交给 Superpowers `brainstorming`，带你做多轮方案探讨，最后产出 Design Doc。你会经历一段**真正的头脑风暴**。Comet 会反复和你探讨实现方案、权衡、风险，确认方案后才会写文档。

<Tip>
  正常使用时只要运行 <code>/comet</code>。
  配置选择 Classic 后，内部 <code>/comet-classic</code> 会读取状态，并在 open 完成后自动调用{' '}
  <code>/comet-design</code>。
  本文介绍的是内部阶段 Skill。只有你想<strong>手动控制</strong>
  阶段时，才需要直接输入。详见<a href="#本阶段如何触发">本阶段如何触发</a>。
</Tip>
