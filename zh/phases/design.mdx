...
<Tip>
  <strong>正常情况下你只需要 <code>/comet-classic</code>。</strong>open 阶段完成后，<code>/comet-classic</code> 会自动衔接调用 <code>/comet-design</code>。本文描述的 <code>/comet-design</code> 是它内部调用的阶段 Skill——只有当你想<strong>手动控制</strong>（例如关闭了 <code>auto_transition</code>、或想只跑这一个阶段）时，才需要直接输入 <code>/comet-design</code>。详见<a href="#本阶段如何触发">本阶段如何触发</a>。
</Tip>
