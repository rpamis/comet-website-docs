---
title: "build 阶段"
description: "build 阶段写实施计划，让你选择执行、TDD 和审查方式，然后把设计变成代码——逐任务执行、审查、提交。"
---

build 阶段把设计变成代码。Classic 会在 Open 阶段先准备并绑定工作区；进入 build 后，它先写实施计划，再让你在停顿点选择执行、TDD 和审查方式，然后逐任务执行、审查、提交。从你的角度看，这是**动手实现**的阶段——但 Comet 会把执行方式的选择权交给你，并在过程中处理 spec 增量更新和异常调试。
...
flowchart TD
    ...
    Stop2 -->|继续| Stop3["停顿点 7：一次性选定<br/>执行 + TDD + 审查"]
...
| 选项               | 行为                                                                                                                |
| ------------------ | ------------------------------------------------------------------------------------------------------------------- |
| **A 继续执行**     | 保持在当前模型，进入 Step 3 选择执行、TDD 和审查模式                                                                |
...
确认继续后，Comet **一次性问你三个执行选择**（不是分三次问）：下面先说明已在 Open 阶段确定的工作区绑定，再列出这三个执行选择。

#### 工作区绑定（isolation）

Classic 的新 change 已在 Open 阶段选择并准备工作区。下面的值用于说明 change 的工作区绑定；恢复或检查时，Comet 会验证当前目录和分支是否匹配，不会在 build 阶段静默换目录。
...
创建时选择 `branch`，Comet 会在 Open 阶段让你确认或覆盖分支名，命名规则：`feature/YYYYMMDD/<name>`（full）、`hotfix/...`（hotfix）、`tweak/...`（tweak）。三种隔离方式都适用于 full、hotfix 和 tweak；选择结果会写入 `isolation`，并把实际执行目录的当前分支记录为 `bound_branch`，后续入口检查会阻止意外切换分支。
...
<Note>
  ...
  ——只有主会话在双阶段验收通过后才勾选。审查者拿到的是完整任务 + 实现
