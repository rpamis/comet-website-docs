...
| **`standard`** | 默认不逐任务派审查者。命中风险信号时才派。最终综合审查在 verify 阶段 | **默认推荐**，大多数普通改动   |
...
| 审查     | 强度随 `review_mode` 调整。standard 不做 build 内整变更审查；thorough 每 3 个任务做分段审查。最终综合审查在 verify | 按 review_mode 派任务级审查者 + 修复 agent（thorough=每任务；standard=仅风险任务） |
