...
<Tip>
  <strong>正常情况下你只需要 <code>/comet-classic</code>。</strong>design 阶段完成后，<code>/comet-classic</code> 会自动衔接调用 <code>/comet-build</code>。本文描述的 <code>/comet-build</code> 是它内部调用的阶段 Skill——只有当你想<strong>手动控制</strong>（例如关闭了 <code>auto_transition</code>、或想只跑这一个阶段）时，才需要直接输入 <code>/comet-build</code>。详见<a href="#本阶段如何触发">本阶段如何触发</a>。
</Tip>

## 本阶段如何触发

`/comet-build` 通常**不是你手敲的命令**，而是 `/comet-classic` 自动衔接的结果。
...
| **B 暂停切换模型** | 记录 `build_pause: plan-ready`，本次 `/comet-build` 停止；你换模型后可从 `/comet-classic` 或 `/comet-build` 恢复 |
