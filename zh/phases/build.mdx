...
| `direct` | *(none — bypass)* | 不使用 plan skill 的 single-agent direct execution；**仅作为 hotfix/tweak 预设的默认值**。Full-workflow changes 需要显式 `direct_override: true` flag，否则 guard 会阻止 transition |

<Note>
**Full workflow 中的 `direct` mode**：若要在 hotfix 或 tweak 预设之外使用 `direct` mode，你必须明确选择。AI 会在 `.comet.yaml` 中记录 `direct_override: true`；没有它，guard 和 build→verify state transition 都会阻塞。这是有意设计：full-workflow changes 应该通过合适的 plan execution skill。
</Note>
