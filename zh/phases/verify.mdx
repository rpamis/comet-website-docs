...
flowchart TD
    ...
    Dirty -->|实现/测试/tasks| Fail1b["报失败，进停顿点 1b"]
    Dirty -->|仅验证产物| Cont["继续"]
    Dirty -->|干净| Cont
    Cont --> Check["Step 2：按 verify_mode 跑检查<br/>light 6 项 / full 7 项"]
    Check --> Pass{"全部通过?"}
    Pass -->|否| Stop1b["停顿点 1：验证失败决策<br/>全部修复 / 逐项处理"]
    Pass -->|是| Drift{"full 检查 6：spec 漂移?"}
    Stop1b -->|verify-fail| Build["回 build 修复"]
    Stop1b -->|逐项处理| Check
    Drift -->|是| Stop2b["停顿点 2：spec 漂移决策<br/>A / B / C"]
    Drift -->|否| Stop3["停顿点 3：分支收尾<br/>四选一"]
    Stop2b -->|B| Build
    Stop2b -->|A/C| Stop3
...
<Note>
  如果你 build 时每个任务都提交了，工作区 diff 可能<strong>低估</strong>改动规模。Comet 会读 plan 头的 <code>base-ref</code> 用提交区间复核。你也可以随时手动覆盖：<code>node "$COMET_STATE" set <change-name> verify_mode full</code>
</Note>
...
| 情况 | 处理 |
| --- | --- |
| 改动属于当前 change 且触及实现/测试/tasks/delta spec/design doc | **不在 verify 修**，报失败进停顿点 1b |
| 改动只是验证产物（报告草稿、分支处理笔记） | 可继续 |
| 有实现但 tasks.md 没勾选 | 当"build 状态滞后"，报失败进停顿点 1b |
...
| 4 | 能力 spec 场景全部通过 |
| 5 | proposal.md 目标满足 |
| 6 | delta spec 与 Design Doc 无矛盾（**漂移检测，触发停顿点 2**） |
| 7 | 关联 Design Doc 可定位 |
...
| `brainstorming` | spec 漂移修复 | 停顿点 2 选 B 时 |
...
| 停顿点 | 何时出现 | 你要做什么 |
| --- | --- | --- |
| 1 验证失败 | 任何检查失败 | 全部修复回 build / 逐项处理（CRITICAL 必修） |
| 2 spec 漂移 | full 检查 6 发现矛盾 | A 追加说明 / B 回 build 重设计 / C 接受偏差 |
| 3 分支收尾 | 验证通过后 | 合并 / PR / 保持 / 丢弃 |
