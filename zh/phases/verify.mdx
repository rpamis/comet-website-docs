...
| Skill                            | 用途                 | 何时调用                                                         |
| -------------------------------- | -------------------- | ---------------------------------------------------------------- |
| `verification-before-completion` | 完成声明前的证据核验 | light/full 分支判断之前加载一次，不可跳过                        |
| `requesting-code-review`         | 轻量代码审查         | light 验证的审查项，`review_mode: standard`/`thorough` 时执行一次 |
| `openspec-verify-change`         | OpenSpec 规格覆盖检查 | full 验证模式，检查 proposal、design 和验收场景的规格覆盖        |

spec 漂移需要重新对齐时，verify 不直接加载 `brainstorming`：它通过 `verify-fail` 回到 Build，由 build 阶段加载。
