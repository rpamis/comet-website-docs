...
flowchart LR
  ...
  K --> M["--suite sync to LangSmith / Langfuse"]
```
...
| Composite Skill artifact form | Just a single `SKILL.md` file — can't be composed, can't carry dependencies, can't be distributed | `/comet-any` generates a stable, composable **Skill Bundle** — rules, hooks, and Engine metadata can be assembled; it's **reviewable** and **distributable** (landed on 35 AI coding platforms via `comet publish distribute`, just like `comet init`) |
| Skill quality verification | Relies on feel — if the Agent runs it once without errors, it passes                              | `comet eval` runs **real tasks** in an isolated environment, **quantifies** capability and reliability with multi-dimensional scores and `pass@k` / `pass^k`, and automatically **attributes** failures to what should change — every iteration is compared against baseline data, so you can see at a glance whether an improvement worked; Skill evolution runs on evidence, not gut feeling |
