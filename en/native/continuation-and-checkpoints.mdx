...
| `blocked`    | The normal loop pauses and enters recovery                  | Resolve the finding, then read the new continuation       |
...
`blocked` is not a terminal task result. State, evidence, or conflict findings that the Agent can repair enter recovery handling. After the repair, the Skill rereads the phase, formal artifacts, and continuation before resuming the Build → Verify loop. It changes to `await-user` only when the Runtime requires a user decision.
