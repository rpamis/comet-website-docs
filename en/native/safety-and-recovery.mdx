...
- the second recurrence of the same gap warns, and the third recurrence without progress enters the `repair-stagnation-stop` recovery branch;
...
- one no-progress override is available and is bound to the current signature; while it remains, the Agent must form a concrete new repair hypothesis;
- the default limit is five Verify failures per contract and can be changed to a positive integer with `native.max_verify_failures`. A user decision is required only after the override or verification budget is exhausted.

Repeated-gap recovery and the total failure budget apply independently. Only a user-confirmed contract change starts a new goal cycle; recovery conditions do not mechanically count phase transitions.
