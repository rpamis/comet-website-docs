...
Changing any input marks old evidence stale and blocks Archive. `comet native check` performs only a built-in, bounded, read-only text-hygiene scan. It invokes no Git, shell, test command, or external Skill and does not replace project tests.
...
For individual commands and exit semantics, see [Native CLI](/en/cli/native).
