...
The authoritative status for cross-session and cross-device recovery is `comet-state.yaml` in the change directory. `.comet/runtime/native/changes/<change-name>/state.json` only records the execution status of this machine.

The Runtime will select the recovery behavior based on the scene:

|"On-site|Restore the result|
| --- | --- |
|Local machine status matching|Continue from the current stable boundary|
|The status of this machine is missing, invalid or the version is outdated|Rebuild the execution status of this machine based on YAML|
|The binding between branch or worktree and change is inconsistent|Return to `await-user` and keep the existing file|
|The repeatable check was interrupted|Mark it as an interruption and re-execute according to the latest continuation|
|The non-repeatable check was interrupted|Return to Build and request a new implementation candidate|
|The Verifier task is lost|Retain the candidates and start a new attempt from the stable Verify boundary|
|`archive-ready` has lost the local execution basis|Return to Verify and conduct a full acceptance of the synchronized implementation again|
|`verification.md` is missing or lagging behind|According to the YAML reconstruction report of the same version|

The recovery operation itself is not counted towards the number of acceptance failures or suspensions. The new device cannot continue the processes, Agent sessions and log connections on the old device, but can only restore the facts that have been written to the stable state.
