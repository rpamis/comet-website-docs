...
A Verify failure returns to Build. The Runtime builds a semantic signature from the contract, failed or missing Acceptance IDs, and failed check IDs:

- the second recurrence of the same gap warns; on the third recurrence without progress, an available override requires the Agent to provide a concrete new repair hypothesis;
- only fewer gaps, passing checks, or missing evidence restored as current valid evidence count as progress; an ordinary implementation-scope change does not reset the counters;
- one no-progress override is available, is bound to the current signature, and remains Agent-owned rather than being delegated to the user;
- the default limit is five Verify failures per contract and can be changed to a positive integer with `native.max_verify_failures`.

Repeated-gap stopping and the total failure budget apply independently. After the override is used or the total failure budget is exhausted, Runtime returns one user decision: increase the budget, change the confirmed contract, or stop. Only a user-confirmed contract change starts a new goal cycle; stop conditions do not mechanically count phase transitions.
...
For parallel requirements, see [Multiple changes and specification conflicts](/en/native/multi-change-and-conflicts). For interruptions, cross-device discovery, or transaction problems, see [Recovery and troubleshooting](/en/native/recovery-playbook).
