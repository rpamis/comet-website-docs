...
Native organizes user-visible results as a decision tree. Questions that meet all three conditions form the current frontier, also called the currently answerable set. Questions that still depend on another decision or investigation remain for a later round. After each answer, Native recomputes the frontier from the tree instead of reusing the previous question list.

When a candidate branch needs an environment fact, investigating it remains the Agent's responsibility. If the host supports sub-agents or other parallel work, Native starts independent fact investigations in parallel. A fact under investigation defers only its downstream questions; other frontier questions remain in the current round. Without parallel support, the current Agent investigates directly and never delegates a discoverable fact to the user. This reduces round trips without asking you to guess before the necessary context exists.
