...
The final confirmation will be saved as an independent blocking item:

```markdown
- [blocking] CONFIRM: Confirm entering the Build as per the current digest
```

Only after you clearly confirm will Native remove this item and enter the Build through the continue command with `--confirmed`. The Runtime implements the same access control for Sequential and Batch. When the final requirement confirmation is lacking, change will remain at Shape. The compatibility status in the old change also needs to be completed for this confirmation.
