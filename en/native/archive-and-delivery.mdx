...
After a change reaches Archive-ready, the Runtime first provides one complete `archive --dry-run` in `continuation`. When an isolated workspace does not yet have a delivery option, the preflight includes the corresponding `--finish` option.
