...
description: "Understand the one decision you make when a change passes verification and reaches Archive: how this archive is delivered. The options depend on the workspace chosen at creation, and you confirm one of five mutually exclusive options or defer."
---

Once all acceptance items pass and the change stops at Archive-ready, you face the only decision before the work is done: how this archive is delivered. Whether you make this decision at all, and which options are available, depends on the workspace you chose when you created the change. Comet first shows the current branch, the target branch, the directories, and the actual impact of each option, and then you confirm or defer.

## The workspace choice decides whether you are asked here
...
## The confirmation gate: when `--confirmed` is needed
...
When `required` is active, the final Archive candidate has one more gate: after you decide explicitly, run the real archive with `--confirmed`; `--dry-run` only previews and neither requires nor writes a final confirmation. `--confirmed` only matters when the change is genuinely stopped at an Archive-ready candidate waiting for final confirmation — intermediate repair rounds never trigger the archive confirmation, and if the requirements or acceptance criteria are modified while you wait, the change first returns to Shape, where the old verification results and archive approval no longer apply. When in doubt, run `--dry-run` first and let the Runtime return the current next step.

## How to come back after deferring
...
## Interruptions and concurrent conflicts
