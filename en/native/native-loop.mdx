...
This article takes the implementation of the Comet 0.4.0-rc series (from rc.1 onward, RC below) as the standard. Native Loop sets the constraints of clear input, complete evidence, limited budget, and knowing when to stop for each round of implementation, and does not let the model reflect on itself indefinitely.
...
RC will record the following counts:
...
"Failed acceptance" and "Verifier task execution error" are two different types of issues. The former indicates that there is still a gap in implementation or requirements, while the latter suggests that the Verifier has not completed the task properly. RC allows a maximum of `3` consecutive execution errors, and each acceptance attempt can accept a maximum of `2` rounds of additional inspection requests. When the upper limit is reached, the Runtime will explicitly pause and will not retry indefinitely, nor will it treat an exception as a pass.
