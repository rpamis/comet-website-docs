...
description: "Understand how Native coordinates Builder, Runtime, and independent Verifier to deliver trusted results through an implementation and acceptance loop."
---
...
Shape has been confirmed.
    ↓
Builder implementation → New read-only Reviewer Subagent reviews code → Submit candidate implementation
    ↓
...
| --- | --- | --- |
| Builder |Read `brief.md`, Specs and acceptance items, complete the implementation and fix the failed acceptance items|It cannot be declared approved merely based on one's own summary|
...
For the same candidate implementation, the Runtime can reuse the checks that have been completed in the current acceptance attempt to avoid duplicate execution. When the Verifier finds the existing evidence insufficient, it sends a `request-checks` request to the Runtime stating which project commands still need to run; the commands are still executed and recorded by the Runtime, never by the Verifier itself. For such requests, the Runtime limits the number of rounds and filters out duplicate items.
...
| --- | --- | --- |
| `iteration` |When the Builder modifies the implementation and submits a new candidate implementation|Which round of implementation or repair|
| `attempt` |When the Runtime starts a new Verifier for the same candidate implementation|The acceptance attempt number of this candidate implementation|

For instance, when the Runtime starts a new Verifier acceptance attempt for the same candidate implementation, `attempt` will increase, but `iteration` remains unchanged. The Verifier requests supplementary checks in the current attempt and does not add `attempt` separately. Only after the Builder modifies the code and submits a new candidate implementation will `iteration` proceed to the next round.
...
Continue reading [Verification and Repair ](/en/native/verification-and-repair) to learn about the Verifier protocol and failure handling. Read [Task progress and interruption recovery](/en/native/continuation-and-checkpoints) to learn about continuation, Checkpoints and Recovery Mechanisms
