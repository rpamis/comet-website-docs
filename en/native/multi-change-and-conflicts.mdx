...
`new` and `select` update the shared selection. `show` and `status` never switch the current change as a side effect. `status` without a change name provides the paginated active-change list. When several active changes exist and the selection is absent, invalid, or points to another workflow, writes stop and require an explicit selection.

<Info>
  Native has no separate <code>resume</code> command. Recovery consists of read-only state
  inspection, target confirmation, and <code>select</code>. In the Agent, you still enter through
