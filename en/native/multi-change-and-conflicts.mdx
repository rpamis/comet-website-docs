...
`new` and `select` update the shared selection. `list`, `show`, and `status` never switch the current change as a side effect. When the selection is missing or points to a missing or archived change, the Router enumerates active changes again: ordinary work is not blocked when there are no candidates, exactly one candidate is inferred read-only, and multiple candidates stop writes until one is selected. An unreadable selection, a disabled workflow, or change state that cannot be read safely still stops routing.

<Info>
  Native has no separate <code>resume</code> command. Recovery consists of
  read-only state inspection, target confirmation, and <code>select</code>. In
  the Agent, you still enter through
...
<p align="center">
  <img
    ...
    alt="Xiaoyu compares the newest bookmarked specification, rewrites her page, and prepares to verify again, showing safe rebase after concurrent changes"
  />
</p>
