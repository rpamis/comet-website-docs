...
The first Verify found that the session timeout was still 60 minutes, while the confirmation requirement was 30 minutes. This means the implementation does not match the requirements. The requirements themselves have not changed.
...
When repairing, priority is given to the items that have not passed and the acceptance items affected by this modification. After the partial repair is passed, the Runtime will still arrange for the final full acceptance.
...
## User decisions are bound to the state version
...
Native retains facts that can still be verified and invalidates conclusions that rely on old goals or old implementations. This way, existing work can continue to be utilized while avoiding carrying outdated approval results into new deliveries.

Continue reading: [Decision Attribution ](/en/native/decision-ownership), [Verification and Repair ](/en/native/verification-and-repair), and [Task progress and interruption recovery](/en/native/continuation-and-checkpoints).
