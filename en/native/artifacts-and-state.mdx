...
<p align="center">
  <img
    ...
    alt="Xiaoyu files readable documents on an upper shelf and keeps state records and hash indexes in locked boxes below, showing recoverable responsibilities"
  />
</p>
...
Every active or archived Native change has its own `comet-state.yaml`. It records machine-owned phase, revision, approval, `approved_contract_hash`, verification, and specification-operation state. The normal flow enters Build only with `approval: confirmed`. `implicit` exists only for compatibility with older changes, does not prove user confirmation, and must be upgraded through explicit confirmation before leaving Build. `approved_contract_hash` binds approval to the brief/spec contract from that moment, and later contract drift also requires fresh user confirmation. Do not edit these fields manually or confuse this file with Classic's `.comet.yaml`.
...
`baseline-manifest.json` is the bounded project snapshot captured when a change is created. It stores not only file-content identities, but also normalized `include` and `exclude` patterns, the policy hash, execution limits, omission facts, and completeness. The Git provider includes only tracked and non-ignored untracked files and represents each submodule/gitlink atomically; non-Git projects use a bounded physical-tree provider with before/after enumeration fences. Creation and a v1/v2 migration cutover require a complete baseline. A current v3 baseline cannot be rebuilt automatically from current files.

Later current snapshots always use the policy stored in the baseline manifest instead of rereading include/exclude from project configuration. This prevents an edit to `.comet/config.yaml` during implementation from hiding a change that was already in scope. Resource budgets still come from current project configuration, so you may raise `max_files`, `max_total_bytes`, or `max_duration_ms` as the repository grows and continue capturing the same audit boundary.

See [Native configuration](/en/native/configuration#content-snapshot-scope-and-budgets) and [Native FAQ](/en/native/faq#content-snapshots-and-implementation-scope) for pattern semantics, three copy-ready configurations, and the decision between raising a budget and excluding a file.
