...
The command does not commit. After success, review and commit the change move, main-spec merge, and frontmatter annotations. Archived `docs/superpowers/` pointers remain rooted at the project, so the Dashboard can still display Design Docs, Plans, and verification reports.

<Note>
  The packaged <code>comet-archive.mjs</code> launcher remains for compatibility. Prefer <code>comet archive</code> in user and agent instructions.
</Note>
