...
<Info>
  This is advanced content, for users who want to deeply understand the artifacts produced by `/comet-any` and the underlying mechanics of `comet skill`. Classic Spec-mode `/comet-classic` users don't need to understand the Engine to use the five-phase workflow.
</Info>
...
| --- | --- | --- | --- |
| Built-in workflow Skills | Distributed in `assets/skills/` | `/comet-classic`, `/comet-open`, `/comet-any` | Drive the five-phase workflow and Skill Creator |
...
| --- | --- | --- |
| `change` | `<changeDir>/.comet/` | Bound to the OpenSpec change directory; one Run per directory. Used by the classic `/comet-classic` workflow. |
...
The classic `/comet-classic` five-phase workflow is driven by the Engine under the hood, but users don't need to operate the Engine directly.
...
<Note>
  The internal <code>comet-classic</code> Engine definition is a deterministic Skill with 30 steps covering the full/hotfix/tweak profiles. Users enter through the permanent <code>/comet-classic</code> entry point (or through the project-configured <code>/comet</code> alias) rather than operating those internal YAML files; the Engine loads and runs them underneath.
</Note>
...
This Engine definition is **not** the user-facing top-level `comet-classic/` entry directory. The internal artifact consists of these three YAML files, registered by `assets/manifest.json`'s `internalSkills` and loaded by the Engine; top-level `/comet-classic` provides the stable entry and workflow orchestration.
...
- [Workflow concepts](/en/concepts/workflow) — How `/comet-classic` uses the Engine under the hood
