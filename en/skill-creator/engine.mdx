...
description: "Understand Skill package structure, the Engine loop, snapshots, guardrails, runtime checks, and Run state."
---

<Tip>
When creating, optimizing, or combining Skills, calling <code>/comet-any</code> in the Agent and confirming prompts is enough for most cases.
Unless you are debugging an Engine Run, runtime checks, or package structure, you do not need the internals on this page first. Classic Spec mode users of <code>/comet-classic</code> also do not need Engine internals to run the five-phase workflow.
</Tip>
...
<Warning>
Dynamic re-planning of <strong> cannot relax guardrails</strong>. The allow list and budget are fixed by the Skill, and the runtime Agent
cannot expand them at runtime. This is the core of Engine security - Skill defines the boundaries, and the executor can only act within the boundaries.
</Warning>
...
<Note>
The <code>ComET-Classic </code> definition within the Engine is a deterministic Skill with 28 steps to cover
...
|"File|"Content|
| --------------------------------------- | ------------------------------------------------------------------------- |
| `comet/runtime/classic/skill.yaml`      |deterministic layout diagram (28 steps, three profiles: full/hotfix/tweak)|
