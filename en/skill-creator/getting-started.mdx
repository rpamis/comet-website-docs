...
<Warning>
  Composing any Skill is still experimental. Future versions may significantly restructure the flow, change command interfaces, modify generated package layouts or publish gates, or remove and replace parts of this capability. Try it first in non-critical projects, and review the changelog before upgrading Comet.
</Warning>

## Who It's For

- Those who want to distill a recurring team process into a **reusable Skill** instead of re-describing it every time.
- Those who want to **add, replace, or disable** certain steps on top of the classic `/comet-classic` five phases.
...
| --- | --- | --- | --- |
| **Five-phase customization based on existing Comet Skills** | `customize-comet` | Incremental adjustments on top of the classic five phases | Want to add a review step to `/comet-classic`, or require using a component library |
...
<AccordionGroup>
  <Accordion title="What is the difference between /comet-any and /comet">
    ...
    Use <code>/comet</code> for features, bug fixes, or modifying project code. Use <code>/comet-any</code> to distill a process into a reusable Skill, or to add team rules on top of the <code>/comet-classic</code> five phases.
  </Accordion>
  ...
  <Accordion title="Will the artifacts be automatically distributed">
