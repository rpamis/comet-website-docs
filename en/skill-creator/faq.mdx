...
<AccordionGroup>
  <Accordion title="What is the difference between /comet-any and /comet">
    `/comet-classic` is the permanent entry point for Classic Spec Mode. `/comet` is the shared alias that forwards according to project configuration. `/comet-any` is the Skill Creator, used to create, optimize, or organize reusable Skills.

    If you want to build a product feature, fix a bug, or modify project code, use `/comet`. If you want to distill a workflow into a reusable Skill, or add team rules on top of the `/comet-classic` five phases, use `/comet-any`.
  </Accordion>

  <Accordion title="How do I choose among the three starting points">
    Choose based on your goal:

    - **Five-phase customization based on existing Comet Skills**: You want to keep the `/comet-classic` five phases and only add a component library, review rules, or team constraints.
  ...
  <Accordion title="What do keep/add/replace/disable mean on the confirmation page">
    ...
    When customizing based on `/comet-classic`, the core control steps cannot be replaced arbitrarily. The plan confirmation page will list what can and cannot be done.
  </Accordion>
</AccordionGroup>
