...
<Tip>
  Normally you only need <code>/comet-classic</code>. When your request is about fixing existing broken behavior, routing prefers the hotfix preset and invokes <code>/comet-hotfix</code> automatically. Use <code>/comet-hotfix</code> directly only when you want manual control.
</Tip>
...
Hotfix is idempotent. After interruption, run `/comet-classic`; it reads `.comet.yaml` and routes back to `/comet-hotfix` when the change is in `phase: build`.
