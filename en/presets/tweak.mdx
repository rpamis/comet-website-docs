...
<Tip>
  Normally you only need <code>/comet-classic</code>. When your request can fit into one OpenSpec change and does not need full design, routing prefers tweak automatically.
</Tip>
...
Tweak is idempotent. After interruption, run `/comet-classic`; it reads `.comet.yaml` and routes back to `/comet-tweak` when needed.
