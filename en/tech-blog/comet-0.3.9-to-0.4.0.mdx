...
**The 0.4.0 decision**: rewrite every script as a Node `.mjs` launcher, backed by a single shared TypeScript runtime (`domains/comet-classic/`). Each `.mjs` is an esbuild-generated single-file bundle with dependencies like `yaml` inlined.
