...
Beta 15 makes global setup, Native multi-change work, and the Dashboard easier to use across projects of different sizes.

### Added

- **Global defaults activate projects**: After global initialization, the first explicit `/comet` call in an unconfigured project creates its project configuration from those defaults. Artifacts remain inside the project, and later global changes do not rewrite activated projects.
- **Native workspace isolation**: Before creating a new change, Native detects existing changes. When isolation is needed, it offers `current`, `branch`, or `worktree` and continues automatically in a new worktree.

### Changed

- **Dashboard change lists**: Active, archived, and all lists load in pages, showing lightweight rows first and fetching detail after selection. The current detail stays visible while loading, with a retry when a request fails.
- **Hook installation and updates**: Projects maintain one project-rooted Router. Init, Update, and Doctor handle historical Comet Hooks, preserve user configuration, and report incomplete cleanup clearly.

### Fixed

- **Parallel-change recovery**: Recovery checks only the target change, so unrelated active changes no longer add irrelevant blockers.
- **Verification scope protection**: If the implementation scope changed, verification stops before using stale results; unchanged successful checks can be reused.
- **Workflow recovery**: Classic returns to plan creation when no valid implementation plan exists, instead of writing project source without one.
- **Global update scope**: `comet update` refreshes workflows that are already installed and does not implicitly add Native or Classic during an update.
