...
- **Comet Native workflow**: Adds a self-contained strong-model workflow with a front-loaded clarification lock that keeps unresolved user-visible outcomes user-owned, asks one upstream question with a recommendation, and blocks Build or implementation until shared understanding. It also provides one continuously advancing Skill, configurable artifact roots and language, complete target specifications, checkpoints, and bounded status without OpenSpec, Superpowers, or another external Skill.
...
- **Selectable project workflows**: Project-scope `comet init` explains and offers Native, Classic, or both. New projects default `/comet` to Native; `--workflow both` installs two independent capability sets that share `.comet/config.yaml` without merging state.
