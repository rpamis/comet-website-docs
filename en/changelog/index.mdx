...
<Update label="0.4.0-beta.20" description="2026-08-27" rss={{ title: "Comet 0.4.0-beta.20", description: "Release notes for 2026-08-27" }}>
Beta 20 makes CodeGraph diagnostics more actionable, lets Classic continue through phase and preset Skills from the normal entry point, and improves Native support for large and evolving requirements.

### Changed

- **Actionable CodeGraph diagnostics**: `comet init` and `comet doctor` now separate CodeGraph CLI installation, project-index freshness, Agent MCP registration, and effective capability for each detected Agent. A current project index no longer implies that an Agent can use CodeGraph.
- **Classic Skill dispatch**: `/comet` can now invoke the required Classic phase or preset Skill through the internal router, so users can normally continue from the single `/comet` entry point.
...
- **Native Archive-ready requirement revisions**: When a Native change is waiting at `archive-ready`, users can revise the visible requirements or acceptance criteria and return to Shape. The old verification and Archive authorization are invalidated while the change workspace is preserved.

</Update>

<Update label="0.4.0-beta.19" description="2026-08-21" rss={{ title: "Comet 0.4.0-beta.19", description: "Release notes for 2026-08-21" }}>
Beta 19 adds Grok as a first-class platform, introduces the on-demand `/comet-review` entry point and a repository-owned Native pull-request finish provider, and fixes several Windows and Classic workflow issues.

### Added

- **Grok platform support**: `comet init`, `comet update`, `comet doctor`, and `comet uninstall` now treat Grok as a first-class host. Skills, rules, and the Hook Router live in `.grok/skills/`, `.grok/rules/`, and `.grok/hooks/comet.json`; the Router recognizes `--platform grok` and Grok's native `write` and `search_replace` tools.
- **Repository-owned Native pull-request finish provider**: Projects can use a structured repository command to validate pull-request titles, bodies, templates, and repository policy. Comet still owns commits, pushes, remote base/head/SHA checks, existing-PR reuse, recoverable failure state, and safe worktree cleanup. See [Native configuration: Pull-request finish provider](/en/native/configuration#pull-request-finish-provider).
...
- **On-demand code review**: The new `/comet-review` Skill reviews a Native or Classic change against its implementation diff and existing evidence, reporting correctness, security, boundary, and coverage issues by priority. It is read-only, does not advance the workflow, and does not replace Verify. See [Skill types and purpose](/en/concepts/skills).
- **Fork pull-request guidance**: First-time contributors opening a pull request from a fork now receive repository guidance through a trusted workflow comment.
- **Pull-request template validation**: Pull requests missing required template sections or items, or containing unchecked checklist items, now receive an actionable comment and a failing check.
- **Issue triage labels**: New issues are automatically marked for triage and assigned repository-area labels based on structured issue-form selections.

### Changed

- **Hook write allowlist documentation**: The website now explains how to configure project-relative `hook.allow_paths` directories for protected workflow phases.
- **Native child plans**: New Supervisor Change child plans keep a readable parent acceptance index, while Runtime verification retains the complete brief and Spec acceptance matrix; historical child-plan files remain compatible.
- **Dashboard artifact preview**: Full-screen previews can now close with Escape, long tables keep horizontal scrolling and readable headers, and directory navigation uses a larger zoom level.
- **Native requirement sources**: File- and link-based requirement sources now retain a complete coverage map in the Native brief, mapping every active executable requirement to a target Spec and acceptance criterion. Incomplete or unavailable sources continue to block until clarified.
- **Native verification decisions**: Native `await-user` continuations now let users accept the current result, revise the implementation, or revise the requirements and acceptance criteria, while invalidating authorization to archive the old target cycle.
- **Pull-request title scopes**: Conventional pull-request titles now support repository areas including Native, Classic, Hook, Dashboard, Platform, Workflow, and Eval, such as `feat(native): ...`.
- **Ambient Resume exemption for non-Comet Skills**: Managed Ambient Resume instructions in `AGENTS.md` and `CLAUDE.md` now exempt explicit non-Comet Skill and slash-command calls, so unrelated plugin or tool installation tasks no longer begin with `comet resume-probe`. Existing projects receive the revised block through `comet update`.

### Fixed

- **Reliable Windows initialization**: `comet init` no longer aborts OpenSpec and Skill installation on filesystems without stable file identifiers, such as exFAT/FAT32 removable or network drives. Integrity checks now compare the post-write snapshot instead of treating Comet's own temporary-file write as tampering; symlink and directory-move detection remain enabled.
- **Classic Design handoff refresh**: Running `comet handoff <change> design --write` after OpenSpec artifacts change no longer reports a stale handoff, so the Design Guard can pass and the full Classic workflow can advance to Build. Refresh rewrites stale context even when manually aligned hashes would otherwise short-circuit, and regenerates the package when delta Specs are added, changed, or removed. Refresh remains available after the Guard has advanced to Build.
- **Classic Ambient Resume**: When `ambient_resume` is enabled, `comet init` and `comet update` now preserve managed Ambient Resume instructions for Classic-only projects instead of removing them on repeat runs.
- **Classic workspace command context**: `comet classic workspace prepare` and `comet classic workspace resolve` no longer report “Classic command project context is unavailable” for every isolation mode (`current`, `branch`, and `worktree`), fixing workspace preparation in the Classic Open flow.
- **Native Archive**: Archive staging now follows Git ignore rules; valid portable verification reports are no longer treated as incomplete migrations.
- **Windows Eval packaging**: Packaging no longer traverses ignored pytest and Eval runtime artifacts beyond the application boundary, so stale Windows test directories no longer make `pnpm pack` fail with `EPERM`.
- **Windows Eval isolation**: Repeated `comet eval` runs no longer copy generated `.comet` caches, run artifacts, or framework Runtime state into the Skill workspace, avoiding nested caches and deep `MAX_PATH` failures on Windows.
- **Monorepo Dashboard workspaces**: Starting `comet dashboard` from a monorepo subdirectory containing `.comet/config.yaml` now uses that subdirectory as the workspace root and maps sibling Git worktrees to the same subdirectory. Change lists no longer appear empty when the Comet project root differs from the worktree root.
- **Ambient Resume replies inside a workflow**: An Agent following managed Ambient Resume instructions no longer gets stuck when the user replies with a short option to an in-workflow question. The managed block now exempts replies inside a Comet workflow and clarifies that `out_of_scope` only prevents entering a workflow; it never interrupts one already in progress.
- **Windows stdin and file JSON parsing**: Runtime commands now strip a leading UTF-8 BOM before parsing JSON, so frame JSON redirected by Windows PowerShell 5.1 or written by `Out-File` no longer makes `comet-intent.mjs route --stdin` fail with “Invalid JSON”. The same tolerance covers `comet-resume-probe.mjs --stdin`, `comet-hook-guard.mjs`, Hook Router payloads, and Native `evidence format` entries read from stdin or `--entries` files.
- **Classic record-check step synchronization**: After an Agent checks the final `tasks.md` item during Build, `comet state record-check` no longer reports a “Classic Run step mismatch”. Task completion now advances the evidence-derived Run step without a separate state command; record-check reconciles the Run projection before recording evidence instead of blocking valid Build evidence. Real corruption involving Skill identity, snapshots, or migration markers still fails hard.
- **Classic Build plan dispatch**: The Build Step 1 planning subagent no longer frequently falls back to the main session (#345). Dispatch prompts tell the subagent to skip interactive Skill steps, the main session fixes the complete plan path before dispatch, and the subagent must end with one `PLAN_PATH:` line. Without Skill tools, it fails quickly with `SKILL_UNAVAILABLE`; the plan stays scoped to `tasks.md`, and later Step 1 items switch directly to inline execution after a dispatch failure in the same session.

</Update>

<Update label="0.4.0-beta.18" description="2026-08-13" rss={{ title: "Comet 0.4.0-beta.18", description: "Release notes for 2026-08-13" }}>
Beta 18 makes Eval a practical entry point for any local Skill, while adding Native orchestration for large requirements and clearer change visibility across worktrees.

### Added

- **Standalone Skill evaluation entry point**: Run `comet eval ./my-skill` directly against a local Skill. You do not need to create `comet/eval.yaml` before your first run; see [Quickstart: Evaluating a Skill](/en/eval/quickstart).
- **Task discovery and offline pre-checks**: Define inline tasks or reference task packages from the Skill package, and let a normal run automatically generate and cache 2–4 cases from the Skill content when no tasks are declared. `--collect` checks discovery and configuration without starting an Agent or Docker.
- **Selectable evaluation Agents and independent Judge**: Choose `claude-code`, `codex`, `qoder`, or `codebuddy`, configure the subject Agent and Judge independently, and keep reports in a predictable local run directory.
- **Langfuse evaluation suite**: Send task traces, rubric scores, pass metrics, and experiment summaries to Langfuse while retaining the local report.
- **Native Supervisor Change mode**: Confirm Supervisor Change mode during Shape for a large requirement. Runtime advances child changes by dependency and verifies the complete Supervisor Change goal on the final integrated branch.
- **Dashboard across worktrees**: Dashboard discovers registered Git worktrees, keeps independent changes as root entries, and expands Native Supervisor Changes to show child status and progress.
- **WorkBuddy platform support**: `comet init` and `comet update` install and refresh Comet Skills in project `.workbuddy/skills/` or user `~/.workbuddy/skills/`. Project installs also merge the Comet Hook into `.workbuddy/settings.json` while preserving existing settings.

### Changed

- **Project-local Hook write allowlist**: Projects can configure project-relative directories under `hook.allow_paths` in `.comet/config.yaml` so shared rules or notes remain writable during guarded Native Shape or Classic non-coding phases. Comet Runtime and workflow-owned artifacts remain protected. The Hook itself does not block writes outside the project, so external paths do not need to be listed.
- **Classic workspace routing**: Creating or resuming a Classic change now follows its branch and worktree binding and reuses an existing worktree when available.
- **Native workspace reuse**: Native parallel changes reuse a matching linked worktree and recreate it during recovery when the branch still exists but the worktree is missing.

### Fixed

- **Classic Guard project-root resolution**: Classic design Guards now read delta specs from the discovered project root even when invoked from a nested working directory, instead of resolving the specs directory from the current working directory.

</Update>

<Update label="0.4.0-beta.17" description="2026-08-10" rss={{ title: "Comet 0.4.0-beta.17", description: "Release notes for 2026-08-10" }}>
Beta 17 rebuilds Native completion and recovery: user-readable state and documents can travel across devices, the Runtime owns necessary checks and the bounded Loop, and Dashboard reads the same portable state.

### Added

- **Independent Native Subagent verification**: After a Builder submits an implementation handoff, the Runtime runs necessary checks and dispatches a fresh read-only Verifier to cover every A1…An acceptance item. A Builder's self-reported completion cannot archive a change; the Skill-coordinated path also requires one explicit user confirmation before Archive.
- **Trae Hook support**: `comet init`, `comet update`, `comet doctor`, and `comet uninstall` support project and global Hook Router entries for Trae and Trae CN while preserving user-owned Hook settings.

### Changed

- **Faster Native Verify**: Normal checks run once and reuse completed results, with output streamed to local logs. Verify no longer scans or fingerprints the whole project, and the normal Archive path does not re-run acceptance. Long Maven, Gradle, npm, or Python output does not invalidate an otherwise valid result just because it is long.
- **Portable Native state**: `comet-state.yaml` records the phase, Loop, acceptance results, handoff, blockers, and next action. `verification.md` is a rebuildable user report, while in-flight execution state and logs stay under `.comet/runtime/native/`.
- **Native Dashboard**: Native details show the Build/Verify stage, iteration, attempt, acceptance items, checks, blockers, history, and user-readable artifacts. Machine Runtime files stay out of the artifact list, and legacy archives are available through a read-only adapter.
- **Clarification and workspace choices**: New projects default to Batch clarification, with decision dependencies resolved before asking. Creating a change presents `current`, `branch`, and `worktree` choices and keeps ownership correct when resuming or using linked worktrees.
- **Skill and CLI guidance**: Public Native commands and the bilingual Skills follow Runtime continuations instead of requiring hand-built internal state, and phase Skill invocation policies are explicit.

### Fixed

- **Workspace and recovery routing**: Linked worktrees, cross-device recovery, wrong checkouts, and interrupted Archive steps validate the current state first instead of silently reusing an old pass or routing a change to another working directory.
- **Cross-platform Native checks**: Windows resolves npm, pnpm, and similar command shims correctly, and timed-out checks terminate their child process tree to reduce platform-specific Verify failures.
- **Hook and workflow isolation**: Legacy global Hooks without trusted project context stay neutral; Classic-only references remain scoped to Classic installs; and `comet doctor` recognizes plugin-managed Superpowers installs.

### Removed

- **Legacy Native Runtime verification chain**: New changes no longer create the old project-scan, checkpoint, check, evidence, or receipt chain. Active legacy changes migrate conservatively when needed, and archived legacy changes remain read-only.

</Update>

<Update label="0.4.0-beta.16" description="2026-08-05" rss={{ title: "Comet 0.4.0-beta.16", description: "Release notes for 2026-08-05" }}>
Beta 16 improves coexistence between Classic and standalone OpenSpec projects, strengthens Codex Native Hook parsing for patch files, and updates dependencies with known security fixes.

### Fixed

- **Classic and standalone OpenSpec coexistence**: Classic now uses only the configured artifact root, so projects that do not use Comet can keep the other OpenSpec root at the same time. Explicit root migration still refuses to overwrite a non-empty destination.
- **Codex Native Hook parsing**: Native now recognizes Add, Update, Delete, and standard `+++ b/...` file headers in raw `apply_patch` input, keeping phase protection consistent across patch formats.

### Security

- **Dependency security updates**: Updated PostCSS, Undici, and brace-expansion to releases that address reported security vulnerabilities.

</Update>

<Update label="0.4.0-beta.15" description="2026-08-05" rss={{ title: "Comet 0.4.0-beta.15", description: "Release notes for 2026-08-05" }}>
Beta 15 adds global project activation and Native parallel changes, reworks the Dashboard change explorer and Hook lifecycle, and fixes several issues across verification, parallel recovery, and Hooks.
...
<Update label="0.4.0-beta.14" description="2026-08-02" rss={{ title: "Comet 0.4.0-beta.14", description: "Release notes for 2026-08-02" }}>
...
<Update label="0.4.0-beta.13" description="2026-08-02" rss={{ title: "Comet 0.4.0-beta.13", description: "Release notes for 2026-08-02" }}>
...
<Update label="0.4.0-beta.12" description="2026-07-30" rss={{ title: "Comet 0.4.0-beta.12", description: "Release notes for 2026-07-30" }}>
...
<Update label="0.4.0-beta.11" description="2026-07-29" rss={{ title: "Comet 0.4.0-beta.11", description: "Release notes for 2026-07-29" }}>
...
<Update label="0.4.0-beta.10" description="2026-07-28" rss={{ title: "Comet 0.4.0-beta.10", description: "Release notes for 2026-07-28" }}>
...
- **CodeGraph index lifecycle**: Non-interactive project setup can explicitly choose `comet init --codegraph init|skip`. `comet doctor` distinguishes a missing CLI, missing index, incomplete index, stale index, and ready index; only authorized `comet doctor --repair --yes` runs initialization, rebuild, or sync actions ([#245](https://github.com/rpamis/comet/issues/245)).
- **Classic configurable artifact layout**: Projects can select `classic.artifact_layout: legacy|docs` for `openspec/` or `docs/openspec/`. `comet classic root show` reports the active layout, while `comet classic root move docs --dry-run` and `--apply` safely migrate existing artifacts and configuration ([#173](https://github.com/rpamis/comet/issues/173)).
- **Native evidence-backed verification**: Every mandatory acceptance item must be supported by current evidence bound to the active snapshot and scope. Native records automated checks, manual observations, implementation attestations, independent review, and approved structured waivers. Failed, skipped, blocked, stale, or incomplete evidence cannot produce a pass, and high-risk changes require independent review ([#240](https://github.com/rpamis/comet/issues/240)).

### Changed

- **Classic documentation layout**: New Classic and dual-workflow projects store OpenSpec artifacts in `docs/openspec/` beside `docs/comet/` and `docs/superpowers/`. Existing projects keep root-level `openspec/` until explicitly migrated, and all Classic commands follow the selected layout ([#173](https://github.com/rpamis/comet/issues/173)).
...
- **Native archive confirmation**: Set `native.archive_confirmation: required` to request one explicit decision for the final Archive candidate, or keep automatic archiving. Intermediate repair iterations never request archive confirmation, and choosing not to archive preserves the active change ([#238](https://github.com/rpamis/comet/issues/238)).
...
- **Classic archive references**: Archive rewrites change-local handoff and related artifact references to the dated archive location, verifies them before reporting success, and preserves the handoff hash. Archived changes no longer require manual `.comet.yaml` repair ([#244](https://github.com/rpamis/comet/issues/244)).
- **Secondary worktree diagnostics**: `comet doctor` distinguishes current-worktree assets, primary-worktree-only assets, and an available global fallback. Intentionally absent ignored assets in a secondary worktree are no longer reported as project corruption ([#246](https://github.com/rpamis/comet/issues/246)).
- **Local Eval execution environment**: Local task images include the required Claude Code runner and run as a non-root user, allowing quick and full Eval to identify the execution environment and start the runner reliably ([#229](https://github.com/rpamis/comet/issues/229)).
- **Local Eval validators**: Copied lightweight validators can reliably load their bundled scaffold runtime. Workflow Eval uses tasks that match the workflow contract, while no-Skill CONTROL results remain observational data instead of incorrectly blocking a valid Dynamic Skill result ([#229](https://github.com/rpamis/comet/issues/229)).
- **Creator Bundle portability**: Creator-generated workflow Bundles persist project-relative evidence paths and use platform-neutral authoring guidance. A Claude-specific custom Agent is no longer treated as a required Codex runtime capability, and successful local Eval results are recorded as evidence for the current Bundle ([#229](https://github.com/rpamis/comet/issues/229)).
...
<Update label="0.4.0-beta.9" description="2026-07-25" rss={{ title: "Comet 0.4.0-beta.9", description: "Release notes for 2026-07-25" }}>
### Added
...
- **Classic archive final state**: Classic now confirms immediate remote delivery before irreversible archiving, writes `branch_status: handled` before the one archive commit, and pushes that complete commit only once. A successful archive no longer leaves an uncommitted `.comet.yaml` or a remote archive stuck at `pending` ([#237](https://github.com/rpamis/comet/issues/237)).
...
<Update label="0.4.0-beta.8" description="2026-07-22" rss={{ title: "Comet 0.4.0-beta.8", description: "Release notes for 2026-07-22" }}>
...
<Update label="0.4.0-beta.7" description="2026-07-22" rss={{ title: "Comet 0.4.0-beta.7", description: "Release notes for 2026-07-22" }}>
...
<Update label="0.4.0-beta.6" description="2026-07-20" rss={{ title: "Comet 0.4.0-beta.6", description: "Release notes for 2026-07-20" }}>
### Added
...
- **Workspace isolation choice**: Full, hotfix, and tweak workflows now ask users to explicitly keep the current branch, create a branch, or create a worktree instead of choosing silently. Full workflows now also support `isolation: current` ([#190](https://github.com/rpamis/comet/issues/190)).
...
<Update label="0.4.0-beta.5" description="2026-07-14" rss={{ title: "Comet 0.4.0-beta.5", description: "Release notes for 2026-07-14" }}>
### Changed
...
- **Codex hook configuration**: Project and global Codex installs write supported `.codex/hooks.json` and safely migrate older Comet-managed configuration ([#199](https://github.com/rpamis/comet/issues/199)).
...
<Update label="0.4.0-beta.4" description="2026-07-11" rss={{ title: "Comet 0.4.0-beta.4", description: "Release notes for 2026-07-11" }}>
### Added
...
- **Stable Classic commands**: Adds top-level `comet state`, `comet guard`, `comet handoff`, and `comet archive` commands so agents and automation no longer depend on internal installed script paths ([#186](https://github.com/rpamis/comet/issues/186)).
- **Custom project command evidence**: Projects without an inferred npm, Maven, or Cargo command can record the real build or verification command and exit result as auditable evidence instead of relying on an undocumented skip path ([#192](https://github.com/rpamis/comet/issues/192)).
- **CodeBuddy hook support**: Project-level and global CodeBuddy Code installs non-destructively merge the Comet PreToolUse hook into `.codebuddy/settings.json`, preserving existing plugins, settings, and hooks. Invalid configuration is reported explicitly and left byte-for-byte unchanged ([#193](https://github.com/rpamis/comet/issues/193)).

### Changed

- **Mixed change status**: `comet status` now distinguishes Comet-managed changes from plain OpenSpec changes and recommends the appropriate archive command only for ready entries ([#187](https://github.com/rpamis/comet/issues/187)).

### Fixed

- **Parallel active-change guards**: Source-write hooks bind work on a branch or worktree to the explicitly selected current change. Legal build work is no longer blocked by another change in open, design, or archive, while ambiguous selection fails with an actionable prompt ([#196](https://github.com/rpamis/comet/issues/196)).
...
- **Generated Eval manifests**: `comet eval` resolves a Skill Factory `draftHash` placeholder into a temporary manifest bound to the current Bundle draft, so `/comet-any` output can be evaluated without rewriting generated Bundle files ([#183](https://github.com/rpamis/comet/issues/183)).
...
- **Responsive Dashboard workspace**: `comet dashboard` keeps the change workspace within the viewport, preserves scroll position around artifact previews, and shows archived changes as complete instead of recommending verify again ([#170](https://github.com/rpamis/comet/issues/170)).
- **Symlink installs with existing Skills**: `comet init` and `comet update --install-mode symlink` preserve an existing platform `skills/` directory and add links only for Comet Skills, so local and third-party Skills do not make installation fail ([#171](https://github.com/rpamis/comet/issues/171)).
- **OpenSpec CLI install scope**: `comet init` installs or upgrades OpenSpec CLI as a global tool even during project-scope setup, avoiding a project `node_modules/` directory for that dependency ([#175](https://github.com/rpamis/comet/issues/175)).
- **Archived Dashboard artifacts**: The Dashboard resolves archived `docs/superpowers/` pointers from the project root, so Design Docs, Plans, and verification reports remain available after archive ([#176](https://github.com/rpamis/comet/issues/176)).
...
- **Archive annotation formatting**: Classic archive annotations remain idempotent and preserve a clean Markdown EOF, preventing `git diff --check` failures ([#185](https://github.com/rpamis/comet/issues/185)).
- **Global artifact language**: Global `comet init` and `comet update` persist the selected artifact language in `~/.comet/config.yaml`. Classic workflows prefer project config and then fall back to the global default, while projects can still override it ([#174](https://github.com/rpamis/comet/issues/174)).

</Update>

<Update label="0.4.0-beta.3" description="2026-07-08" rss={{ title: "Comet 0.4.0-beta.3", description: "Release notes for 2026-07-08" }}>
### Fixed
...
- **Project config lookup**: Classic guards now resolve project defaults only from `.comet/config.yaml`, while workflow state remains in each change's `.comet.yaml`. Repository-root `.comet.yaml` and `comet.yaml` files no longer participate in config or state resolution ([#158](https://github.com/rpamis/comet/issues/158)).
...
<Update label="0.4.0-beta.2" description="2026-07-07" rss={{ title: "Comet 0.4.0-beta.2", description: "Release notes for 2026-07-07" }}>
### Fixed
...
<Update label="0.4.0-beta.1" description="2026-07-06" rss={{ title: "Comet 0.4.0-beta.1", description: "Release notes for 2026-07-06" }}>
...
<Update label="0.3.9" description="2026-06-17" rss={{ title: "Comet 0.3.9", description: "Release notes for 2026-06-17" }}>
### Added
...
<Update label="0.3.8" description="2026-06-13" rss={{ title: "Comet 0.3.8", description: "Release notes for 2026-06-13" }}>
### Added
...
<Update label="0.3.7" description="2026-06-07" rss={{ title: "Comet 0.3.7", description: "Release notes for 2026-06-07" }}>
### Added
...
<Update label="0.3.6" description="2026-06-02" rss={{ title: "Comet 0.3.6", description: "Release notes for 2026-06-02" }}>
### Added
...
<Update label="0.3.5" description="2026-05-29" rss={{ title: "Comet 0.3.5", description: "Release notes for 2026-05-29" }}>
### Added
...
<Update label="0.3.4" description="2026-05-29" rss={{ title: "Comet 0.3.4", description: "Release notes for 2026-05-29" }}>
### Changed
...
<Update label="0.3.3" description="2026-05-27" rss={{ title: "Comet 0.3.3", description: "Release notes for 2026-05-27" }}>
### Fixed
...
<Update label="0.3.2" description="2026-05-27" rss={{ title: "Comet 0.3.2", description: "Release notes for 2026-05-27" }}>
### Added
...
<Update label="0.3.1" description="2026-05-26" rss={{ title: "Comet 0.3.1", description: "Release notes for 2026-05-26" }}>
### Added
...
<Update label="0.3.0" description="2026-05-25" rss={{ title: "Comet 0.3.0", description: "Release notes for 2026-05-25" }}>
### Added
...
<Update label="0.2.9" description="2026-05-24" rss={{ title: "Comet 0.2.9", description: "Release notes for 2026-05-24" }}>
### Changed
...
<Update label="0.2.8" description="2026-05-24" rss={{ title: "Comet 0.2.8", description: "Release notes for 2026-05-24" }}>
### Added
...
<Update label="0.2.7" description="2026-05-24" rss={{ title: "Comet 0.2.7", description: "Release notes for 2026-05-24" }}>
### Fixed
...
<Update label="0.2.6" description="2026-05-23" rss={{ title: "Comet 0.2.6", description: "Release notes for 2026-05-23" }}>
### Added
...
<Update label="0.2.5" description="2026-05-22" rss={{ title: "Comet 0.2.5", description: "Release notes for 2026-05-22" }}>
### Added
...
<Update label="0.2.4" description="2026-05-21" rss={{ title: "Comet 0.2.4", description: "Release notes for 2026-05-21" }}>
### Added
...
<Update label="0.2.3" description="2026-05-19" rss={{ title: "Comet 0.2.3", description: "Release notes for 2026-05-19" }}>
### Added
...
<Update label="0.2.2" description="2026-05-18" rss={{ title: "Comet 0.2.2", description: "Release notes for 2026-05-18" }}>
### Fixed
...
<Update label="0.2.1" description="2026-05-18" rss={{ title: "Comet 0.2.1", description: "Release notes for 2026-05-18" }}>
### Fixed
...
<Update label="0.2.0" description="2026-05-18" rss={{ title: "Comet 0.2.0", description: "Release notes for 2026-05-18" }}>
...
<Update label="0.1.8" description="2026-05-17" rss={{ title: "Comet 0.1.8", description: "Release notes for 2026-05-17" }}>
### Added
...
<Update label="0.1.7" description="2026-05-16" rss={{ title: "Comet 0.1.7", description: "Release notes for 2026-05-16" }}>
### Added
...
<Update label="0.1.6" description="2026-05-16" rss={{ title: "Comet 0.1.6", description: "Release notes for 2026-05-16" }}>
### Added
...
<Update label="0.1.5" description="2026-05-15" rss={{ title: "Comet 0.1.5", description: "Release notes for 2026-05-15" }}>
### Added
...
<Update label="0.1.4" description="2026-05-15" rss={{ title: "Comet 0.1.4", description: "Release notes for 2026-05-15" }}>
### Fixed
...
<Update label="0.1.3" description="2026-05-15" rss={{ title: "Comet 0.1.3", description: "Release notes for 2026-05-15" }}>
### Added
