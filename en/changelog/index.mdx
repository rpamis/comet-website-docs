...
- **Native parent-child changes**: Confirm a parent-child structure during Shape for a large requirement. Runtime advances children by dependency and verifies the complete parent goal on the final integrated branch.
- **Dashboard across worktrees**: Dashboard discovers registered Git worktrees, keeps independent changes as root entries, and expands Native parents to show child status and progress.
