...
- **Comet Native workflow**: Adds a self-contained workflow for strong models. Clarification supports the default `sequential` mode and configurable `batch` mode: Sequential asks one upstream question per round, while Batch asks every currently answerable question together and requires final shared-understanding confirmation before Build. In Claude Code, Native prefers one `AskUserQuestion` call when the whole round fits; otherwise it presents the entire round as numbered text without splitting it. The continuously advancing Skill, configurable artifact root and language, complete target specifications, checkpoints, and bounded status do not depend on OpenSpec, Superpowers, or another external Skill.
- **Evidence-bound autonomous execution**: Adds Git-owned project snapshots that include tracked and non-ignored untracked files while treating each submodule/gitlink atomically, plus bounded physical-tree snapshots with before/after enumeration fences for non-Git projects. It also adds bounded overflow evidence, contract-hash-bound approval, Acceptance IDs, immutable verification report snapshots, optional built-in read-only check receipts, stale-evidence fallback, partial-scope authorization, and repair episodes. New changes and old-schema migrations require a complete baseline before committing state, and incomplete current snapshots never invent deletions. Repeated failures stop without genuine scope progress.
- **Recoverable Native safety boundary**: Adds schema migration, revision/CAS with serialized live mutation, protected Run and file I/O, workspace identity, current-root conflict inspection, two-step Archive, exactly-once transaction recovery, safe root moves, explicit stale-lock takeover, bounded evidence retention, and a read-only Dashboard projection.
- **Stable workflow entries**: Adds permanent `/comet-native`, `/comet-classic`, and `comet workflow resolve` entries. `/comet` is a configuration-only alias that selects one workflow without converting or combining changes, state, or artifacts; Skill-only installs retain a bundled resolver when the CLI is unavailable.
- **Native baseline evaluation**: Aligns Native to the same 16 tasks and repetitions as the 0.4.0 baseline, pairs by `task_id + repetition`, and corrects multi-call duration totals. Wave A–F fixtures and validators measure mechanical contracts separately from real-model performance.
- **Selectable eval suites**: Adds `comet eval --suite local|langsmith`. Local remains the default. LangSmith reuses the same task corpus and scoring logic while the CLI selects the tracing-aware runner, report directory, and Claude Code tracing plugin.
- **Unified workflow safeguards**: Installs one cross-workflow Rule per platform and one Router where Hooks are supported. Shared current-change selection routes one atomic multi-file write to at most one Guard, avoids Native/Classic double blocking, and migrates released Classic selection and legacy managed Hook/Rule assets.

### Changed

- **Supported Node.js runtime**: Comet now requires Node.js 22 or newer.
- **Aligned eval efficiency metrics**: Native comparison reports now recompute paired model starts and resumes, turns, tool calls, cumulative duration, tokens, cost, and context pressure from raw traces, using the strict success intersection so missing telemetry or incomplete tasks cannot inflate efficiency.
- **Selectable project workflows**: Project-scope `comet init` explains and offers Native, Classic, or both. New projects default `/comet` to Native and store Native artifacts under `docs/comet/`. `--workflow both` installs two independent capability sets that share `.comet/config.yaml` without merging state, and completion summaries show only the work directories actually initialized.
- **Explicit Native state files**: Active and archived Native changes now use `comet-state.yaml` while remaining independent from Classic `.comet.yaml` schemas and lifecycles.
- **Partitioned status and recovery**: `comet status` reports the configured entry and separate Native, Classic, and unmanaged OpenSpec changes. `comet resume-probe` returns a workflow-aware v2 result, resumes only through `/comet-native` or `/comet-classic`, and fails closed on malformed configuration or state.
- **Configurable Ambient Resume**: `.comet/config.yaml` exposes one `ambient_resume` switch for Native and Classic. Generated comments follow the init/update language, and older configuration remains enabled by default.
- **Classic configuration namespace**: Moves Classic `language`, `context_compression`, `review_mode`, and `auto_transition` defaults into the `classic:` block in `.comet/config.yaml`. `comet init` and `comet update` migrate legacy top-level values while preserving explicit values already stored in the new format.
- **Explicit npm self-update**: Current-project `comet update` refreshes managed assets without changing any npm installation unless `--self-update` is passed. Explicit `--scope global` also uses the current-project asset scope deterministically and never updates the package implicitly; broader updates can opt out with `--skip-self-update`. Self-update compares full semver including prereleases, refuses downgrades, validates the exact candidate CLI in isolation, and attempts to restore the exact previous version after an installation failure.

### Fixed

- **Init and update failure reporting**: `comet init` and `comet update` return non-zero exits for incomplete work, preserve npm diagnostics in JSON, identify failed and unattempted batch projects, and avoid success banners or hidden cleanup failures after component errors.
- **OpenSpec setup diagnostics**: Setup explains when an installed OpenSpec CLI is too old and an optional upgrade was not selected instead of reporting that the CLI is unavailable.
- **Windows interactive evals**: Interactive Docker evaluations prefer Git Bash, preserve container prompt paths, count real driver turns, and distinguish user decisions, completion, and environment failures correctly.
- **Parallel eval coordination**: Windows xdist workers wait for the shared Docker build lock instead of reporting normal concurrency as a resource deadlock.
- **npm publish cache pruning**: Package preflight excludes nested and platform-specific pytest caches before filesystem inspection, so unreadable local Eval byproducts cannot block publishing or enter the npm package.
