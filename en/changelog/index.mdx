...
Beta 12 simplifies Classic root migration and Dashboard discovery while narrowing Native verification evidence and repair interactions.
...
- **Native workflow guidance**: Native identifies the current change and phase before loading the corresponding reference, and always completes Shape classification, silent-assumption checks, and shared-understanding confirmation before implementation. Project and change commands are organized by task to reduce unrelated guidance.
...
- **Explicit Comet Skill routing**: When the host has already loaded `/comet`, the entry follows project configuration into Native or Classic and preserves the original request instead of reassessing relevance or selecting a workflow by task size.
