...
<Update label="0.4.0-beta.6" description="2026-07-17" rss={{ title: "Comet 0.4.0-beta.6", description: "Release notes for 2026-07-17" }}>
### Added

- **Comet Native workflow**: Adds a self-contained workflow for strong coding models with one-question decision clarification, one continuously advancing Skill, configurable Comet-owned artifact roots, a default change language, complete target specifications, checkpoints, and bounded paginated status—without OpenSpec, Superpowers, or any external Skill dependency.
- **Evidence-bound autonomous execution**: Adds content-addressed implementation scopes, Acceptance IDs, verification envelopes, optional built-in read-only check receipts, stale-evidence fallback, partial-scope authorization, and repair episodes. Models may implement and repair autonomously while the Runtime stops repeated failures without real progress.
- **Recoverable Native safety boundary**: Adds schema migration, revision/CAS, protected Run and file I/O, process-free workspace identity, current-root conflict inspection, two-step Archive, exactly-once transaction recovery, safe root moves, explicit stale-lock takeover, bounded evidence retention, and a read-only Dashboard projection backed by shared Runtime facts.
- **Stable workflow entries**: Adds permanent `/comet-native`, `/comet-classic`, and `comet workflow resolve` entry points. `/comet` chooses exactly one configured workflow without converting or combining either side's changes, state, or artifacts.
- **Aligned Native evaluation**: Runs Native against the same 16 tasks and repetitions as the 0.4.0 baseline, pairs by `task_id + repetition`, and corrects multi-call duration totals. Wave A–F fixtures and validators measure mechanical contracts separately from real-model performance.

### Changed

- **Native-first project setup**: `comet init` defaults genuinely new projects to self-contained Native, supports explicit `--workflow native|classic` and Native `--root`, and preserves projects with clear Classic state. Native setup installs no OpenSpec, Superpowers, CodeGraph, Classic rules/hooks, or `.comet` state.
- **Partitioned status and recovery**: `comet status` separates Native, Classic, and unmanaged OpenSpec changes. `comet resume-probe` resolves the default workflow first, inspects only that side, and fails closed on damaged configuration or workflow state.
