...
- **Evidence-bound autonomous execution**: Adds content-addressed implementation scopes, Acceptance IDs, immutable verification report snapshots, optional built-in read-only check receipts, stale evidence, partial-scope authorization, and repair episodes. Repeated failures stop without real progress; only genuine implementation-scope change opens new repair progress.
