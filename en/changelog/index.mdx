...
Beta 12 simplifies Native verification and recovery, unifies current-change routing, and improves Classic root migration and Dashboard discovery.

### Changed

- **Classic root migration**: `comet classic root move docs --dry-run` reports current state, conflicts, and blockers directly without exposing a plan ID. `--apply` migrates the complete `openspec/` tree, including active and incompletely archived changes, without requiring a plan ID. Projects already using `docs/openspec/` receive an explicit no-op result.
- **Native workflow guidance**: Native identifies the current change and phase before loading phase-specific references. Shape completes information classification, silent-assumption checks, and shared-understanding confirmation before implementation. New user decisions discovered during Build pause implementation and require fresh confirmation. A `blocked` continuation enters a recoverable branch instead of indicating task completion.
- **Native verification and repair**: A passing Verify transition runs and binds the built-in required check automatically. Acceptance evidence may come from manual observations or Runtime-executed project commands, and the Runtime derives repair gaps from failed evidence. While an override remains, the Agent must provide a concrete new repair hypothesis; exhausted override or verification budgets require a user decision.

### Fixed

- **Classic Dashboard discovery**: Dashboard discovers the repository root when launched from a nested directory, then reads Classic changes from the configured legacy or docs layout through the built CLI. Collection failures are shown as errors instead of an empty workspace.
- **Comet entry routing**: The root `/comet` entry applies only to an explicit invocation or a request to use Comet without choosing Native or Classic. Once loaded, it selects exactly one permanent entry from project configuration instead of reconsidering the workflow by task size.
- **Stale current selection**: When a selection points to a missing or archived change, routing reevaluates active changes by count: zero candidates do not block ordinary work, exactly one is inferred read-only, and multiple candidates require an explicit selection.
- **Native baseline recovery guidance**: Incomplete baseline diagnostics now include `native.snapshot.max_files` alongside total-byte and duration budgets, so file-count truncation points to the relevant configuration.

### Removed

- **Native cryptographic review system**: Removed controller trust, signing identities, implementation attestations, independent-review and waiver receipts, and their CLI and Eval handoff paths. Native verification now relies on complete acceptance evidence and the built-in required check bound to the current revision, contract, scope, snapshot, and artifacts.
- **Redundant Native CLI inputs**: Removed the `comet native list` alias, manual-receipt `--responsible` and `--confirmed`, and caller-supplied `next --receipt`, `--evidence-receipt`, `--failure-category`, and `--failed-check`. Use unnamed `status` for discovery and record acceptance receipt references in `verification.md`.
