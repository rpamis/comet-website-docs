...
<Update label="0.4.0-beta.9" description="2026-07-23" rss={{ title: "Comet 0.4.0-beta.9", description: "Release notes for 2026-07-23" }}>
### Changed

- **Sequential Native clarification**: Sequential now re-traverses user-visible outcome branches after every answer, asks only one most-upstream decision per round with a recommendation and impact, and audits parsing and boundary behavior with counterexamples. Every product behavior in the final summary must trace to the request, a confirmed answer, or an applicable published contract. A separate final shared-understanding confirmation is required before Build, and the Runtime blocks a Sequential transition without `--confirmed`. Batch question grouping and progression remain unchanged.

### Fixed

- **Task-specific eval simulators**: Interactive evaluations add controller-owned per-decision reply queues for exact multi-round answers, while still preferring task-private simulated-user prompts and removing those files before the first subject-model call. Queue exhaustion fails closed instead of allowing the generic simulator to invent additional decisions.
