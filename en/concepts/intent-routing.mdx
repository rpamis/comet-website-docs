...
description: "Understand how the Classic route recognizes intent, reads state, and selects full, hotfix, tweak, or recovery."
---
...
| `tweak`        |Medium changes using OpenSpec, with the delta spec as a formal artifact| `comet-tweak`          |no|
...
- **`name`**: (Intent type) `start_change`/`fix_bug`/`make_tweak`/`resume_change`/`ask_question`/`unknown`
...
- **`/comet-tweak`**: **tweak-only** OpenSpec action path for a medium change whose scope is already confirmed.
...
- [Core scripts overview](/en/scripts/overview) — how scripts such as `comet-intent.mjs` collaborate
