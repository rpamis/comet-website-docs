---
title: "How Comet decides which flow to route to"
...
- **`/comet-classic`**: the permanent internal entry of Classic; the routing context decides between `full`/`hotfix`/`tweak`/`resume`. It is usually reached by forwarding from `/comet`.
...
5. **Insufficient evidence → `ask_user`.** Rather than guess a route, Comet asks.
