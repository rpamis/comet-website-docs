...
description: 'After /comet selects Classic, learn how internal routing recognizes intent, reads state, and selects full, hotfix, tweak, or resume.'
---

Users type `/comet` plus a request. After project configuration selects Classic, the internal `/comet-classic` entry decides which Classic profile applies using **intent recognition**, **slot extraction**, and actual file state.
...
<p align="center">
  <img
    ...
    alt="Xiaoyu checks the utterance, repository state, and risk signals on a routing-context triage board before selecting the final route"
    width="800"
  />
</p>
...
<Note>
  In one sentence: routing context makes <code>/comet-classic</code> routing{' '}
  <strong>explainable, reviewable, and recoverable</strong> instead of a black-box guess.
</Note>
...
<Warning>
  Risk signals take precedence over casual wording. If you ask for a hotfix but the request changes
  a public API, Comet stops at <code>ask_user</code> instead of silently allowing a risky
  lightweight path.
</Warning>
