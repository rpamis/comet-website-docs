...
<Accordion title="comet status doesn't show my change">
  ...
  <Accordion title="Can I edit .comet.yaml by hand">
    User-visible fields (workflow, phase, build_mode, etc.) should in principle flow through `/comet-classic` and the phase guards — do not hand-edit `phase`. Machine-owned Run fields (in `.comet/run-state.json` or `.comet/runs/<run-id>`) must never be hand-edited. For troubleshooting you can use the `comet-state` command. See [State management](/en/concepts/state-management).
  </Accordion>
