...
`review_mode` controls when Classic build dispatches automatic code review and how many repair rounds are allowed. `off` dispatches no reviewer; `standard` reviews risky tasks and runs one final lightweight review; `thorough` reviews every task and runs one final full review.
...
(Risk-triggered review per task) processing, rather than no review at all. This default applies uniformly to the init
template, the runtime fallback, and the compatibility path for older versions.
</Note>
