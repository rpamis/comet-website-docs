...
Comet creates a series of intermediate artifacts during the five-stage `/comet` flow. These working files support context compression, state recovery, evidence recording, and phase handoff, while proposal/design/tasks/code remain the final deliverables.
...
- At this stage (`implementing`/`task-review`/`checkoff`/`done`/`blocked`)
...
If the user chooses to accept the bias, the acceptance reason and its impact are recorded in the verification artifacts. When archiving, the Design Doc is annotated `archived-with` through the normal flow.
...
|Design Doc (if there are any deviations)|Acceptance reason recorded in the verification artifacts|The implementation deviation has been confirmed, and the main spec prevails|
