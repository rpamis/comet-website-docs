...
Comet creates intermediate artifacts so the Classic workflow can survive context loss, phase transitions, model switches, and later review. These files are not noise; they are the evidence trail that lets `/comet-classic` resume from facts instead of memory.
