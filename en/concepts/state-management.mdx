...
| Allow implementation dirs to be writable in any phase       | `hook.allow_paths` in `.comet/config.yaml`            | Defaults to empty; `.comet`, artifact roots, openspec/superpowers roots cannot be allowed |
...
| `hook.allow_paths` | Project-relative directory list (default empty) | Project-relative directories the Agent may write during guarded phases; prefix-matched, protected areas cannot be allowed. See [Native configuration](/en/native/configuration#hook-write-allowlist) |

`hook.allow_paths` is a write-allow policy shared by Native and Classic: when empty (default), writes inside the project (outside artifact areas) follow each workflow's phase rules; once configured, writes to matching directories and their subdirectories pass through directly. `.comet`, artifact roots, and the openspec/superpowers roots are always protected and cannot be allowed through it. See [Native configuration — Hook write allowlist](/en/native/configuration#hook-write-allowlist) for full details.
