...
| `verify_failures`      | Integer (machine-owned)    | Consecutive verification failure count. `verify-fail` increments it; `verify-pass` or `archive-reopen` resets it to `0`. At 3, the next failure requires a retry-limit decision |
