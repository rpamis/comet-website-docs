...
The following `classic.*` fields belong only to Classic and are snapshotted into new Classic changes:
...
<Info>
  Legacy flat Classic configuration still lets <code>/comet</code> enter Classic through the legacy fallback, but Classic Runtime reads project defaults only from the <code>classic:</code> block. Run <code>comet init</code> or <code>comet update</code> to migrate old values into the new block.
</Info>
...
When a new Classic change is created, Comet snapshots project-level `classic.language` into `openspec/changes/<name>/.comet.yaml`. OpenSpec proposal, design, tasks, Superpowers design/plan artifacts, verification reports, and archive notes follow this configured language instead of guessing from whichever request triggered the workflow.
