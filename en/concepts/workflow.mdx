...
**Write allowlist**: By default, writes inside the project (outside artifact areas) are constrained by the Classic phase guard. If an implementation directory must be writable in any phase, configure the shared `hook.allow_paths` in `.comet/config.yaml`; see [Classic configuration — Hook write allowlist](/en/classic/configuration#hook-write-allowlist).
...
## Skills used by Classic

The five-phase workflow is driven by these built-in Skills, installed into the platform directory by `comet init`:

| Skill                            | Purpose                                      |
| -------------------------------- | -------------------------------------------- |
| `/comet-classic`                 | Main Classic entry; detects state and routes to the current phase |
| `/comet-open`                    | open phase: create an OpenSpec change        |
| `/comet-design`                  | design phase: produce a deep technical design |
| `/comet-build`                   | build phase: plan and execute                |
| `/comet-verify`                  | verify phase: verify the implementation      |
| `/comet-archive`                 | archive phase: archive the change            |
| `/comet-hotfix` / `/comet-tweak` | Lightweight presets                           |

These Skills ship with Comet and are installed into the platform directory during Classic initialization. Normally invoke `/comet`; when configuration selects Classic, the internal `/comet-classic` entry detects state and invokes the appropriate phase Skill.
