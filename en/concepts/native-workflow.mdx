...
Comet Native adopts that principle without introducing a Loop Engineer role or a fifth phase. It bounds Native Loop to Build and Verify. One implementation, one commit, or one Agent turn is only an iteration; work is complete only when current evidence supports every acceptance condition under the current contract.
...
For example, an Agent may finish one Build iteration and then discover in Verify that an edge case from the complete target specification was not implemented. Native records that omission as an unsatisfied acceptance item and returns it to Build. The Agent fills the gap in the next iteration and runs the complete verification again. Native Loop continues while repairable specification gaps remain and no stop boundary has been reached; finishing one coding pass is not completion.
