...
When a user decision remains, Native asks one upstream question at a time with a recommendation, rationale, and option impact. A `[blocking]` question prevents Build. Changes to durable behavior also receive a complete target specification for each capability rather than an incremental patch alone.
