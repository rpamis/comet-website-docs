...
description: 'A self-contained requirements workflow for strong models: concrete outcomes, lightweight methods, and recoverable acceptance.'
---

Native assumes the model can investigate, implement, and choose proportional tests, so it does not mandate fixed design, planning, TDD, or review methods. It does not, however, accept a self-reported completion verdict. Runtime runs necessary checks, dispatches a fresh read-only Verifier, and persists user-readable state that can recover across devices.
...
### Build: let the strong model implement
