...
- **pass@k/pass^k** : distinguish between ** upper capability ** (at least one success out of k attempts) and ** lower reliability ** (all success out of k attempts). Calculated based on multiple repeated runs (`--count N`), likewise used for diagnosis only.
