...
<Note>
  This is a historical July 2026 experiment using the beta16/legacy Native artifact protocol. It
  preserves the model-performance comparison from that run; it is not the beta17 file-layout,
  Verify, or recovery contract. For beta17, use [Native workflow](/en/concepts/native-workflow),
  [Artifacts and state](/en/native/artifacts-and-state), and the current [Changelog](/en/changelog).
</Note>

This experiment asks a narrower question: when the model is already capable (Fable 5, GPT-5.6 tier), can Comet Native preserve requirements, state, verification, and archive evidence while running a lighter workflow than 0.4.0 Classic?
...
These results can be read from two angles. Both modes reach `pass@3 = 100%`, so every task succeeds at least once in three attempts. Native has the higher `pass^3`, which indicates better repeated-run reliability: 14 of 16 tasks pass all three Native runs, compared with 12 of 16 for 0.4.0 Classic.
...
<Warning>
  ...
  controlled. The duration difference is only directional evidence, not a controlled causal
  estimate.
</Warning>
...
This shows the different failure patterns of the two modes. Classic has more workflow dependencies, so a run can fail at the main Skill, a phase, OpenSpec, or a dependency Skill. Native has fewer workflow steps but keeps a terminal-evidence check: completed code without complete brief, specification, or verification evidence is still not a strict pass.
...
The aligned experiment supports a few conclusions:
...
It does not support the conclusion that Classic is obsolete. Native serves strong models such as Fable 5, GPT-5.6, and peers: such models can reason through complex code but still benefit from clarification, state, verification, and archive evidence. Classic continues to serve models below that tier, and tasks that need finer phase guidance and stronger process constraints.
