---
title: "Evaluation: The Compass of Skill Evolution"
description: "The scientific nature behind rubric, pass@k, and pass^k, evaluating how to determine the evolution direction of Skill, and why it should be a priority ability to be established."
---

<Info>
This page doesn't explain how to type commands. If you want to run an assessment directly, check out [Quick Start ](/en/eval/quickstart)]. This page answers a more fundamental question: Why should assessment be given top priority among all abilities
</Info>
...
When subjective judgment is indeed required (such as "the substantive depth of the product"), Comet offers an optional LLM-as-judge: allowing the judge model to read the product and score it. judge is constrained to a fixed dimension (`task_completion`/`output_quality`/`instruction_adherence`), and each scoring reason is required to quote specific content, thereby limiting subjective judgment within an auditable range.
...
pass@k/pass^k data that needs to be run repeatedly. Comet repeats each task N times with `--count N`, generating N independent pass/fail results. When the number of runs is insufficient (less than 2 times), the report will prompt to use `--count 5`. A single run can only calculate pass@1/pass^1, and the metric k>1 needs to be repeated multiple times.
...
| --- | --- |
|The evaluation should be systematic and large-scale|eval harness + `--count N` repeat run + multi-task /profile|
