...
description: "Define an eval task from zero: task.toml, instruction.md, validation scripts and Dockerfile, configure treatment, profile and interaction modes, and let comet eval evaluate your own skills."
---

`comet eval` is ready to use out of the box to evaluate built-in tasks and any local Skill - assessing your own Skill only requires one command, `comet eval ./your-skill --html` (see [Quick Start ](/en/eval/quickstart)). But when you want to evaluate how your Skill performs in your own scenario, you need to define a custom task. This article will guide you through it step by step: from defining a task, to configuring an evaluation profile, and then to generating a report.
...
## A panoramic view of the eval configuration
...
|Field|Function|
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `profile`                  |Select rubric: `generic` (7-dimensional)/`comet-workflow` (9-dimensional)/`authoring-skill` (11-dimensional). For detailed dimensions, please refer to [Scoring Metrics and Dual-Agent Evaluation ](/en/eval/scoring)|
...
This is a task instruction for the agent, supporting template variables such as zxQ0 and QXZ:
...
flowchart LR
    ...
    Q1 -->|"Comet five-stage workflow"| CW["Comet-workflow <br/> 9-dimensional, multi-round"]
...
# 3. Real Run (Inject your Skill treatment)
...
1. **`pass@1` and `pass^k`**: upper capability versus the reliability floor. See [Scoring criteria](/en/eval/scoring#core-metrics-passk-and-passk).
...
The manifest will automatically select `authoring-skill` profile, inject recommended tasks and quality access control. The complete format of the manifest can be found in [Overview of the Evaluation System ](/en/eval/overview#eval-manifest)

## Next step

- [Scoring metrics and Dual-Agent evaluation ](/en/eval/scoring) - Dimensions, weights and pass@k/pass^k of three sets of rubric
- Overview of the Evaluation System ](/en/eval/overview) - manifest format, profile System and task System
