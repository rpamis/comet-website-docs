...
If the model never invokes the Skill, evaluation is measuring the bare model instead of the Skill. A LangChain practical article <sup>[2]</sup> highlighted this: even with explicit prompts requiring Skill calls, invocation rates were only around 70%.
...
The LangChain article emphasizes pairing Skill evaluation with observable experiment platforms like LangSmith. These platforms score each run and capture Agent actions (read files, create scripts, call Skills) for diagnosis. This is why Comet eval includes LangSmith integration.
