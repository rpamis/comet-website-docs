...
<AccordionGroup>
  ...
  <Accordion title="What is the difference between comet eval and comet skill check">
    `comet eval` evaluates a Skill's product capability and produces pre-release evidence. It can read `comet/eval.yaml` (a `/comet-any` generated full package), or take any local Skill directory directly (defaulting to the `generic-skill-smoke` smoke; publish evidence requires the full package). `comet skill check` checks whether a specific Skill run is missing artifacts or state, does not execute model tasks, and does not produce publish evidence (reads `comet/checks.yaml`). Publish readiness only requires `comet eval`. See [Runtime check](/en/eval/runtime).
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Should I use --manifest or --skill-path">
    To evaluate any local Skill directory use `--skill-path` (just pass the directory; it defaults to `generic-skill-smoke`, and `--quick` is optional) — this is the default entry point. To evaluate a `/comet-any` generated full package (with `comet/eval.yaml`) use `--manifest`, which runs the full profile task set and whose result is the only publish evidence. To enter publish readiness you must use `--manifest`. The two are mutually exclusive.
  </Accordion>
  ...
  <Accordion title="What is --quick and what is the default task">
    `--quick` is used with `--skill-path`; it selects the `generic-skill-smoke` task by default. In fact, passing a directory already defaults to this path, so the `--quick` flag is optional. It is a low-cost smoke and is not equivalent to complete pre-release evidence — you still need the manifest path when preparing to publish.
  </Accordion>
