...
Select a CodeBuddy custom model with `--model <model-id>`. `CODEBUDDY_MODEL` sets the default model and `CODEBUDDY_BASE_URL` overrides the request endpoint. Do not reuse Claude Code's `ANTHROPIC_BASE_URL` directly for CodeBuddy, especially an `/anthropic` or `/messages` endpoint; the model ID must be accepted by CodeBuddy/the provider instead of using a Claude-specific suffix.
