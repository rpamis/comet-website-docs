...
description: 'Prepare the CLI, authentication, endpoint, and model settings for Claude Code, Codex, or Qoder before running comet eval.'
---

`comet eval` starts the selected Agent inside an isolated Docker container. Agent login and model protocols are not interchangeable: Claude Code can use an Anthropic-compatible gateway, Codex requires an OpenAI Responses API provider, and Qoder requires its own login session or Personal Access Token.
...
<Info>
  Beta18 does not yet document CodeBuddy as a guaranteed working startup path. Its CLI configuration
  and current Eval adapter are still being validated, so do not assume Claude Code settings will
  work for CodeBuddy.
</Info>
