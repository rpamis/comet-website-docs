---
title: "Quickstart: Evaluating a Skill"
description: "From your first run and evaluation configuration to reading reports, learn how to evaluate a local Skill with comet eval."
---

All you need is a local directory containing `SKILL.md` to evaluate a Skill with `comet eval`. Even if you have not written evaluation cases in advance, **Eval can automatically generate 2–4 evaluation cases from the Skill content and run them**. This page follows the practical order: get one run working, choose an Agent and evaluation backend, configure tasks, generate reports, and diagnose failures.

## What You Need

### Required

- `@rpamis/comet` installed. The npm package includes the eval harness; you do not need to clone the Comet repository.
- A local Skill directory containing `SKILL.md`.
- For a real evaluation run: `uv`, Python 3.11+, Docker, the selected Agent CLI, and its model credentials.

`--collect` only performs static discovery and configuration checks. It does not start the Agent, Docker, plugins, credentials, or network requests, so you can check a Skill before preparing the full model environment.

### Supported Evaluation Agents

`comet eval` uses `claude-code` by default. The subject Agent, user simulator, and optional Judge can use the following Agents:

| Agent         | Suitable for                                    | Common credentials                            |
| ------------- | ----------------------------------------------- | --------------------------------------------- |
| `claude-code` | Default choice; runs the Skill with Claude Code | `ANTHROPIC_API_KEY` or `ANTHROPIC_AUTH_TOKEN` |
| `codex`       | Codex CLI or OpenAI-compatible models           | `OPENAI_API_KEY` or `CODEX_API_KEY`           |
| `qoder`       | Qoder CLI                                       | `QODER_PERSONAL_ACCESS_TOKEN`                 |
| `codebuddy`   | CodeBuddy CLI or API                            | `CODEBUDDY_API_KEY` or `CODEBUDDY_AUTH_TOKEN` |

You can also register a custom Agent adapter. Putting an executable on `PATH` is not enough to enable it; place the adapter here:

```text
~/.comet/eval/adapters/<agent-id>/adapter.yaml
```

If an evaluation finishes instantly without a real run, Docker, the Agent CLI, or model credentials are usually not ready. In these cases the harness normally skips instead of fabricating a successful run.

## First Run

Start with a model-free pre-check, then run a low-cost smoke test, and finally run the configured or generated task set:

```bash
# 1. Check Skill, manifest, and task discovery
comet eval ./my-skill --collect

# 2. Run the fixed low-cost smoke task
comet eval ./my-skill --quick --html

# 3. Run the configured or automatically generated tasks
comet eval ./my-skill --html
```

When there is no `comet/eval.yaml`, or the manifest has neither `evaluation.tasks` nor `recommendedTasks`, a normal run **automatically generates 2–4 evaluation cases** from the Skill content, freezes and caches them, and then executes them. You do not need to write tasks before starting an evaluation. `--quick` does not use these generated cases; it always runs `generic-skill-smoke`, which checks that the Skill can be injected, invoked, and produce `result.md`.

### Passing the Target

You can pass a directory, a direct `SKILL.md`, or a manifest as the target:

```bash
# Skill directory: automatically discovers comet/eval.yaml if present
comet eval ./my-skill --collect

# Direct SKILL.md
comet eval ./my-skill/SKILL.md --html

# Direct manifest
comet eval ./my-skill/comet/eval.yaml --html
```

Without a manifest, Comet creates the basic configuration in memory and does not rewrite the Skill source. If the Skill is outside the current repository, use `--project` to specify the project directory where runtime state and reports are stored:

```bash
comet eval ./my-skill --project ./my-project --html
```

## Configure the Evaluation Agent and Judge

The simplest way to choose the subject Agent is on the command line:

```bash
comet eval ./my-skill --agent codex --model subject-model --html
```

You can also create `comet/eval.yaml` under the **root of the Skill being evaluated** to configure the default Agent. The layout is:

```text
my-skill/
├── SKILL.md
└── comet/
    └── eval.yaml
```

When you run `comet eval ./my-skill`, Comet automatically discovers `./my-skill/comet/eval.yaml`. You can also pass the file directly as the target:

```bash
comet eval ./my-skill/comet/eval.yaml --collect
```

Configure the default Agent in `eval.yaml`. CLI options take precedence over the manifest:

```yaml
execution:
  agent: codex
  model: subject-model
  baseUrl: https://subject.example/v1

judge:
  agent: claude-code
  model: judge-model
  baseUrl: https://judge.example/v1
```

The subject Agent and the LLM-as-Judge can use different Agents, models, API endpoints, and credentials. The Judge does not silently inherit the subject Agent's model or credentials; when you enable it, provide a separate Judge configuration.

## Choose an Evaluation Backend

Use `--suite` to select the evaluation backend. All three backends use the same target, task, and Agent configuration; they differ in whether results are synchronized to an external evaluation service:

| Backend     | Command                                          | Result location and purpose                                                                                            |
| ----------- | ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `local`     | `comet eval ./my-skill --html`                   | Default backend; creates local reports for everyday development                                                        |
| `langsmith` | `comet eval ./my-skill --suite langsmith --html` | Creates a local report and synchronizes traces, scores, and experiment data to LangSmith                               |
| `langfuse`  | `comet eval ./my-skill --suite langfuse --html`  | Creates a local report and synchronizes task traces, rubric scores, pass metrics, and experiment summaries to Langfuse |

Langfuse example:

```bash
LANGFUSE_PUBLIC_KEY=pk-lf-... LANGFUSE_SECRET_KEY=sk-lf-... \
  comet eval ./my-skill --suite langfuse --html
```

If you only want to check task and configuration discovery, any backend can be used with `--collect`:

```bash
comet eval ./my-skill --suite langfuse --collect
```

`--collect --suite langfuse` does not initialize an SDK, connect to the network, or download plugins.

## Report Types

Evaluation state and reports are written to the following directory under the target project by default:

```text
.comet/eval/runs/<experiment-id>/
├── summary.md       # Markdown summary; always generated
├── summary.html     # HTML summary; generated with --html
├── metadata.json    # Metadata for this experiment
├── events/          # Agent runtime events
├── raw/             # Raw output
├── reports/         # report.json for each run
└── artifacts/       # Task artifacts and validation evidence
```

Without `--html`, the evaluation still generates `summary.md`. To generate HTML, run:

```bash
comet eval ./my-skill --html
```

You can also customize report outputs with `--report-config <path>` or `COMET_EVAL_REPORT_CONFIG`. The CLI prints `Experiment` and `Report path`; use those values when locating a specific report.

When reading a report, start with three questions:

1. Did the evaluation pass?
2. Is the failure attributed to `harness`, `workflow`, `task`, or `model`?
3. Are expected artifacts missing, or are token, cost, or duration values unusual?

In general, `harness` means an environment or dependency problem, `workflow` means the Skill flow did not meet expectations, `task` means the task definition or validation condition needs attention, and `model` means model behavior or invocation was unstable.

## Customize Tasks

Tasks are selected in this order:

1. The task specified with `--task`.
2. `generic-skill-smoke` selected by `--quick`.
3. `evaluation.tasks` in the manifest.
4. `recommendedTasks` or a task package referenced by `source`.
5. **2–4 evaluation cases automatically generated and cached from the Skill content** when no usable task is available.

If the automatically generated tasks do not fit your Skill well, create or edit `comet/eval.yaml` under the **root of the Skill being evaluated**, and declare an inline task under `evaluation.tasks`. For example:

```yaml
evaluation:
  tasks:
    - name: writes-summary
      prompt: Create summary.md with the requested result.
      expect:
        files:
          - summary.md
        contains:
          summary.md:
            - "# Summary"
```

You can also use `source` to reference a task package inside the Skill package that contains `task.toml` and `instruction.md`. Tasks can check files, text, JSON, or command results; the task workspace and expected artifacts must remain inside the allowed Skill package or evaluation workspace.

After configuring a task, confirm that it can be discovered before running the real evaluation:

```bash
comet eval ./my-skill --collect
comet eval ./my-skill --task writes-summary --html
```

Only continue to [Configuring Evaluations and Custom Tasks](/en/eval/configuration) when you need a custom Docker environment, validation scripts, a profile, or a treatment.

## Recommended Evaluation Flow

```text
1. Prepare a directory containing SKILL.md
2. comet eval ./my-skill --collect
3. Fix path, manifest, or task discovery issues
4. comet eval ./my-skill --quick --html
5. Configure an Agent, Judge, suite, or custom task as needed
6. comet eval ./my-skill --html
7. Read .comet/eval/runs/<experiment-id>/summary.md or summary.html
```

## `comet eval` vs. `comet skill check`

- `comet eval` evaluates whether a Skill can complete a real task as a product capability and produces a reviewable report.
- `comet skill check` only checks whether a particular Skill run is missing runtime artifacts or state; it does not rerun model tasks.

Use `comet eval` when you want to know whether a Skill can achieve its goal. Use `comet skill check` when you are troubleshooting a Skill Run that has already happened.

## Further Reading

| Topic                                                         | Suitable scenario                                                    |
| ------------------------------------------------------------- | -------------------------------------------------------------------- |
| [Evaluation System Overview](/en/eval/overview)               | Understand tasks, profiles, scoring, and the execution model         |
| [Scoring Metrics and Dual-Agent Evaluation](/en/eval/scoring) | Understand rubric, pass@k/pass^k, and multi-turn Agent interaction   |
| [Eval harness](/en/eval/harness)                              | Troubleshoot the harness, environment variables, and suite internals |
| [Reading Evaluation Reports](/en/eval/reports)                | Analyze reports and failure attribution in depth                     |
| [comet eval command](/en/cli/eval)                            | See the complete CLI options and troubleshooting reference           |
