...
Simply placing the executable file on the PATH will not automatically enable it.

If the evaluation ends instantly and does not actually run, it is usually because Docker, Agent CLI or model credentials are not ready. harness will mark the sample as skipped in these cases to avoid false successful runs.
...
The assessment status and reports are written by default to the items to which target belongs:
...
The meaning of failure attribution is as follows: `harness` usually indicates environmental or dependency issues, `workflow` indicates that the Skill process has not met expectations, `task` indicates problems with task definitions or validation conditions, and `model` indicates unstable model behavior or calls.
