...
<Note>
  A generated <code>comet/eval.yaml</code> can keep <code>&lt;current-bundle-hash&gt;</code>. Before collect or run, <code>comet eval</code> calculates the current Bundle draft hash and creates a temporary manifest. It does not modify the source Bundle.
</Note>
...
If the placeholder cannot be resolved, the error explains whether the manifest has left its original Bundle, `bundle.yaml` is missing, or the draft cannot be loaded. Repair the Bundle structure instead of replacing the placeholder with a guessed hash.
