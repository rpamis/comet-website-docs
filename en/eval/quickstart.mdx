...
<Info>
  Custom Agent adapters are an advanced source-maintainer capability. Ordinary users do not need to
  register an adapter; use one of the built-in Agents above. Only extend the Eval harness when you
  need the [advanced configuration](/en/eval/configuration).
</Info>

If you maintain the source repository, you can also register a custom Agent adapter. Putting an executable on `PATH` is not enough to enable it; place the adapter here:
