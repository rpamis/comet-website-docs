...
description: "In-depth understanding of Comet eval 's metric drive evaluation: rubric multidimensional scoring criteria, pass@k and pass^k (upper capability vs lower reliability), weighted scoring formula, and the process of automatic interaction between two agents to complete the evaluation."
---
...
Comet's eval does not merely provide "pass/fail", but rather ** index-driven ** evaluation: it quantifies the performance of Skill in various quality dimensions using **rubric multi-dimensional scoring **, and distinguishes the "upper limit of capability" from the "lower limit of reliability" using **pass@k/pass^k**. For multi-stage workflow-type skills, the evaluation is accomplished through the automatic interaction of ** two agents ** - one runs the Skill under test, and the other simulates the user's response at the decision point.
...
## Overview of the Indicator System
...
The runtime evidence of Comet eval comes from the stream-json output by Claude CLI itself, not guessed from the terminal text, nor is it recalled and summarized by another model.
...
**gap (pass@k − pass^k) quantified instability **. The report will explicitly indicate this gap.
...
The relative path of `BENCH_SIMULATOR_PROMPT_FILE` is parsed from `eval/`, with the default value being `simulator-instruction.md`. The file will only be read if it exists. This enables you to change the user behavior profile (such as being more picky and asking for more clarification) to stress test the performance of the workflow under "difficult users" without having to harness the code.
