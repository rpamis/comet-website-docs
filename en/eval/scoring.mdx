...
| `comet-workflow` | 10 | 12.0 |`auto_user` (Multi-wheel)|The classic `/comet` five-stage|
| `authoring-skill` | 11 | 12.8 |`auto_user` (Multi-wheel)|Products of `/comet-any`|
...
| --- | --- | --- |
| `business_completion` |**2.0** (maximum|The baseline validator passes the proportion (whether the business task was done correctly)|
| `workflow_completion` | 1.0 |The completeness of the workflow's own steps|
...
| `output_quality` |Well-structured, complete, extraordinary and with traces of thought|It exists but the content is thin and barely meets the standards|Missing, empty, obvious stub|
...
# (Repeated runs are driven internally by the eval harness's pytest --count 5)
```
...
The loop has a maximum of `max_turns` outer round trips (comet-workflow is commonly 12, authoring-skill commonly 8). It ends early when a "Complete" signal is detected (`archive complete`, `workflow complete`, `all 5 phases`, and similar markers).
...
| **reps** |Repetition count (harness-internal `--count N`)|`--count 5` run each (task,treatment) five times|
