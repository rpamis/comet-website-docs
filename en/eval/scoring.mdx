...
<Note>
  <strong>Key distinction</strong>: rubric scores and pass@k/pass^k are all <strong>informational metrics</strong>, for diagnosis and comparison; the real <strong>pass/fail</strong> is determined by the task validators (<code>target_artifacts</code> existence + <code>test_scripts</code>). The definition of a "pass" for a run is <strong>zero task validator failures</strong> (<code>checks_failed == []</code>).
</Note>
...
Evaluates the classic five-phase workflow. **This rubric never produces a hard failure** (always returns an empty failure list).
...
| `skill_invocation` | 1.0 | Whether the comet entry and sub-Skills were called in order (6 core Skills; dispatcher called first gives +0.10 bonus, capped at 1.0) |
...
`--count N` (a pytest option) repeats each `(task, treatment)` combination N times, producing N independent pass/fail results. These N booleans are the input to pass@k/pass^k.
...
| `BENCH_SIMULATOR_PROMPT_FILE=<path>` | Environment variable (can be written into `eval/.env`) | All auto_user runs |
