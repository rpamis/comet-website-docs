...
After content enters a task, the Agent can report these results:
...
Application results affect later ranking, status promotion, content rewriting, or replacement. Delivery does not establish adoption: `comet task` feedback can include `--decision`, `--verification`, and `--verification-result passed|failed` alongside `--application` and `--outcome` to record the actual decision, verification command, and result. The Dashboard displays this evidence separately from delivery records.
