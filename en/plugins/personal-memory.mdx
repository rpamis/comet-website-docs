...
`remember` is for explicit long-term storage requests. `observe` records stable collaboration practices the Agent discovered; it must not capture task summaries, progress, command output, or test results. `forget` keeps rollback available by default. Explicit `--permanent` deletion clears the matching text, history, and associated state while retaining a tombstone that prevents automatic relearning. Other memories with different text remain, and surviving conflicts stay unresolved.
...
The first sync can connect to an empty repository or an existing memory branch while preserving remote content. Sync failures return an error and distinguish connection failures from merge conflicts. Resolve conflicts before retrying.
