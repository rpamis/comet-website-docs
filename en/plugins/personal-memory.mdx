...
|"Don't submit it this time."|It only works on the current request|The one-time authorization boundary should not be extended to long-term default behavior|
...
When you propose "remember", "Always be like this", or directly correct or forget a record, Comet uses a deterministic management path. Clear long-term requirements can directly enter the `proven` (confirmed) status and take effect in the next related task.
...
Both Personal Memory and Project Knowledge serve the Agent context, but they have different owners and sources of fact:
...
- No authorization to submit, push, delete or publish;
