...
Project Knowledge does not replace Rule. You can keep writing team directives and behavioral constraints in the Rule files or rule directories your target platform supports, such as `AGENTS.md` for Codex, `CLAUDE.md` for Claude Code, `.cursor/rules` for Cursor, and the Rule carriers other platforms provide. The host still loads this content according to its own scoping rules.
...
- Full-text and source-code search recall relevant candidates;
...
| Rule / Agent instructions | Prescribe how the Agent behaves in scope | Rule files or rule directories the target platform supports | The host loads and provides to the Agent |
| Hook / Guard | Observe, block, or adjust the flow when an action happens | Hooks, permission rules, Comet Guard | The host or Runtime |
...
Extra paths are resolved relative to the project root. The default corpus does not automatically include every Rule file or rule directory of the target platform. If you also want their Markdown as a knowledge source, add it explicitly through `include`.
...
Comet extracts candidate records from these events and re-checks their sources before writing. Candidates that need semantic induction and have not yet survived reuse first enter `trial`; content whose source can be checked directly or that was re-verified successfully can enter `proven` directly. A `trial` can also be promoted to `proven` after it helps a later task. Project Knowledge therefore grows with real work instead of depending on a one-shot, complete project description.
...
Context Director filters candidates by the current project, path, task, operation, phase, type, and state, then ranks them by combining full-text search, limited source search, project relationships, source status, and application results.
...
- The host keeps using the original Rule files and loading mechanism;
