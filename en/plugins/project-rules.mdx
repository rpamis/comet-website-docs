...
- Hook, linter, test, build and CI handle the actual rule checks and enforcement.
...
Project Policy saves engineering practices accumulated through actual work:
...
The Project Policy can enter the `enforced` (Verified Constraint) state only after the associated verification command currently exists and has run successfully. In the Native workflow these verification commands run as part of the Verify-phase checks (see [Verification and Repair](/en/native/verification-and-repair)), and a policy is promoted only when it actually passes. This status indicates that the project already has tools capable of checking this strategy, and the natural language text is still provided to the Agent as context.
...
> Project Knowledge provides understanding, rules provide instructions, hooks and engineering checks handle enforcement.
...
Additional paths are resolved relative to the project root directory. The default corpus range will not automatically include all Rule files or rule directories of the target platform. When it is necessary to use the Markdown as a knowledge source at the same time, please explicitly add it through `include`.
...
- Disabling the plugin stops learning, querying, policy validation, and Remote requests; disable it from the Dashboard plugin center, and it is enabled by default after initialization.
