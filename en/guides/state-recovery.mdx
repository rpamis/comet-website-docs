...
description: "Use comet status and comet doctor to diagnose bad /comet-classic routing, missing .comet.yaml files, malformed state, or missing evidence."
---

Normal Classic recovery only needs `/comet-classic`; see [resuming interrupted work](/en/guides/resuming-workflow). This page is for cases where `/comet-classic` routes to the wrong phase or the state looks damaged.
...
| --- | --- | --- |
| Session ended, context compacted, or device changed | Normal interruption | Run `/comet-classic`. |
...
| Build cannot enter verify | Missing `build_mode`, `isolation`, or `tdd_mode` | Return to build through `/comet-classic`. |
