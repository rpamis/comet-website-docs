...
New Classic projects use `docs/openspec/` by default. If the repository already has a root-level `openspec/`, Comet retains it as the `legacy` layout instead of moving existing changes during initialization or upgrade. To migrate from the legacy layout to `docs/openspec/`, see [Migrate Classic layout](/en/guides/classic-layout-migration).
