...
- **Commit**: the Classic artifact root (`docs/openspec/` for new projects or `openspec/` for retained legacy layouts), `docs/superpowers/`, `.comet/config.yaml`, and `.comet/skill-preferences.yaml`. These are sources of truth and must be committed for cross-device recovery. See [Artifacts and state](/en/native/artifacts-and-state) for Native artifact commit guidance.
