...
Upgrading does not move an existing Classic project's root-level `openspec/`. Inspect the active layout and review the dry-run report first:
...
The dry run reports current state, conflicts, and blockers without issuing a plan ID. Apply moves the complete `openspec/` tree, including active and incompletely archived changes, updates `classic.artifact_layout`, and rewrites Comet-managed references. Commit or stash existing work first. To keep the legacy layout, do not run the migration and do not substitute a direct configuration edit for it.
