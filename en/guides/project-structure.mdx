...
## Native workflow directories

Native separates user-readable facts from device-local execution state. A change root keeps only the files users read, sync, and archive:

The example below uses the default `native.artifact_root: docs`. If the project selects another project-relative root, replace the `docs/` prefix; the file responsibilities stay the same.

```text
docs/comet/
├── changes/<name>/
│   ├── brief.md
│   ├── comet-state.yaml       # portable state for sessions and devices
│   ├── specs/<capability>/spec.md
│   └── verification.md        # user report rebuildable from YAML
└── archive/<date>-<name>/     # archived changes keep the same readable artifacts
```

The project-level `.comet/` directory keeps configuration and the centralized Runtime:
...
`comet-state.yaml` is Native's recovery boundary. `state.json`, logs, locks, and transactions are device-local execution data and cannot replace YAML. A new Native change does not create change-local Runtime, trajectory, checkpoint, receipt, or evidence files.
...
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| `docs/comet/changes/<name>/comet-state.yaml`                      | Portable Native state: phase, Loop, acceptance, checks, blockers, and next action                             | Runtime; Agent follows continuation |
| `docs/comet/changes/<name>/brief.md`, `specs/`, `verification.md` | Native user-readable requirements, target Specs, and verification report                                      | Agent and Runtime                   |
| `.comet/runtime/native/`                                          | Native device-local execution state, logs, locks, and transactions                                            | Runtime                             |
...
- **Commit**: Native `docs/comet/changes/` (including `brief.md`, Specs, `comet-state.yaml`, and `verification.md` once generated), the active Classic root (`docs/openspec/` for new projects or `openspec/` for retained legacy layouts), `docs/superpowers/`, `.comet/config.yaml`, and `.comet/skill-preferences.yaml`.
