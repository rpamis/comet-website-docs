...
If a session ends, context is compressed, or you switch devices, enter `/comet` again to resume. Comet reads the project-root configuration, enters `/comet-native` or `/comet-classic`, and lets that workflow continue from its own disk-backed state. Neither side scans, guesses, or converts changes from the other.
...
Comet does not depend on conversation history. Native rereads `.comet/config.yaml`, the change's `brief.md`, Specs, `comet-state.yaml`, and the implementation. Classic rereads the same project configuration, OpenSpec, `.comet.yaml`, and runtime evidence.

<Tip>
  <strong>
    You do not need to run <code>comet status</code> or{" "}
    <code>comet doctor</code> before recovery.
  </strong>{" "}
  Those are diagnostic commands for when something <strong>looks wrong</strong>,
  such as broken CLI or Skill installation, incorrect change routing, or
  inconsistent evidence. Normal recovery is one step: <code>/comet</code>.
</Tip>
...
<Note>
  This section explains the <strong>background mechanics</strong>: after you
  type <code>/comet continue</code>, how the Native workflow rebuilds the
  breakpoint from on-disk state internally. You don't need to run any of the
  commands here manually.
</Note>
...
<Note>
  This section is also <strong>background mechanics</strong>: it explains which
  files <code>/comet-classic</code> re-reads internally and the rules it uses to
  route to the next phase. You only need to type <code>/comet continue</code> —
  the routing below is handled automatically by Comet.
</Note>

`/comet-classic` can recover through a few mechanisms:
...
### Classic change workspace reuse

When creating or resuming a Classic change, Comet first reads the branch and worktree binding recorded for that change. It reuses a matching registered worktree; if the branch still exists but the worktree is missing, recovery recreates it. If the current directory, branch, or worktree does not match, Comet pauses for a choice instead of silently using another workspace.

From the target project directory, enter `/comet` or `/comet continue`. If several Classic changes are active, select the target first and then let Classic continue; do not copy change directories by hand to “recover” work.

## Resume across devices and platforms

Portable Native recovery uses the repository's `.comet/config.yaml`, `docs/comet/changes/<name>/brief.md`, Specs, and `comet-state.yaml`; include `verification.md` once it exists. The device-local `.comet/runtime/native/` contains execution state, logs, locks, and transactions and is not required for cross-device recovery. Classic changes continue to use `.comet.yaml`, OpenSpec, and `docs/superpowers/`. Open the same repository on another device or agent platform and `/comet` can recover from the corresponding files.

<Note>
  Uncommitted worktree changes do not travel with <code>git push</code>. Commit
  and push before switching devices, or the new device will recover from a clean
  worktree. Checked tasks in <code>tasks.md</code> still persist when committed.
</Note>

### Cross-device recovery walkthrough

The image below shows a real cross-device recovery. As long as the repository is the same, `/comet` reconstructs the checkpoint from file state and continues on any device or platform, without additional context transfer.

<img
  src="/img/comet-zero-resume.png"
  alt="Zero-context Comet recovery across devices"
/>
...
<Tip>
  When you <strong>know the current Classic phase</strong>, this rule lets you
  skip the routing overhead of <code>/comet-classic</code> and work directly. If
  you remember incorrectly, the entry check stops the mismatch before it damages
  state.
</Tip>
...
<Warning>
  A dirty worktree represents code facts only. It does{" "}
  <strong>not automatically advance</strong> the <code>phase</code> in{" "}
  <code>.comet.yaml</code> or check tasks in <code>tasks.md</code>. State
...
- **Verification failed** (`verify_result: fail`): you decide whether to repair or accept the deviation.
- **A genuine plan-ready pause**: you choose isolation and execution mode.
- **Missing Build decisions**: you choose `isolation`, `build_mode`, or `tdd_mode`.
- **Unattributable uncommitted changes**: you explain where the changes belong.
- **Multiple active changes**: you select the target to recover.

Other phase-specific pauses are described in [Five-phase decision points](/en/concepts/decision-points). Outside these cases, `/comet-classic` continues on its own.
