...
If restored through natural language, multiple active changes will not be automatically guessed; The recovery detection will return `ask_user`, waiting for you to name the target.
...
### Cross-device 0-context breakpoint recovery of the scene
...
<img src="/img/comet-zero-resume.png" alt="Cross-device 0-context breakpoint recovery of the scene" />
...
This is the deterministic guarantee brought by the progressive loading of 0.4.0-beta.1: when each sub-skill enters, it will first locate the script through `comet/reference/scripts.md`, and then run the ** entry check or recovery check ** of its own stage. The stage is subject to the current file status.
...
Internal routing
Directly enter the corresponding stage, and at the same time ensure that even if the stage is remembered wrongly, the status will not be disrupted: The entry check will block the entry of mismatched entries.
</Tip>
...
|"Situation|"Behavior"|
| ------------------------------------------------------------------------ | ------------------------------------------------------------------- |
|`build_pause: plan-ready`, but `isolation`/`build_mode` have already been set|** Automatically clear ** stale pause and continue execution (no action required)|
|`build_pause: plan-ready` and plan exist, but `isolation`/`build_mode` are not set|Return to the Plan-Ready recovery point, ** Let you select ** isolation and execution mode (without regenerating the plan)|
...
## The situation where your intervention is truly necessary
