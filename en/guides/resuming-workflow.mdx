...
description: "After a session interruption, context compression, or device switch, use /comet to resume interrupted Classic work."
---
...
Comet does not depend on conversation history. Classic rereads the project configuration, OpenSpec, `.comet.yaml`, and runtime evidence.
...
Portable Classic recovery uses repository files: `.comet/config.yaml`, the OpenSpec artifact root (`docs/openspec/` for new projects), the change's `.comet.yaml` and runtime evidence, and `docs/superpowers/`. Open the same repository on another device or agent platform and `/comet` can recover from the corresponding files. For Native change recovery across devices, see [Recovery and troubleshooting](/en/native/recovery-playbook).
