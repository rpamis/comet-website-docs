...
<Note>
  This section explains the <strong>background mechanics</strong>: after you type{' '}
  <code>/comet continue</code>, how the Native workflow rebuilds the breakpoint from on-disk state
  internally. You don't need to run any of the commands here manually.
</Note>
...
<Note>
  This section is also <strong>background mechanics</strong>: it explains which files{' '}
  <code>/comet-classic</code> re-reads internally and the rules it uses to route to the next phase.
  You only need to type <code>/comet continue</code> — the routing below is handled automatically by
  Comet.
</Note>
