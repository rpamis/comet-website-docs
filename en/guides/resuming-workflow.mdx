...
That is all. Comet does not depend on conversation history. Native rereads `.comet/config.yaml`, the change's `brief.md`, Specs, `comet-state.yaml`, and the implementation. Classic rereads the same project configuration, OpenSpec, `.comet.yaml`, and runtime evidence.
