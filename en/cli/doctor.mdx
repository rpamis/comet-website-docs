...
`comet doctor` is used to locate the reasons why Comet is not working properly.

It will check the installation environment, CLI and dependencies, project directory, Skill/Rule/Hook, Runtime, CodeGraph, and the current status of change. `comet status` mainly tells you where change has come. `comet doctor` mainly tells you where the environment or project is incomplete.
...
CodeGraph now reports in four dimensions: whether the CLI is installed, the current project index status, whether the detected Agent is registered for MCP, and whether each detected Agent has valid capabilities. The update of the project index itself does not represent that the Agent has registered a valid CodeGraph MCP; When detailed status is needed, you can check the `codegraph` objects in the JSON output.
...
| `--repair`              |Fix the Hook, Rule and deterministic selection states in Comet management.|
...
| `--yes`                 |Authorization supports the repair of authorized projects, such as CodeGraph indexes; It must be used together with `--repair`.|
...
- `⚠`: Detected warnings or missing optional components. It is usually possible to continue working, but the output will provide suggested actions.
...
`--repair` will perform a write operation. When the migration of the Classic root directory is interrupted, select `continue` or `rollback` according to the scene. When fixing projects that require authorization, such as CodeGraph, add `--yes`. If you only want to recheck, please omit `--repair`.
