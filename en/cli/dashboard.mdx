...
Starting in 0.4.0-beta.5, artifact preview is upgraded:

- Artifact drawers now render full Markdown (tables, quotes, task lists, syntax highlighting, and Mermaid diagrams) instead of the previous subset renderer.
- `.comet.yaml` / YAML and handoff / checkpoint JSON artifacts render as structured tables instead of raw text.
- Fullscreen mode adds an expand/collapse control and a table of contents when headings exist; the side-panel preview stays distraction-free without a TOC.
- Long artifact paths wrap in the drawer header and offer a one-click copy control.
- Preview HTML is sanitized with DOMPurify before injection, and Mermaid runs with `securityLevel: 'strict'`, so untrusted artifact content cannot execute scripts.
...
The front end auto-refreshes every **30 seconds**. There is also a manual refresh button, a search box, and tabs (active/archived/all). The artifact drawer includes a full Markdown renderer (tables, syntax highlighting, Mermaid diagrams, and structured YAML/JSON tables); see [Dashboard overview](/en/dashboard/overview).
