...
description: "Start a local browser dashboard to view Classic and Native workflows and manage plugins."
---

`comet dashboard` launches a **local** browser dashboard.
It provides Classic/OpenSpec and Native data in a single snapshot, and the workflow switch in the upper-left toggles between the two views.

Viewing workflows does not advance or modify Changes. Personal Memory and Project Knowledge management actions in the plugin center update their respective stores.
...
The Classic workspace reads `docs/openspec/changes/` or the legacy `openspec/changes/` path according to `classic.artifact_layout`, including the matching `changes/archive/` directory. The Native workspace reads the configured Native artifact root from `.comet/config.yaml`.

Both data paths are read-only and use partial-failure tolerance. A Native collection failure does not block the Classic snapshot. The page shows one workflow view at a time, and switching views does not change project state.
...
The Native workspace provides Active, Archived, and All filters. Its status data comes directly from Native Runtime projections, archive preflight, and conflict detection:
...
The Native workspace shows only limited checkpoint and evidence summaries; it does not expose hashes, Runtime paths, raw evidence, or other internal state files, and it offers no progression, repair, or archive controls. Use the `comet native` commands for complete detail and all writes.

With `--json`, the same snapshot includes a `native` field with schema `comet.dashboard.native.v2` when Native is available. The field is omitted when the project does not enable Native.
