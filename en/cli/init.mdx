...
`comet init` is the setup and onboarding entry for Comet.
It detects the current project environment, installs required Skills and runtime assets, and writes the default workflow used by `/comet`.
...
Interactive initialization supports Native, Classic, or both at project and global scope. At project scope, non-interactive initialization chooses the default workflow in this order:

1. Read valid `.comet/config.yaml` first. It is the source of truth.
2. If `--workflow native|classic|both` is provided, honor the explicit choice. Under `both`, `/comet` defaults to Native.
3. If workflow config is missing but Classic `.comet.yaml` state or legacy Ambient Resume evidence exists, keep Classic.
4. In all other cases, default to Native. Existing code, `openspec/`, or `docs/superpowers/` does not change that default.
...
At global scope, `--workflow` selects which capabilities are installed globally. On the first explicit `/comet` invocation in an unconfigured project, Comet snapshots those defaults into the project's `.comet/config.yaml`; later global-default changes do not rewrite an activated project. Use project initialization when you want project-local Skill, Rule, and Hook copies or explicit overrides. Non-interactive global initialization keeps Classic as the default when `--workflow` is omitted. `--root` remains valid only for project-scope Native initialization.
...
See [Supported platforms · Installation mode](/en/platforms#installation-mode-copy-vs-symbolic-link) for details and guidance.
