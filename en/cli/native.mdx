...
`comet native` is the deterministic Runtime entry for the Native workflow. It reads the shared `.comet/config.yaml` project configuration and operates on the configured `<artifact-root>/comet/`. It neither reads nor converts Classic/OpenSpec changes and never invokes external Skills, Git, a shell, or project commands.
