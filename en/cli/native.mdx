...
`change-name` and capability IDs start with a letter and use lowercase kebab-case. `new` creates default configuration and `<project>/docs/comet/` when configuration is absent. `--creation-authorization` must reference an authorization file issued by an external controller and bound to the current change. The implementation Agent cannot authorize itself with a project-local key.
...
- The second recurrence of the same semantic gap warns; the third recurrence without progress changes continuation to manual stop.
- One explicit new hypothesis may override the exact signature returned by `status` once with a non-empty summary.
...
Only a user-confirmed contract change starts a new goal cycle. At a stop boundary, let the user decide whether to change the goal or constraints or end the work. Do not keep calling `next`, weaken checks, or archive early.
