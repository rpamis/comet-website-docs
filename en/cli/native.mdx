...
When Verify is submitted as `pass`, the Runtime runs this built-in required check automatically and binds its receipt to the current verification. `comet native check` remains available to preview the same check earlier.
...
- The second recurrence of the same semantic gap warns. The third recurrence without progress enters the `repair-stagnation-stop` recovery branch.
- While an override remains, the Agent forms one explicit new repair hypothesis, then continues once with the exact signature returned by `status` and a non-empty summary.
- The default limit is five Verify failures per contract. Set `native.max_verify_failures` to a positive integer to change it. The Runtime requires a user decision only after the override or verification budget is exhausted.

Only a user-confirmed contract change starts a new goal cycle. In a recovery branch, form a new hypothesis from the finding instead of repeatedly calling `next`, weakening checks, or archiving early. Wait for the user only when the continuation explicitly requires a user decision.
