...
Only the Classic path installs OpenSpec, Superpowers, and CodeGraph. Project-scope installation deploys one shared workflow Rule and, where Hooks are supported, one Router. Global initialization installs global capabilities only; it does not select a default workflow for a project.
