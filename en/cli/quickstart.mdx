---
title: 'CLI quickstart'
description: 'Get Comet running with three commands: init, status, and doctor. New users start here.'
---

This page is the shortest onboarding path for the Comet CLI. Just three commands cover installation, checking, and recovery. For the full options of each command, see the [advanced topics](#advanced-topics).

## Prerequisites

- Node.js 22+
- npm/npx
- Git

<Info>
  Since 0.4.0-beta.1, Comet is a pure Node runtime and no longer requires Bash, Git Bash, or WSL.
  macOS, Linux, and Windows use the same set of commands.
</Info>

## Step 1: Install Comet

Install the CLI globally:

```bash
npm install -g @rpamis/comet
```

## Step 2: Initialize the project

Run it in the project root:

```bash
comet init
```

It detects your AI coding platform. A new project-scope initialization installs self-contained Native by default and writes `.comet/config.yaml` plus `docs/comet/`; projects with clear existing Classic state keep their current workflow.

Choose explicitly when needed:

```bash
comet init --workflow native --root docs
comet init --workflow classic
comet init --workflow both
comet init --scope global --workflow native
comet init --scope global --workflow both
```

Only the Classic path installs OpenSpec, Superpowers, and CodeGraph. Project-scope installation deploys one shared workflow Rule and, where Hooks are supported, one Router. Global initialization also offers Native, Classic, or both and installs the selected capabilities globally, but it does not select a default workflow for a project. Non-interactive global initialization keeps Classic as the default when `--workflow` is omitted.

### Troubleshooting: comet doctor

If `comet status` shows something abnormal, or the installation looks off:

```bash
comet doctor
```

It checks both project-level and global installations, platform directories, Skills, scripts, and active change status, and provides recovery hints.

## Update and uninstall

Refresh only the Comet-managed Skills, hooks, and rules in the current project without changing
the installed npm package version:

```bash
comet update --current-project
```

To upgrade the Comet npm package at the same time, explicitly enable self-update:

```bash
comet update --current-project --self-update
```

Self-update validates the candidate first and refuses to downgrade the current installation to an
older version.

Uninstall Comet-managed Skills, hooks, and rules while preserving your OpenSpec changes, design documents, and code:

```bash
comet uninstall
```

## FAQ

<Accordion title="Where does comet init install to?">
  By default it detects the AI coding platform directories in the current project. Use `--scope
  project` for project-level only, or `--scope global` for a global install.
</Accordion>

<Accordion title="Do scripts run on Windows?">
  Since 0.4.0-beta.1, all scripts run through Node.js, so Windows no longer requires Git Bash or
  WSL.
</Accordion>

## Advanced topics

The basic path ends here. Below are the full command references grouped by scenario:

| Advanced topic                       | Suited for                                          |
| ------------------------------------ | --------------------------------------------------- |
| [comet init](/en/cli/init)           | Full initialization options and component selection |
| [comet status](/en/cli/status)       | Full status fields and runtime data                 |
| [comet dashboard](/en/cli/dashboard) | Local read-only browser dashboard                   |
| [comet doctor](/en/cli/doctor)       | Full diagnostic scope and troubleshooting flow      |
| [comet update](/en/cli/update)       | Upgrade flow and language switching                 |
| [comet uninstall](/en/cli/uninstall) | Safe uninstall and scope control                    |

### Skill, evaluation, and publishing commands

These three groups of commands belong to advanced scenarios. Regular users typically use them indirectly through `/comet-any`:

| Advanced topic                   | Suited for                                             |
| -------------------------------- | ------------------------------------------------------ |
| [comet skill](/en/cli/skill)     | Local Skill package management and advanced Skill runs |
| [comet eval](/en/cli/eval)       | General-purpose Skill evaluation entry point           |
| [comet publish](/en/cli/publish) | Skill publishing and distribution                      |
| [comet bundle](/en/cli/bundle)   | Advanced Bundle backend (auditing/debugging)           |

To create reusable Skills, start with [Composing any Skill quickstart](/en/skill-creator/getting-started); to evaluate Skills, start with [Evaluation quickstart](/en/eval/quickstart).
