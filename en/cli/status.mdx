...
Run `comet status` when you take over a project or are unsure what to run next. It shows the default entry, active changes, and the recommended next action for each change.

`comet status` is fully read-only. It does not select a change, modify files, or run Build/Verify checks.
