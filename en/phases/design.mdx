...
<Steps>
  <Step title="Generate the OpenSpec → Superpowers handoff package">
    Before brainstorming begins, `comet handoff` generates a deterministic context package from the Phase 1 artifacts. This produces two files and writes their paths and a SHA256 hash to `.comet.yaml`:
