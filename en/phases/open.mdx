---
title: "open Stage"
description: "Understand how Classic open clarifies requirements, selects a workspace, and creates an OpenSpec change."
---
...
`/comet-open` is usually automatically routed after `/comet` enters Classic. The internal `/comet-classic` will read the active change list and file status, organize the routing context, and then the runtime score decides which phase to route to.
...
flowchart TD
    ...
    S1a -->|no| S1sum["Generate the five essential parts of the clarification summary"]
    Stop1 --> S1sum
    S1sum --> S2["Load openspec-new-change<br/> to derive the unique change name and tell you"]
    S2 --> S2b{"Pause point 3: Workspace decision<br/>current / branch / worktree"}
    S2b --> S3["Entry status verification + content integrity check"]
    S2 --> S3["Entry status verification + content integrity check"]
    S3 --> Stop4["Pause point 4: Review the three documents and confirm"]
    Stop4 --> Exit["comet-guard open --apply<br/> advances to the next stage"]
```
...
### Pause point 3: Workspace decision
...
| Option     | Meaning                                                                          |
| ---------- | -------------------------------------------------------------------------------- |
| `current`  | Work directly on the current branch and bind the current Git branch as-is        |
| `branch`   | Create a new branch in the current repository and confirm the branch name        |
| `worktree` | A separate worktree for parallel development (use it directly when you explicitly want to work in parallel) |

The choice is written to `isolation`; the current branch of the actual working directory is recorded as `bound_branch`, and later entry checks block accidental branch switches. The hotfix/tweak presets default to `current`.
...
There are 3 pause points in the open phase (globally numbered 2-4, with a total of 10 in the entire five-stage workflow. See [Five-stage Pause Points and User Selection points ](/en/concepts/decision-points)) :

|Pause point|When will it appear?|What are you going to do|
| -------------- | --------------------------- | ---------------------- |
|2 PRD split|The input is a large PRD or multiple independent capabilities|Split/keep as one/Adjust the plan|
|3 Workspace decision|Before creating the artifacts and `.comet.yaml`|current / branch / worktree|
|4 Document review|After the proposal/design/tasks are generated|Confirm the name, scope, and the three documents / adjust|
