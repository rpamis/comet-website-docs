...
<Tip>
  <strong>Normally you only need <code>/comet-classic</code>.</strong> <code>/comet-classic</code> has built-in intent recognition and reads file state to automatically determine that you're in the open phase and invoke <code>/comet-open</code>. The <code>/comet-open</code> described here is the phase Skill it calls internally — you only need to type <code>/comet-open</code> directly when you want <strong>manual control</strong> (for example, you've disabled <code>auto_transition</code>, or you want to run only this phase). See <a href="#how-this-phase-is-triggered">How this phase is triggered</a>.
</Tip>

## How this phase is triggered

`/comet-open` is usually **not a command you type yourself** — it's the result of `/comet-classic` routing automatically. `/comet-classic` first reads the active change list, organizes the routing context, and then the runtime scoring decides the route. Only when the route is `full`, or you explicitly choose the full workflow, does it enter `/comet-open`.

### Default: use /comet-classic for automatic recognition
...
You only need to type `/comet-classic` and describe what you want to do — it decides for itself whether a new change should be opened. After the open phase completes, `/comet-classic` also automatically continues to the next phase (design). See [Auto-transition mechanism](/en/concepts/auto-transition).
...
**If auto-transition is on, use `/comet-classic`; only when it's off do you need to use the phase commands manually.**
...
<Tip>
  After splitting, Comet <strong>does not</strong> automatically advance any single change to design. It pauses to ask which one you want to do first, advances only the one you choose, and keeps the rest active for future `/comet-classic` resumption.
</Tip>
...
<Warning>
  If <code>openspec instructions</code> fails, returns invalid JSON, or doesn't provide a usable <code>resolvedOutputPath</code>, Comet <strong>stops immediately</strong> and reports the OpenSpec error — it <strong>will not</strong> fall back to a hardcoded document structure, as that would bypass project rules. The change name must be the kebab-case name you confirmed; Comet won't expand or shrink scope on its own.
</Warning>
...
All operations in the open phase can be safely repeated. If `.comet.yaml` is already at `phase: open` and the three artifacts already exist, Comet **skips completed steps** and resumes from the first missing step. This means re-running `/comet-classic` after an interruption won't corrupt state.
...
<AccordionGroup>
  <Accordion title="What if .comet.yaml is missing">
    Don't bypass it with `/opsx:new` — it only creates OpenSpec artifacts and won't create `.comet.yaml`, leaving the change outside Comet's state machine. Re-run `/comet-classic` and let it backfill state through `/comet-open`.
  </Accordion>
