...
Normally, invoke `/comet`. When configuration selects Classic, the internal `/comet-classic` router dispatches `/comet-verify` when all tasks in `tasks.md` are checked. Call the phase Skill directly only for manual control. Both OpenSpec and Superpowers contribute to this phase.
