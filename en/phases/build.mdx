...
Run `/comet-build` directly, or let `/comet-classic` auto-dispatch after Phase 2 completes. If you paused at `build_pause: plan-ready`, running `/comet-classic` or `/comet-build` again resumes from where you left off without regenerating the plan.
...
<Steps>
  ...
  <Step title="Plan-ready pause — you choose isolation method and build mode">
    ...
    - **Pause to switch model** — record `build_pause: plan-ready` and stop this invocation; resume later with `/comet-classic` or `/comet-build` and Comet will reuse the existing plan without regenerating it
...
`build_pause: plan-ready` is a **recoverable pause point**, not a build mode. It records that the plan has been generated and the AI has stopped — waiting for you to resume, possibly in a different model or session. When you run `/comet-classic` or `/comet-build` again, Comet reads the existing plan and picks up from the isolation/execution-mode selection step. The plan is never regenerated on resume.
