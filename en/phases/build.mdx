---
title: "Phase 3: Plan and Build — Plan, Isolate, and Execute"
sidebarTitle: "Build"
description: "The Build phase creates an implementation plan, then executes it through your chosen isolation method and build mode — committing one task at a time."
---
...
<Steps>
  ...
  <Step title="Workspace is isolated">
    Based on your choice, the AI either creates a new git branch (`git checkout -b <change-name>`) or loads the Superpowers `using-git-worktrees` skill to set up a fully isolated worktree. If you chose `current` (hotfix/tweak), it continues in the existing workspace without creating a branch or worktree. The worktree path cannot be bypassed with plain shell commands — the skill must be loaded.
  </Step>
...
| `current` | Truthfully records the current workspace without creating a branch or worktree | Hotfix/tweak presets (default); full-workflow changes must use `branch` or `worktree` before leaving build |

A full-workflow change must select `branch` or `worktree` before it can leave the build phase; `current` is only valid for the hotfix and tweak presets.
...
- `isolation` must be set to `branch` or `worktree` (full workflow); hotfix/tweak presets may use `current`
...
```bash
comet state check <change-name> build --recover
```
