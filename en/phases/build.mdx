...
There are 2 pause points in the build phase (globally numbered 6-7, with a total of 10 in the entire five-stage workflow. See [Five-stage pause Points and User Selection points ](/en/concepts/decision-points)). The following is an explanation in the order of occurrence.
...
flowchart TD
    ...
    Stop3 --> Exec["Execute each task in build_mode"]
...
| **`standard`** |By default, no reviewer is dispatched per task; one is only dispatched when a task hits a risk signal. The final comprehensive review happens in the verify phase|** Default recommendation **, most common changes|
| `thorough`     |A reviewer (spec + quality) is dispatched for every task. The final comprehensive review happens in the verify phase|High risk, multi-module, architecture/security|
...
|Review|Intensity adjusts by `review_mode` (standard = no whole-change review inside build; thorough = segmented review every 3 tasks); the final comprehensive review happens in verify|Assign each task reviewer + repair agent according to review_mode (thorough= each task; standard= only risk tasks)|
...
|Over 50%|The number of newly added tasks exceeds half of the total number of initial tasks|**Pause point 7**: Split into a new change/Continue to complete within the current change|
